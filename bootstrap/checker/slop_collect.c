...
void collect_collect_types(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_195 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_195.has_value) {
                __auto_type expr = _mv_195.value;
    ...
    switch (_mv_196.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_196.data.lst;
            {
                ...
                __auto_type _mv_197 = ({ __auto_type _lst = items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_197.has_value) {
                    __auto_type name_expr = _mv_197.value;
                    __auto_type _mv_198 = (*name_expr);
                    switch (_mv_198.tag) {
                        case types_SExpr_sym:
                        {
                            __auto_type sym = _mv_198.data.sym;
                            {
                                ...
                                __auto_type _mv_199 = ({ __auto_type _lst = items; size_t _idx = (size_t)2; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void collect_collect_record_fields(env_TypeEnv* env, slop_arena* arena, types_ResolvedType* resolved, types_SExpr* record_expr) {
    ...
    __auto_type _mv_200 = (*record_expr);
    switch (_mv_200.tag) {
        case types_SExpr_lst:
        {
...
types_SExpr* collect_get_type_arg(types_SExpr* type_expr, int64_t idx) {
    ...
    switch (_mv_201.tag) {
        case types_SExpr_lst:
        {
            ...
            __auto_type _mv_202 = ({ __auto_type _lst = lst.items; size_t _idx = (size_t)idx; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_202.has_value) {
                __auto_type arg = _mv_202.value;
                return arg;
...
types_ResolvedType* collect_get_field_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    ...
    switch (_mv_203.tag) {
        case types_SExpr_sym:
        {
            ...
            {
                __auto_type type_name = sym.name;
                __auto_type _mv_204 = env_env_lookup_type(env, type_name);
                if (_mv_204.has_value) {
                    __auto_type t = _mv_204.value;
                    return t;
        ...
        case types_SExpr_lst:
        {
            ...
            {
                __auto_type items = lst.items;
                __auto_type _mv_205 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void collect_collect_module_types(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    {
        __auto_type arena = env_env_arena(env);
        __auto_type _mv_206 = (*module_form);
        switch (_mv_206.tag) {
            case types_SExpr_lst:
            {
...
slop_option_types_ResolvedType_ptr collect_lookup_payload_type(env_TypeEnv* env, slop_string type_name) {
    if (string_eq(type_name, SLOP_STR(""))) {
        ...
        __auto_type _mv_207 = env_env_lookup_type(env, type_name);
        if (_mv_207.has_value) {
            __auto_type t = _mv_207.value;
...
uint8_t collect_is_range_type_expr(types_SExpr* type_expr) {
    if (!(parser_sexpr_is_list(type_expr))) {
        ...
        if ((parser_sexpr_list_len(type_expr) < 2)) {
            ...
            __auto_type _mv_208 = parser_sexpr_list_get(type_expr, 0);
            if (_mv_208.has_value) {
                __auto_type first_elem = _mv_208.value;
                {
...
types_ResolvedType* collect_get_range_base_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_209 = parser_sexpr_list_get(type_expr, 0);
    if (_mv_209.has_value) {
        __auto_type first_elem = _mv_209.value;
        {
            __auto_type base_name = parser_sexpr_get_symbol_name(first_elem);
            if (string_eq(base_name, SLOP_STR("Int"))) {
                return env_env_get_int_type(env);
            } else if (string_eq(base_name, SLOP_STR("Float"))) {
                __auto_type _mv_210 = env_env_lookup_type(env, SLOP_STR("Float"));
                if (_mv_210.has_value) {
                    __auto_type t = _mv_210.value;
                    return t;
    ...
    if (parser_sexpr_is_list(union_expr)) {
        {
            ...
            for (int64_t i = 1; i < len; i++) {
                __auto_type _mv_212 = parser_sexpr_list_get(union_expr, i);
                if (_mv_212.has_value) {
                    __auto_type variant_form = _mv_212.value;
...
slop_option_types_ResolvedType_ptr collect_get_variant_payload_type(env_TypeEnv* env, types_SExpr* variant_form) {
    if ((parser_sexpr_list_len(variant_form) <= 1)) {
        ...
        __auto_type _mv_213 = parser_sexpr_list_get(variant_form, 1);
...
slop_string collect_get_variant_name(types_SExpr* variant_form) {
    if (parser_sexpr_is_list(variant_form)) {
        if ((parser_sexpr_list_len(variant_form) == 0)) {
            ...
            __auto_type _mv_214 = parser_sexpr_list_get(variant_form, 0);
            if (_mv_214.has_value) {
                __auto_type name_expr = _mv_214.value;
        ...
        __auto_type _mv_215 = (*variant_form);
        switch (_mv_215.tag) {
            case types_SExpr_sym:
            {
...
void collect_collect_constants(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type len = ((int64_t)((ast).len));
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_217 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_217.has_value) {
                __auto_type expr = _mv_217.value;
        ...
        if (parser_sexpr_is_list(module_form)) {
            {
                ...
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_218 = parser_sexpr_list_get(module_form, i);
                    if (_mv_218.has_value) {
                        __auto_type item = _mv_218.value;
...
void collect_collect_single_constant(env_TypeEnv* env, slop_arena* arena, types_SExpr* const_form) {
    ...
    if (parser_sexpr_is_list(const_form)) {
        if ((parser_sexpr_list_len(const_form) >= 3)) {
            __auto_type _mv_219 = parser_sexpr_list_get(const_form, 1);
            if (_mv_219.has_value) {
                __auto_type name_expr = _mv_219.value;
                {
                    __auto_type const_name = parser_sexpr_get_symbol_name(name_expr);
                    if (!(string_eq(const_name, SLOP_STR("")))) {
                        __auto_type _mv_220 = parser_sexpr_list_get(const_form, 2);
...
void collect_collect_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type len = ((int64_t)((ast).len));
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_223 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_223.has_value) {
                __auto_type expr = _mv_223.value;
...
void collect_collect_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    SLOP_PRE(((module_form != NULL)), "(!= module-form nil)");
    {
        __auto_type arena = env_env_arena(env);
        if (parser_sexpr_is_list(module_form)) {
            __auto_type _mv_224 = parser_sexpr_list_get(module_form, 1);
            ...
            }
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_225 = parser_sexpr_list_get(module_form, i);
                    if (_mv_225.has_value) {
                        __auto_type item = _mv_225.value;
...
void collect_collect_single_function(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form) {
    ...
    if (parser_sexpr_is_list(fn_form)) {
        if ((parser_sexpr_list_len(fn_form) >= 3)) {
            __auto_type _mv_226 = parser_sexpr_list_get(fn_form, 1);
            if (_mv_226.has_value) {
                __auto_type name_expr = _mv_226.value;
                {
                    ...
                    if (!(string_eq(fn_name, SLOP_STR("")))) {
                        {
                            ...
                            if (string_eq(fn_name, SLOP_STR("main"))) {
                                collect_validate_main_params(env, fn_form, params);
                            }
...
void collect_validate_main_params(env_TypeEnv* env, types_SExpr* fn_form, slop_list_types_ParamInfo params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        if ((num_params == 0)) {
            ...
            __auto_type _mv_227 = ({ __auto_type _lst = params; size_t _idx = (size_t)0; slop_option_types_ParamInfo _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_227.has_value) {
                __auto_type p0 = _mv_227.value;
                {
            ...
            __auto_type _mv_228 = ({ __auto_type _lst = params; size_t _idx = (size_t)1; slop_option_types_ParamInfo _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_228.has_value) {
                __auto_type p1 = _mv_228.value;
                {
...
slop_list_types_ParamInfo collect_collect_fn_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        __auto_type _mv_229 = parser_sexpr_list_get(fn_form, 2);
        if (_mv_229.has_value) {
            __auto_type params_expr = _mv_229.value;
...
types_ResolvedType* collect_find_fn_return_type(env_TypeEnv* env, types_SExpr* fn_form) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_230 = parser_sexpr_list_get(fn_form, i);
            if (_mv_230.has_value) {
                __auto_type item = _mv_230.value;
...
types_ResolvedType* collect_extract_spec_return_type(env_TypeEnv* env, types_SExpr* spec_form) {
    ...
    {
        __auto_type arena = env_env_arena(env);
        __auto_type _mv_231 = parser_sexpr_list_get(spec_form, 1);
        if (_mv_231.has_value) {
            __auto_type spec_body = _mv_231.value;
            if (parser_sexpr_is_list(spec_body)) {
                {
                    __auto_type len = parser_sexpr_list_len(spec_body);
                    __auto_type _mv_232 = parser_sexpr_list_get(spec_body, (len - 1));
                    if (_mv_232.has_value) {
                        __auto_type ret_expr = _mv_232.value;
