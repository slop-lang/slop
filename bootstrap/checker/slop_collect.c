...
void collect_collect_types(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_265 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_265.has_value) {
                __auto_type expr = _mv_265.value;
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_266 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_266.has_value) {
                __auto_type expr = _mv_266.value;
                if (parser_is_form(expr, SLOP_STR("type"))) {
            ...
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_267.data.lst;
                {
                    ...
                    if (_mv_268.has_value) {
                        __auto_type name_expr = _mv_268.value;
                        __auto_type _mv_269 = (*name_expr);
                        switch (_mv_269.tag) {
...
void collect_resolve_type_body(env_TypeEnv* env, slop_arena* arena, types_SExpr* expr) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_271 = (*expr);
    switch (_mv_271.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_271.data.lst;
            {
                __auto_type items = lst.items;
                ...
                if (_mv_272.has_value) {
                    __auto_type name_expr = _mv_272.value;
                    __auto_type _mv_273 = (*name_expr);
                    switch (_mv_273.tag) {
                        case types_SExpr_sym:
                        {
                            __auto_type sym = _mv_273.data.sym;
                            {
                                ...
                                if (_mv_274.has_value) {
                                    ...
                                    if (parser_is_form(type_expr, SLOP_STR("enum"))) {
                                        ...
                                        __auto_type _mv_275 = env_env_lookup_type_direct(env, type_name);
                                        if (_mv_275.has_value) {
                                            __auto_type resolved = _mv_275.value;
                                            collect_collect_record_fields(env, arena, resolved, type_expr);
                                        } else if (!_mv_275.has_value) {
                                        }
                                    } else if (parser_is_form(type_expr, SLOP_STR("union"))) {
                                        __auto_type _mv_276 = env_env_lookup_type_direct(env, type_name);
                                        ...
                                        __auto_type _mv_277 = env_env_lookup_type_direct(env, type_name);
                                        ...
                                        __auto_type _mv_278 = env_env_lookup_type_direct(env, type_name);
                                        if (_mv_278.has_value) {
                                            __auto_type resolved = _mv_278.value;
                                            {
                                        ...
                                        __auto_type _mv_279 = env_env_lookup_type_direct(env, type_name);
                                        if (_mv_279.has_value) {
                                            __auto_type resolved = _mv_279.value;
                                            {
                                        ...
                                        __auto_type _mv_280 = env_env_lookup_type_direct(env, type_name);
                                        if (_mv_280.has_value) {
                                            __auto_type resolved = _mv_280.value;
                                            {
                                                __auto_type elem_type = collect_get_field_type(env, arena, collect_get_type_arg(type_expr, 1));
                                                types_resolved_type_set_inner(resolved, elem_type);
                                            }
                                    ...
                                    } else {
                                        {
                                            ...
                                            if (!(string_eq(alias_name, SLOP_STR("")))) {
                                                __auto_type _mv_281 = env_env_lookup_type_direct(env, type_name);
...
void collect_collect_record_fields(env_TypeEnv* env, slop_arena* arena, types_ResolvedType* resolved, types_SExpr* record_expr) {
    ...
    __auto_type _mv_282 = (*record_expr);
    switch (_mv_282.tag) {
        case types_SExpr_lst:
        {
...
types_SExpr* collect_get_type_arg(types_SExpr* type_expr, int64_t idx) {
    ...
    __auto_type _mv_283 = (*type_expr);
    switch (_mv_283.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_283.data.lst;
            __auto_type _mv_284 = ({ __auto_type _lst = lst.items; size_t _idx = (size_t)idx; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
types_ResolvedType* collect_get_field_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    ...
    switch (_mv_285.tag) {
        case types_SExpr_sym:
        {
            ...
            {
                __auto_type type_name = sym.name;
                __auto_type _mv_286 = env_env_lookup_type_direct(env, type_name);
        ...
        case types_SExpr_lst:
        {
            ...
            {
                __auto_type items = lst.items;
                __auto_type _mv_287 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_287.has_value) {
                    __auto_type head_expr = _mv_287.value;
                    {
...
uint8_t collect_is_type_param(slop_string name, slop_list_string type_params) {
    {
        ...
        __auto_type found = 0;
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_288 = ({ __auto_type _lst = type_params; size_t _idx = (size_t)i; slop_option_string _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_288.has_value) {
                __auto_type tp = _mv_288.value;
...
types_ResolvedType* collect_get_field_type_generic(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr, slop_list_string type_params) {
    ...
    switch (_mv_289.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_289.data.sym;
            {
                ...
                if (collect_is_type_param(type_name, type_params)) {
                    ...
                    __auto_type _mv_290 = env_env_lookup_type_direct(env, type_name);
                    if (_mv_290.has_value) {
                        __auto_type t = _mv_290.value;
                        return t;
        ...
        case types_SExpr_lst:
        {
            ...
            {
                __auto_type items = lst.items;
                __auto_type _mv_291 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_291.has_value) {
                    __auto_type head_expr = _mv_291.value;
                    {
                        __auto_type head_name = parser_sexpr_get_symbol_name(head_expr);
                        if (string_eq(head_name, SLOP_STR("Option"))) {
                            {
...
slop_list_string collect_find_fn_type_params(slop_arena* arena, types_SExpr* fn_form) {
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_292 = parser_sexpr_list_get(fn_form, i);
            if (_mv_292.has_value) {
                __auto_type item = _mv_292.value;
                if (parser_is_form(item, SLOP_STR("@generic"))) {
                    __auto_type _mv_293 = parser_sexpr_list_get(item, 1);
                    if (_mv_293.has_value) {
                        __auto_type params_expr = _mv_293.value;
...
types_ResolvedType* collect_find_fn_return_type_generic(env_TypeEnv* env, types_SExpr* fn_form, slop_list_string type_params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_294 = parser_sexpr_list_get(fn_form, i);
            if (_mv_294.has_value) {
                __auto_type item = _mv_294.value;
...
types_ResolvedType* collect_extract_spec_return_type_generic(env_TypeEnv* env, types_SExpr* spec_form, slop_list_string type_params) {
    ...
    {
        __auto_type arena = env_env_arena(env);
        __auto_type _mv_295 = parser_sexpr_list_get(spec_form, 1);
        if (_mv_295.has_value) {
            __auto_type spec_body = _mv_295.value;
            if (parser_sexpr_is_list(spec_body)) {
                {
                    __auto_type len = parser_sexpr_list_len(spec_body);
                    __auto_type _mv_296 = parser_sexpr_list_get(spec_body, (len - 1));
                    if (_mv_296.has_value) {
                        __auto_type ret_expr = _mv_296.value;
...
slop_list_types_ResolvedType_ptr collect_collect_fn_spec_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form, slop_list_string type_params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        __auto_type len = parser_sexpr_list_len(fn_form);
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_297 = parser_sexpr_list_get(fn_form, i);
            if (_mv_297.has_value) {
                __auto_type item = _mv_297.value;
                if (parser_is_form(item, SLOP_STR("@spec"))) {
                    __auto_type _mv_298 = parser_sexpr_list_get(item, 1);
                    if (_mv_298.has_value) {
                        __auto_type spec_body = _mv_298.value;
                        if (parser_sexpr_is_list(spec_body)) {
                            __auto_type _mv_299 = parser_sexpr_list_get(spec_body, 0);
                            if (_mv_299.has_value) {
                                __auto_type param_types_expr = _mv_299.value;
...
void collect_set_module_name_from_form(env_TypeEnv* env, types_SExpr* module_form) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_300 = parser_sexpr_list_get(module_form, 1);
        ...
        if (parser_sexpr_is_list(module_form)) {
            collect_set_module_name_from_form(env, module_form);
            {
                ...
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_301 = parser_sexpr_list_get(module_form, i);
                    if (_mv_301.has_value) {
                        __auto_type item = _mv_301.value;
    ...
    SLOP_PRE(((module_form != NULL)), "(!= module-form nil)");
    {
        ...
        if (parser_sexpr_is_list(module_form)) {
            collect_set_module_name_from_form(env, module_form);
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_302 = parser_sexpr_list_get(module_form, i);
                    if (_mv_302.has_value) {
                        __auto_type item = _mv_302.value;
                        if (parser_is_form(item, SLOP_STR("type"))) {
...
slop_option_types_ResolvedType_ptr collect_lookup_payload_type(env_TypeEnv* env, slop_string type_name) {
    if (string_eq(type_name, SLOP_STR(""))) {
        ...
        __auto_type _mv_303 = env_env_lookup_type_direct(env, type_name);
...
uint8_t collect_is_range_type_expr(types_SExpr* type_expr) {
    if (!(parser_sexpr_is_list(type_expr))) {
        ...
        if ((parser_sexpr_list_len(type_expr) < 2)) {
            ...
            __auto_type _mv_304 = parser_sexpr_list_get(type_expr, 0);
            if (_mv_304.has_value) {
                __auto_type first_elem = _mv_304.value;
                {
...
types_ResolvedType* collect_get_range_base_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_305 = parser_sexpr_list_get(type_expr, 0);
    if (_mv_305.has_value) {
        __auto_type first_elem = _mv_305.value;
        {
            __auto_type base_name = parser_sexpr_get_symbol_name(first_elem);
            if (string_eq(base_name, SLOP_STR("Int"))) {
                return env_env_get_int_type(env);
            } else if (string_eq(base_name, SLOP_STR("Float"))) {
                __auto_type _mv_306 = env_env_lookup_type_direct(env, SLOP_STR("Float"));
    ...
    if (parser_sexpr_is_list(union_expr)) {
        {
            ...
            for (int64_t i = 1; i < len; i++) {
                __auto_type _mv_308 = parser_sexpr_list_get(union_expr, i);
                if (_mv_308.has_value) {
                    __auto_type variant_form = _mv_308.value;
...
slop_option_types_ResolvedType_ptr collect_get_variant_payload_type(env_TypeEnv* env, types_SExpr* variant_form) {
    if ((parser_sexpr_list_len(variant_form) <= 1)) {
        ...
        __auto_type _mv_309 = parser_sexpr_list_get(variant_form, 1);
...
slop_string collect_checker_get_variant_name(types_SExpr* variant_form) {
    if (parser_sexpr_is_list(variant_form)) {
        if ((parser_sexpr_list_len(variant_form) == 0)) {
            ...
            __auto_type _mv_310 = parser_sexpr_list_get(variant_form, 0);
            if (_mv_310.has_value) {
                __auto_type name_expr = _mv_310.value;
        ...
        __auto_type _mv_311 = (*variant_form);
        switch (_mv_311.tag) {
            case types_SExpr_sym:
            {
...
void collect_collect_constants(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type len = ((int64_t)((ast).len));
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_313 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_313.has_value) {
                __auto_type expr = _mv_313.value;
        ...
        if (parser_sexpr_is_list(module_form)) {
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_314 = parser_sexpr_list_get(module_form, i);
                    if (_mv_314.has_value) {
                        __auto_type item = _mv_314.value;
...
void collect_collect_single_constant(env_TypeEnv* env, slop_arena* arena, types_SExpr* const_form) {
    ...
    if (parser_sexpr_is_list(const_form)) {
        if ((parser_sexpr_list_len(const_form) >= 3)) {
            __auto_type _mv_315 = parser_sexpr_list_get(const_form, 1);
            if (_mv_315.has_value) {
                __auto_type name_expr = _mv_315.value;
                {
                    __auto_type const_name = parser_sexpr_get_symbol_name(name_expr);
                    if (!(string_eq(const_name, SLOP_STR("")))) {
                        __auto_type _mv_316 = parser_sexpr_list_get(const_form, 2);
...
types_ResolvedType* collect_get_const_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    ...
    switch (_mv_317.tag) {
        case types_SExpr_sym:
        {
            ...
            {
                __auto_type type_name = sym.name;
                __auto_type _mv_318 = env_env_lookup_type_direct(env, type_name);
                if (_mv_318.has_value) {
                    __auto_type t = _mv_318.value;
                    return t;
...
void collect_collect_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type len = ((int64_t)((ast).len));
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_319 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_319.has_value) {
                __auto_type expr = _mv_319.value;
...
void collect_collect_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    SLOP_PRE(((module_form != NULL)), "(!= module-form nil)");
    {
        __auto_type arena = env_env_arena(env);
        if (parser_sexpr_is_list(module_form)) {
            __auto_type _mv_320 = parser_sexpr_list_get(module_form, 1);
            ...
            }
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_321 = parser_sexpr_list_get(module_form, i);
                    if (_mv_321.has_value) {
                        __auto_type item = _mv_321.value;
    ...
    if (parser_sexpr_is_list(ffi_form)) {
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_322 = parser_sexpr_list_get(ffi_form, i);
                if (_mv_322.has_value) {
                    __auto_type func_decl = _mv_322.value;
    ...
    if (parser_sexpr_is_list(func_decl)) {
        {
            ...
            if ((decl_len >= 3)) {
                __auto_type _mv_323 = parser_sexpr_list_get(func_decl, 0);
                if (_mv_323.has_value) {
                    __auto_type name_expr = _mv_323.value;
                    {
                ...
                __auto_type _mv_324 = parser_sexpr_list_get(func_decl, 0);
                if (_mv_324.has_value) {
                    __auto_type name_expr = _mv_324.value;
                    {
                        __auto_type const_name = parser_sexpr_get_symbol_name(name_expr);
                        if (!(string_eq(const_name, SLOP_STR("")))) {
                            __auto_type _mv_325 = parser_sexpr_list_get(func_decl, 1);
...
slop_list_types_ParamInfo collect_collect_ffi_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* func_decl) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type _mv_326 = parser_sexpr_list_get(func_decl, 1);
        if (_mv_326.has_value) {
            __auto_type params_expr = _mv_326.value;
            if (parser_sexpr_is_list(params_expr)) {
                {
                    __auto_type num_params = parser_sexpr_list_len(params_expr);
...
types_ResolvedType* collect_get_ffi_return_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* func_decl) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_327 = parser_sexpr_list_get(func_decl, 2);
...
void collect_collect_single_function(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form) {
    ...
    if (parser_sexpr_is_list(fn_form)) {
        if ((parser_sexpr_list_len(fn_form) >= 3)) {
            __auto_type _mv_328 = parser_sexpr_list_get(fn_form, 1);
            if (_mv_328.has_value) {
                __auto_type name_expr = _mv_328.value;
                {
                    __auto_type fn_name = parser_sexpr_get_symbol_name(name_expr);
                    if (!(string_eq(fn_name, SLOP_STR("")))) {
                        {
...
void collect_validate_main_params(env_TypeEnv* env, types_SExpr* fn_form, slop_list_types_ParamInfo params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        if ((num_params == 0)) {
            ...
            __auto_type _mv_329 = ({ __auto_type _lst = params; size_t _idx = (size_t)0; slop_option_types_ParamInfo _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_329.has_value) {
                __auto_type p0 = _mv_329.value;
                {
            ...
            __auto_type _mv_330 = ({ __auto_type _lst = params; size_t _idx = (size_t)1; slop_option_types_ParamInfo _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_330.has_value) {
                __auto_type p1 = _mv_330.value;
                {
...
slop_list_types_ParamInfo collect_collect_fn_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form) {
    ...
    {
        __auto_type params = ((slop_list_types_ParamInfo){ .data = (types_ParamInfo*)slop_arena_alloc(arena, 16 * sizeof(types_ParamInfo)), .len = 0, .cap = 16 });
        __auto_type _mv_331 = parser_sexpr_list_get(fn_form, 2);
        if (_mv_331.has_value) {
            __auto_type params_expr = _mv_331.value;
            if (parser_sexpr_is_list(params_expr)) {
                {
                    __auto_type num_params = parser_sexpr_list_len(params_expr);
...
slop_list_types_ParamInfo collect_collect_fn_params_generic(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form, slop_list_string type_params) {
    ...
    {
        __auto_type params = ((slop_list_types_ParamInfo){ .data = (types_ParamInfo*)slop_arena_alloc(arena, 16 * sizeof(types_ParamInfo)), .len = 0, .cap = 16 });
        __auto_type _mv_332 = parser_sexpr_list_get(fn_form, 2);
        if (_mv_332.has_value) {
            __auto_type params_expr = _mv_332.value;
            if (parser_sexpr_is_list(params_expr)) {
                {
                    __auto_type num_params = parser_sexpr_list_len(params_expr);
...
types_ResolvedType* collect_find_fn_return_type(env_TypeEnv* env, types_SExpr* fn_form) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        types_ResolvedType* found_type = env_env_get_unit_type(env);
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_333 = parser_sexpr_list_get(fn_form, i);
            if (_mv_333.has_value) {
                __auto_type item = _mv_333.value;
                if (parser_is_form(item, SLOP_STR("@spec"))) {
                    if (!(found)) {
...
types_ResolvedType* collect_checker_extract_spec_return_type(env_TypeEnv* env, types_SExpr* spec_form) {
    ...
    {
        __auto_type arena = env_env_arena(env);
        __auto_type _mv_334 = parser_sexpr_list_get(spec_form, 1);
        if (_mv_334.has_value) {
            __auto_type spec_body = _mv_334.value;
            if (parser_sexpr_is_list(spec_body)) {
                {
                    __auto_type len = parser_sexpr_list_len(spec_body);
                    __auto_type _mv_335 = parser_sexpr_list_get(spec_body, (len - 1));
                    if (_mv_335.has_value) {
                        __auto_type ret_expr = _mv_335.value;
                        return collect_get_field_type(env, arena, ret_expr);
