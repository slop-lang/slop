...
void collect_collect_types(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_99 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_99.has_value) {
                __auto_type expr = _mv_99.value;
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_100 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_100.has_value) {
                __auto_type expr = _mv_100.value;
                if (parser_is_form(expr, SLOP_STR("type"))) {
...
void collect_register_type_name(env_TypeEnv* env, slop_arena* arena, types_SExpr* expr) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        switch (_mv_101.tag) {
            ...
            {
                __auto_type lst = _mv_101.data.lst;
                {
...
void collect_resolve_type_body(env_TypeEnv* env, slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_105.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_105.data.lst;
            {
                __auto_type items = lst.items;
                ...
                if (_mv_106.has_value) {
                    __auto_type name_expr = _mv_106.value;
                    __auto_type _mv_107 = (*name_expr);
                    switch (_mv_107.tag) {
                        case types_SExpr_sym:
                        {
                            ...
                            {
                                __auto_type type_name = sym.name;
                                __auto_type _mv_108 = ({ __auto_type _lst = items; size_t _idx = (size_t)2; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_108.has_value) {
                                    __auto_type type_expr = _mv_108.value;
                                    if (parser_is_form(type_expr, SLOP_STR("enum"))) {
                                        collect_collect_enum_variants(env, type_name, type_expr);
                                    } else if (parser_is_form(type_expr, SLOP_STR("record"))) {
                                        __auto_type _mv_109 = env_env_lookup_type_direct(env, type_name);
                                        if (_mv_109.has_value) {
                                            __auto_type resolved = _mv_109.value;
                                            collect_collect_record_fields(env, arena, resolved, type_expr);
                                        } else if (!_mv_109.has_value) {
                                        }
                                    } else if (parser_is_form(type_expr, SLOP_STR("union"))) {
                                        __auto_type _mv_110 = env_env_lookup_type_direct(env, type_name);
                                        if (_mv_110.has_value) {
                                            __auto_type resolved = _mv_110.value;
...
void collect_collect_record_fields(env_TypeEnv* env, slop_arena* arena, types_ResolvedType* resolved, types_SExpr* record_expr) {
    ...
    __auto_type _mv_117 = (*record_expr);
    switch (_mv_117.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_117.data.lst;
            {
                __auto_type items = lst.items;
                ...
                for (int64_t i = 1; i < len; i++) {
                    ...
                    if (_mv_118.has_value) {
                        __auto_type field_form = _mv_118.value;
                        __auto_type _mv_119 = (*field_form);
                        switch (_mv_119.tag) {
                            case types_SExpr_lst:
                            {
                                __auto_type field_lst = _mv_119.data.lst;
                                {
                                    __auto_type field_items = field_lst.items;
                                    __auto_type _mv_120 = ({ __auto_type _lst = field_items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_120.has_value) {
                                        __auto_type name_expr = _mv_120.value;
                                        __auto_type _mv_121 = (*name_expr);
                                        switch (_mv_121.tag) {
                                            case types_SExpr_sym:
                                            {
                                                __auto_type name_sym = _mv_121.data.sym;
                                                __auto_type _mv_122 = ({ __auto_type _lst = field_items; size_t _idx = (size_t)1; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                if (_mv_122.has_value) {
                                                    __auto_type type_expr = _mv_122.value;
                                                    {
...
types_SExpr* collect_get_type_arg(types_SExpr* type_expr, int64_t idx) {
    ...
    __auto_type _mv_123 = (*type_expr);
    switch (_mv_123.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_123.data.lst;
            __auto_type _mv_124 = ({ __auto_type _lst = lst.items; size_t _idx = (size_t)idx; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_124.has_value) {
                __auto_type arg = _mv_124.value;
                return arg;
...
types_ResolvedType* collect_get_field_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    ...
    switch (_mv_125.tag) {
        case types_SExpr_sym:
        {
            ...
            {
                __auto_type type_name = sym.name;
                __auto_type _mv_126 = env_env_lookup_type_direct(env, type_name);
                if (_mv_126.has_value) {
                    __auto_type t = _mv_126.value;
                    return t;
        ...
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_125.data.lst;
            {
                ...
                if (_mv_127.has_value) {
                    __auto_type head_expr = _mv_127.value;
                    {
...
uint8_t collect_is_type_param(slop_string name, slop_list_string type_params) {
    {
        ...
        uint8_t found = 0;
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_128 = ({ __auto_type _lst = type_params; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_128.has_value) {
                __auto_type tp = _mv_128.value;
...
types_ResolvedType* collect_get_field_type_generic(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr, slop_list_string type_params) {
    ...
    switch (_mv_129.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_129.data.sym;
            {
                ...
                if (collect_is_type_param(type_name, type_params)) {
                    ...
                    __auto_type _mv_130 = env_env_lookup_type_direct(env, type_name);
                    if (_mv_130.has_value) {
                        __auto_type t = _mv_130.value;
                        return t;
        ...
        case types_SExpr_lst:
        {
            ...
            {
                __auto_type items = lst.items;
                __auto_type _mv_131 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_131.has_value) {
                    __auto_type head_expr = _mv_131.value;
                    {
                        __auto_type head_name = parser_sexpr_get_symbol_name(head_expr);
                        if (string_eq(head_name, SLOP_STR("Option"))) {
                            {
...
slop_list_string collect_find_fn_type_params(slop_arena* arena, types_SExpr* fn_form) {
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_132 = parser_sexpr_list_get(fn_form, i);
            if (_mv_132.has_value) {
                __auto_type item = _mv_132.value;
                if (parser_is_form(item, SLOP_STR("@generic"))) {
                    __auto_type _mv_133 = parser_sexpr_list_get(item, 1);
                    if (_mv_133.has_value) {
                        __auto_type params_expr = _mv_133.value;
                        if (parser_sexpr_is_list(params_expr)) {
                            {
                                ...
                                for (int64_t j = 0; j < num_params; j++) {
                                    __auto_type _mv_134 = parser_sexpr_list_get(params_expr, j);
                                    if (_mv_134.has_value) {
                                        __auto_type param_expr = _mv_134.value;
                                        {
...
types_ResolvedType* collect_find_fn_return_type_generic(env_TypeEnv* env, types_SExpr* fn_form, slop_list_string type_params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_135 = parser_sexpr_list_get(fn_form, i);
            if (_mv_135.has_value) {
                __auto_type item = _mv_135.value;
...
types_ResolvedType* collect_extract_spec_return_type_generic(env_TypeEnv* env, types_SExpr* spec_form, slop_list_string type_params) {
    ...
    {
        __auto_type arena = env_env_arena(env);
        __auto_type _mv_136 = parser_sexpr_list_get(spec_form, 1);
        if (_mv_136.has_value) {
            __auto_type spec_body = _mv_136.value;
            if (parser_sexpr_is_list(spec_body)) {
                {
                    __auto_type len = parser_sexpr_list_len(spec_body);
                    __auto_type _mv_137 = parser_sexpr_list_get(spec_body, (len - 1));
                    if (_mv_137.has_value) {
                        __auto_type ret_expr = _mv_137.value;
...
slop_list_types_ResolvedType_ptr collect_collect_fn_spec_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form, slop_list_string type_params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        __auto_type len = parser_sexpr_list_len(fn_form);
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_138 = parser_sexpr_list_get(fn_form, i);
            if (_mv_138.has_value) {
                __auto_type item = _mv_138.value;
                if (parser_is_form(item, SLOP_STR("@spec"))) {
                    __auto_type _mv_139 = parser_sexpr_list_get(item, 1);
                    if (_mv_139.has_value) {
                        __auto_type spec_body = _mv_139.value;
                        if (parser_sexpr_is_list(spec_body)) {
                            __auto_type _mv_140 = parser_sexpr_list_get(spec_body, 0);
                            if (_mv_140.has_value) {
                                __auto_type param_types_expr = _mv_140.value;
                                if (parser_sexpr_is_list(param_types_expr)) {
                                    {
                                        ...
                                        for (int64_t j = 0; j < num_ptypes; j++) {
                                            __auto_type _mv_141 = parser_sexpr_list_get(param_types_expr, j);
                                            if (_mv_141.has_value) {
                                                __auto_type ptype_expr = _mv_141.value;
                                                {
...
void collect_set_module_name_from_form(env_TypeEnv* env, types_SExpr* module_form) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_142 = parser_sexpr_list_get(module_form, 1);
    if (_mv_142.has_value) {
        __auto_type name_expr = _mv_142.value;
        {
        ...
        if (parser_sexpr_is_list(module_form)) {
            collect_set_module_name_from_form(env, module_form);
            {
                ...
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_143 = parser_sexpr_list_get(module_form, i);
                    if (_mv_143.has_value) {
                        __auto_type item = _mv_143.value;
    ...
    SLOP_PRE(((module_form != NULL)), "(!= module-form nil)");
    {
        ...
        if (parser_sexpr_is_list(module_form)) {
            collect_set_module_name_from_form(env, module_form);
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_144 = parser_sexpr_list_get(module_form, i);
                    if (_mv_144.has_value) {
                        __auto_type item = _mv_144.value;
                        if (parser_is_form(item, SLOP_STR("type"))) {
...
slop_option_types_ResolvedType_ptr collect_lookup_payload_type(env_TypeEnv* env, slop_string type_name) {
    if (string_eq(type_name, SLOP_STR(""))) {
        ...
        __auto_type _mv_145 = env_env_lookup_type_direct(env, type_name);
        if (_mv_145.has_value) {
            __auto_type t = _mv_145.value;
...
uint8_t collect_is_range_type_expr(types_SExpr* type_expr) {
    if (!(parser_sexpr_is_list(type_expr))) {
        ...
        if (parser_sexpr_list_len(type_expr) < 2) {
            ...
            __auto_type _mv_146 = parser_sexpr_list_get(type_expr, 0);
            if (_mv_146.has_value) {
                __auto_type first_elem = _mv_146.value;
                {
...
types_ResolvedType* collect_get_range_base_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_147 = parser_sexpr_list_get(type_expr, 0);
    if (_mv_147.has_value) {
        __auto_type first_elem = _mv_147.value;
        {
            __auto_type base_name = parser_sexpr_get_symbol_name(first_elem);
            if (string_eq(base_name, SLOP_STR("Int"))) {
                return env_env_get_int_type(env);
            } else if (string_eq(base_name, SLOP_STR("Float"))) {
                __auto_type _mv_148 = env_env_lookup_type_direct(env, SLOP_STR("Float"));
                if (_mv_148.has_value) {
                    __auto_type t = _mv_148.value;
                    return t;
    ...
    if (parser_sexpr_is_list(union_expr)) {
        {
            ...
            for (int64_t i = 1; i < len; i++) {
                __auto_type _mv_150 = parser_sexpr_list_get(union_expr, i);
                if (_mv_150.has_value) {
                    __auto_type variant_form = _mv_150.value;
...
slop_list_types_ResolvedType_ptr collect_get_variant_payload_types(env_TypeEnv* env, types_SExpr* variant_form) {
    ...
    SLOP_PRE(((variant_form != NULL)), "(!= variant-form nil)");
    {
        ...
        for (int64_t idx = 1; idx < vlen; idx++) {
            __auto_type _mv_151 = parser_sexpr_list_get(variant_form, idx);
...
slop_option_types_ResolvedType_ptr collect_get_variant_payload_type(env_TypeEnv* env, types_SExpr* variant_form) {
    if (parser_sexpr_list_len(variant_form) <= 1) {
        ...
        __auto_type _mv_152 = parser_sexpr_list_get(variant_form, 1);
        if (_mv_152.has_value) {
            __auto_type type_expr = _mv_152.value;
...
slop_string collect_checker_get_variant_name(types_SExpr* variant_form) {
    if (parser_sexpr_is_list(variant_form)) {
        if (parser_sexpr_list_len(variant_form) == 0) {
            ...
            __auto_type _mv_153 = parser_sexpr_list_get(variant_form, 0);
        ...
        __auto_type _mv_154 = (*variant_form);
        switch (_mv_154.tag) {
            case types_SExpr_sym:
            {
        ...
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_155.data.lst;
            {
                ...
                if (((int64_t)((items).len)) < 2) {
                    ...
                    __auto_type _mv_156 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_156.has_value) {
                        __auto_type head = _mv_156.value;
                        __auto_type _mv_157 = (*head);
                        switch (_mv_157.tag) {
                            case types_SExpr_sym:
                            {
                                ...
                                if (string_eq(sym.name, SLOP_STR("Option"))) {
                                    ...
                                    if (_mv_158.has_value) {
                                        __auto_type inner = _mv_158.value;
...
uint8_t collect_has_recursive_value_payload(types_SExpr* variant_form, slop_string union_name) {
    SLOP_PRE(((variant_form != NULL)), "(!= variant-form nil)");
    {
        ...
        for (int64_t idx = 1; idx < vlen; idx++) {
            if (!(found)) {
                __auto_type _mv_159 = parser_sexpr_list_get(variant_form, idx);
    ...
    switch (_mv_160.tag) {
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_160.data.lst;
            {
                ...
                __auto_type len = ((int64_t)((items).len));
                for (int64_t i = 1; i < len; i++) {
                    __auto_type _mv_161 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_161.has_value) {
                        __auto_type variant_expr = _mv_161.value;
                        __auto_type _mv_162 = (*variant_expr);
                        switch (_mv_162.tag) {
                            case types_SExpr_sym:
                            {
...
void collect_collect_constants(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type len = ((int64_t)((ast).len));
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_163 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_163.has_value) {
                __auto_type expr = _mv_163.value;
        ...
        if (parser_sexpr_is_list(module_form)) {
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_164 = parser_sexpr_list_get(module_form, i);
                    if (_mv_164.has_value) {
                        __auto_type item = _mv_164.value;
...
void collect_collect_single_constant(env_TypeEnv* env, slop_arena* arena, types_SExpr* const_form) {
    ...
    if (parser_sexpr_is_list(const_form)) {
        if (parser_sexpr_list_len(const_form) >= 3) {
            __auto_type _mv_165 = parser_sexpr_list_get(const_form, 1);
            if (_mv_165.has_value) {
                __auto_type name_expr = _mv_165.value;
                {
                    ...
                    if (!(string_eq(const_name, SLOP_STR("")))) {
                        __auto_type _mv_166 = parser_sexpr_list_get(const_form, 2);
...
types_ResolvedType* collect_get_const_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* type_expr) {
    ...
    switch (_mv_167.tag) {
        case types_SExpr_sym:
        {
            ...
            {
                __auto_type type_name = sym.name;
                __auto_type _mv_168 = env_env_lookup_type_direct(env, type_name);
                if (_mv_168.has_value) {
                    __auto_type t = _mv_168.value;
                    return t;
...
void collect_collect_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type len = ((int64_t)((ast).len));
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_169 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_169.has_value) {
                __auto_type expr = _mv_169.value;
...
void collect_collect_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    SLOP_PRE(((module_form != NULL)), "(!= module-form nil)");
    {
        __auto_type arena = env_env_arena(env);
        if (parser_sexpr_is_list(module_form)) {
            __auto_type _mv_170 = parser_sexpr_list_get(module_form, 1);
            if (_mv_170.has_value) {
                __auto_type name_expr = _mv_170.value;
                {
                    __auto_type mod_name = parser_sexpr_get_symbol_name(name_expr);
                    if (!(string_eq(mod_name, SLOP_STR("")))) {
                        env_env_set_module(env, (slop_option_string){.has_value = 1, .value = mod_name});
            ...
            }
            {
                __auto_type len = parser_sexpr_list_len(module_form);
                for (int64_t i = 2; i < len; i++) {
                    __auto_type _mv_171 = parser_sexpr_list_get(module_form, i);
                    if (_mv_171.has_value) {
                        __auto_type item = _mv_171.value;
                        if (parser_is_form(item, SLOP_STR("fn"))) {
                            ...
                            __auto_type _mv_172 = parser_sexpr_list_get(item, 2);
    ...
    if (parser_sexpr_is_list(ffi_form)) {
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_173 = parser_sexpr_list_get(ffi_form, i);
                if (_mv_173.has_value) {
                    __auto_type func_decl = _mv_173.value;
    ...
    if (parser_sexpr_is_list(func_decl)) {
        {
            ...
            if (decl_len >= 3) {
                __auto_type _mv_174 = parser_sexpr_list_get(func_decl, 0);
                ...
                __auto_type _mv_175 = parser_sexpr_list_get(func_decl, 0);
                if (_mv_175.has_value) {
                    __auto_type name_expr = _mv_175.value;
                    {
                        __auto_type const_name = parser_sexpr_get_symbol_name(name_expr);
                        if (!(string_eq(const_name, SLOP_STR("")))) {
                            __auto_type _mv_176 = parser_sexpr_list_get(func_decl, 1);
                            if (_mv_176.has_value) {
                                __auto_type type_expr = _mv_176.value;
                                {
...
uint8_t collect_ffi_has_variadic(types_SExpr* func_decl) {
    SLOP_PRE(((func_decl != NULL)), "(!= func-decl nil)");
    {
        ...
        if (len >= 4) {
            __auto_type _mv_177 = parser_sexpr_list_get(func_decl, (len - 1));
            if (_mv_177.has_value) {
                __auto_type last_expr = _mv_177.value;
...
slop_list_types_ParamInfo collect_collect_ffi_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* func_decl) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        if (_mv_178.has_value) {
            __auto_type params_expr = _mv_178.value;
            if (parser_sexpr_is_list(params_expr)) {
                {
                    __auto_type num_params = parser_sexpr_list_len(params_expr);
                    for (int64_t j = 0; j < num_params; j++) {
                        __auto_type _mv_179 = parser_sexpr_list_get(params_expr, j);
                        if (_mv_179.has_value) {
                            __auto_type param_form = _mv_179.value;
                            if (parser_sexpr_is_list(param_form)) {
                                if (parser_sexpr_list_len(param_form) >= 2) {
                                    __auto_type _mv_180 = parser_sexpr_list_get(param_form, 0);
                                    if (_mv_180.has_value) {
                                        __auto_type pname_expr = _mv_180.value;
                                        {
                                            __auto_type param_name = parser_sexpr_get_symbol_name(pname_expr);
                                            if (!(string_eq(param_name, SLOP_STR("")))) {
                                                __auto_type _mv_181 = parser_sexpr_list_get(param_form, 1);
...
types_ResolvedType* collect_get_ffi_return_type(env_TypeEnv* env, slop_arena* arena, types_SExpr* func_decl) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_182 = parser_sexpr_list_get(func_decl, 2);
    if (_mv_182.has_value) {
        __auto_type ret_expr = _mv_182.value;
...
void collect_collect_single_function(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form) {
    ...
    if (parser_sexpr_is_list(fn_form)) {
        if (parser_sexpr_list_len(fn_form) >= 3) {
            __auto_type _mv_183 = parser_sexpr_list_get(fn_form, 1);
            if (_mv_183.has_value) {
                __auto_type name_expr = _mv_183.value;
                {
                    __auto_type fn_name = parser_sexpr_get_symbol_name(name_expr);
                    if (!(string_eq(fn_name, SLOP_STR("")))) {
                        {
...
void collect_validate_main_params(env_TypeEnv* env, types_SExpr* fn_form, slop_list_types_ParamInfo params) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        if (num_params == 0) {
            ...
            __auto_type _mv_184 = ({ __auto_type _lst = params; size_t _idx = (size_t)0; slop_option_types_ParamInfo _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_184.has_value) {
                __auto_type p0 = _mv_184.value;
                {
            ...
            __auto_type _mv_185 = ({ __auto_type _lst = params; size_t _idx = (size_t)1; slop_option_types_ParamInfo _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_185.has_value) {
                __auto_type p1 = _mv_185.value;
                {
...
slop_list_types_ParamInfo collect_collect_fn_params(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form) {
    ...
    {
        __auto_type params = ((slop_list_types_ParamInfo){ .data = (types_ParamInfo*)slop_arena_alloc(arena, 16 * sizeof(types_ParamInfo)), .len = 0, .cap = 16 });
        __auto_type _mv_186 = parser_sexpr_list_get(fn_form, 2);
        if (_mv_186.has_value) {
            __auto_type params_expr = _mv_186.value;
            if (parser_sexpr_is_list(params_expr)) {
                {
                    __auto_type num_params = parser_sexpr_list_len(params_expr);
                    for (int64_t i = 0; i < num_params; i++) {
                        __auto_type _mv_187 = parser_sexpr_list_get(params_expr, i);
                        if (_mv_187.has_value) {
                            __auto_type param_form = _mv_187.value;
                            if (parser_sexpr_is_list(param_form) && (parser_sexpr_list_len(param_form) >= 2)) {
                                __auto_type _mv_188 = parser_sexpr_list_get(param_form, 0);
                                    ...
                                    __auto_type first_expr = _mv_188.value;
                                    {
                                        ...
                                        if ((string_eq(first_name, SLOP_STR("in"))) || (string_eq(first_name, SLOP_STR("out"))) || (string_eq(first_name, SLOP_STR("mut")))) {
                                            if (parser_sexpr_list_len(param_form) >= 3) {
                                                __auto_type _mv_189 = parser_sexpr_list_get(param_form, 1);
                                                if (_mv_189.has_value) {
                                                    __auto_type name_expr = _mv_189.value;
                                                    {
                                                        __auto_type param_name = parser_sexpr_get_symbol_name(name_expr);
                                                        if (!(string_eq(param_name, SLOP_STR("")))) {
                                                            __auto_type _mv_190 = parser_sexpr_list_get(param_form, 2);
...
slop_list_types_ParamInfo collect_collect_fn_params_generic(env_TypeEnv* env, slop_arena* arena, types_SExpr* fn_form, slop_list_string type_params) {
    ...
    {
        __auto_type params = ((slop_list_types_ParamInfo){ .data = (types_ParamInfo*)slop_arena_alloc(arena, 16 * sizeof(types_ParamInfo)), .len = 0, .cap = 16 });
        __auto_type _mv_192 = parser_sexpr_list_get(fn_form, 2);
        if (_mv_192.has_value) {
            __auto_type params_expr = _mv_192.value;
            if (parser_sexpr_is_list(params_expr)) {
                {
                    __auto_type num_params = parser_sexpr_list_len(params_expr);
                    for (int64_t i = 0; i < num_params; i++) {
                        __auto_type _mv_193 = parser_sexpr_list_get(params_expr, i);
                        if (_mv_193.has_value) {
                            __auto_type param_form = _mv_193.value;
                            if (parser_sexpr_is_list(param_form) && (parser_sexpr_list_len(param_form) >= 2)) {
                                __auto_type _mv_194 = parser_sexpr_list_get(param_form, 0);
                                if (_mv_194.has_value) {
                                    __auto_type first_expr = _mv_194.value;
                                    {
                                        __auto_type first_name = parser_sexpr_get_symbol_name(first_expr);
                                        if ((string_eq(first_name, SLOP_STR("in"))) || (string_eq(first_name, SLOP_STR("out"))) || (string_eq(first_name, SLOP_STR("mut")))) {
                                            if (parser_sexpr_list_len(param_form) >= 3) {
                                                __auto_type _mv_195 = parser_sexpr_list_get(param_form, 1);
                                                if (_mv_195.has_value) {
                                                    __auto_type name_expr = _mv_195.value;
                                                    {
                                                        __auto_type param_name = parser_sexpr_get_symbol_name(name_expr);
                                                        if (!(string_eq(param_name, SLOP_STR("")))) {
                                                            __auto_type _mv_196 = parser_sexpr_list_get(param_form, 2);
...
types_ResolvedType* collect_find_fn_return_type(env_TypeEnv* env, types_SExpr* fn_form) {
    ...
    SLOP_PRE(((fn_form != NULL)), "(!= fn-form nil)");
    {
        ...
        types_ResolvedType* found_type = env_env_get_unit_type(env);
        for (int64_t i = 3; i < len; i++) {
            __auto_type _mv_198 = parser_sexpr_list_get(fn_form, i);
            if (_mv_198.has_value) {
                __auto_type item = _mv_198.value;
                if (parser_is_form(item, SLOP_STR("@spec"))) {
                    if (!(found)) {
...
types_ResolvedType* collect_checker_extract_spec_return_type(env_TypeEnv* env, types_SExpr* spec_form) {
    ...
    {
        __auto_type arena = env_env_arena(env);
        __auto_type _mv_199 = parser_sexpr_list_get(spec_form, 1);
        if (_mv_199.has_value) {
            __auto_type spec_body = _mv_199.value;
            if (parser_sexpr_is_list(spec_body)) {
                {
                    __auto_type len = parser_sexpr_list_len(spec_body);
                    __auto_type _mv_200 = parser_sexpr_list_get(spec_body, (len - 1));
                    if (_mv_200.has_value) {
                        __auto_type ret_expr = _mv_200.value;
                        return collect_get_field_type(env, arena, ret_expr);
