    ...
    switch (_mv_199.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_199.data.sym;
            {
                ...
                __auto_type _mv_200 = ctype_builtin_type_c(arena, name);
                if (_mv_200.has_value) {
                    __auto_type c_type = _mv_200.value;
                    return c_type;
        ...
        {
            __auto_type _ = _mv_199.data.str;
            return SLOP_STR("void*");
        }
        case types_SExpr_num:
        {
            __auto_type _ = _mv_199.data.num;
            return SLOP_STR("void*");
        }
    }
    SLOP_UNREACHABLE();
...
slop_string ctype_to_c_type_compound(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        if (len < 1) {
            ...
            if (_mv_201.has_value) {
                __auto_type first_expr = _mv_201.value;
                __auto_type _mv_202 = (*first_expr);
                switch (_mv_202.tag) {
                    case types_SExpr_sym:
                    {
                            ...
                            if (string_eq(head, SLOP_STR("Ptr"))) {
                                if (len < 2) {
                                    ...
                                    if (_mv_203.has_value) {
                                        __auto_type inner = _mv_203.value;
                                ...
                                __auto_type _mv_210 = ctype_builtin_type_c(arena, head);
                                if (_mv_210.has_value) {
                                    __auto_type c_type = _mv_210.value;
                                    return c_type;
...
slop_string ctype_build_fn_args_str(slop_arena* arena, types_SExpr* args_expr) {
    __auto_type _mv_211 = (*args_expr);
    switch (_mv_211.tag) {
        case types_SExpr_lst:
        {
            __auto_type args_list = _mv_211.data.lst;
            {
                ...
                if (arg_count == 0) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while (i < arg_count) {
                            __auto_type _mv_212 = ({ __auto_type _lst = arg_items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_212.has_value) {
                                __auto_type arg_expr = _mv_212.value;
                                {
    ...
    switch (_mv_213.tag) {
        ...
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_213.data.lst;
            {
                ...
                __auto_type i = 0;
                while (i < len) {
                    __auto_type _mv_214 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_214.has_value) {
                        __auto_type item_expr = _mv_214.value;
                        {
    ...
    if (len < 4) {
    ...
    } else {
        {
            ...
            if (_mv_215.has_value) {
                __auto_type min_expr = _mv_215.value;
                __auto_type _mv_216 = (*min_expr);
                switch (_mv_216.tag) {
                    case types_SExpr_num:
                    {
                        __auto_type n = _mv_216.data.num;
                        has_min = 1;
            ...
            __auto_type _mv_217 = ({ __auto_type _lst = items; size_t _idx = (size_t)3; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_217.has_value) {
                __auto_type max_expr = _mv_217.value;
                __auto_type _mv_218 = (*max_expr);
                switch (_mv_218.tag) {
                    case types_SExpr_num:
                    {
...
slop_string ctype_resolved_type_to_c(slop_arena* arena, types_ResolvedType* rt) {
    SLOP_PRE(((rt != NULL)), "(!= rt nil)");
    {
        ...
        if (kind == types_ResolvedTypeKind_rk_primitive) {
            __auto_type _mv_220 = ctype_builtin_type_c(arena, name);
            if (_mv_220.has_value) {
                __auto_type c = _mv_220.value;
                return c;
        ...
        } else if ((kind == types_ResolvedTypeKind_rk_record) || ((kind == types_ResolvedTypeKind_rk_union) || (kind == types_ResolvedTypeKind_rk_enum))) {
            {
                ...
                __auto_type _mv_221 = (*rt).module_name;
                if (_mv_221.has_value) {
                    __auto_type mod_name = _mv_221.value;
            ...
            __auto_type _mv_222 = (*rt).inner_type;
