...
slop_string path_path_dirname(slop_arena* arena, slop_string path) {
    {
        ...
        if (len == 0) {
            ...
            __auto_type _mv_230 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_230.has_value) {
                __auto_type idx = _mv_230.value;
                if (idx == 0) {
                    return SLOP_STR("/");
                } else {
...
slop_string path_path_basename(slop_arena* arena, slop_string path) {
    {
        ...
        if (len == 0) {
            ...
            __auto_type _mv_231 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_231.has_value) {
                __auto_type idx = _mv_231.value;
                {
...
slop_string path_path_extension(slop_arena* arena, slop_string path) {
    {
        ...
        if (len == 0) {
            ...
            __auto_type _mv_232 = strlib_last_index_of(path, SLOP_STR("."));
            if (_mv_232.has_value) {
                __auto_type idx = _mv_232.value;
                {
