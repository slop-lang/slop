...
slop_string path_path_dirname(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_194 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_194.has_value) {
                __auto_type idx = _mv_194.value;
                if ((idx == 0)) {
                    return SLOP_STR("/");
                } else {
...
slop_string path_path_basename(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_195 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_195.has_value) {
                __auto_type idx = _mv_195.value;
                {
...
slop_string path_path_extension(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_196 = strlib_last_index_of(path, SLOP_STR("."));
            if (_mv_196.has_value) {
                __auto_type idx = _mv_196.value;
                {
