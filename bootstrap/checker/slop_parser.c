...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_63 = parser_lexer_next_token(arena, (&state));
            if (_mv_63.is_ok) {
                __auto_type tok = _mv_63.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_64 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_64.has_value) {
        __auto_type tok = _mv_64.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_65 = parser_parse_expr(arena, state);
            if (_mv_65.is_ok) {
                __auto_type quoted_expr = _mv_65.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_rparen) {
                        ...
                        __auto_type _mv_66 = parser_parse_expr(arena, state);
                        if (_mv_66.is_ok) {
                            __auto_type expr = _mv_66.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_67 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_67.is_ok) {
        __auto_type e = _mv_67.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_67.is_ok) {
        __auto_type expr = _mv_67.data.ok;
        __auto_type _mv_68 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_68.is_ok) {
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                parser_parser_advance(state);
                __auto_type _mv_69 = parser_parse_infix_primary(arena, state);
                if (!_mv_69.is_ok) {
                    __auto_type e = _mv_69.data.err;
                    return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
                } else if (_mv_69.is_ok) {
                    __auto_type operand = _mv_69.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_70 = parser_parse_infix_primary(arena, state);
            if (!_mv_70.is_ok) {
                __auto_type e = _mv_70.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_70.is_ok) {
                __auto_type operand = _mv_70.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
        ...
        } else if (tok.kind == parser_TokenType_tok_lparen) {
            {
                ...
                parser_parser_advance(state);
                __auto_type _mv_71 = parser_parse_infix_primary(arena, state);
                if (!_mv_71.is_ok) {
                    __auto_type _ = _mv_71.data.err;
                    (*state).pos = saved_pos;
                    return parser_parse_list(arena, state);
                } else if (_mv_71.is_ok) {
                    __auto_type _ = _mv_71.data.ok;
                    {
                        __auto_type next_tok = parser_parser_peek(state);
            ...
            parser_parser_advance(state);
            __auto_type _mv_72 = parser_parse_infix_primary(arena, state);
            if (!_mv_72.is_ok) {
                __auto_type e = _mv_72.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_72.is_ok) {
                __auto_type quoted_expr = _mv_72.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    __auto_type _mv_73 = parser_parse_infix_primary(arena, state);
    if (!_mv_73.is_ok) {
        __auto_type e = _mv_73.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_73.is_ok) {
        __auto_type left = _mv_73.data.ok;
        {
                    ...
                    if ((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or"))))) {
                        {
                            ...
                            if (op_prec < min_prec) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_74 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_74.is_ok) {
                                        __auto_type e = _mv_74.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_74.is_ok) {
                                        __auto_type right = _mv_74.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
    ...
    SLOP_PRE(((parser_parser_peek(state).kind == parser_TokenType_tok_lbrace)), "(== (. (parser-peek state) kind) (quote tok-lbrace))");
    {
        ...
        (*state).in_infix = 1;
        {
            ...
            if (!_mv_75.is_ok) {
                ...
                __auto_type expr = _mv_75.data.ok;
                {
                    __auto_type tok = parser_parser_peek(state);
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_76 = parser_tokenize(arena, source);
    if (_mv_76.is_ok) {
        __auto_type tokens = _mv_76.data.ok;
        {
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_eof) {
                        ...
                        __auto_type _mv_77 = parser_parse_expr(arena, (&state));
                        if (_mv_77.is_ok) {
                            __auto_type expr = _mv_77.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    __auto_type _mv_78 = (*expr);
    switch (_mv_78.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_78.data.sym;
            return s.line;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_78.data.str;
            return s.line;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_78.data.num;
            return n.line;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_78.data.lst;
            return l.line;
        }
    }
}
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    __auto_type _mv_79 = (*expr);
    switch (_mv_79.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_79.data.sym;
            return s.col;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_79.data.str;
            return s.col;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_79.data.num;
            return n.col;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_79.data.lst;
            return l.col;
        }
    }
}
...
uint8_t parser_is_form(types_SExpr* expr, slop_string keyword) {
    __auto_type _mv_81 = (*expr);
    switch (_mv_81.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_81.data.lst;
            if (((int64_t)((l.items).len)) == 0) {
                return 0;
            } else {
                __auto_type _mv_82 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_82.has_value) {
                    __auto_type first = _mv_82.value;
...
slop_list_types_SExpr_ptr parser_find_holes(slop_arena* arena, types_SExpr* expr) {
    {
        ...
        switch (_mv_92.tag) {
            case types_SExpr_lst:
            {
                __auto_type l = _mv_92.data.lst;
                {
                    __auto_type items = l.items;
                    __auto_type len = ((int64_t)((items).len));
                    __auto_type i = 0;
                    while (i < len) {
                        __auto_type _mv_93 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_93.has_value) {
                            __auto_type child = _mv_93.value;
                            {
                                __auto_type child_holes = parser_find_holes(arena, child);
                                __auto_type child_len = ((int64_t)((child_holes).len));
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_94 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_94.has_value) {
                                        __auto_type h = _mv_94.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_95.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_95.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_95.data.num;
            if (num.is_float) {
        ...
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_95.data.lst;
            {
                ...
                if (len == 0) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while (i < len) {
                            __auto_type _mv_96 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_96.has_value) {
                                __auto_type child = _mv_96.value;
                                {
...
slop_string parser_json_print_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_97 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_97.has_value) {
                        __auto_type child = _mv_97.value;
                        {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_98.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_98.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_98.data.num;
            {
