...
slop_option_int types_resolved_type_get_variant_index(types_ResolvedType* t, slop_string name) {
    ...
    SLOP_PRE((((*t).kind == types_ResolvedTypeKind_rk_union)), "(== (. (deref t) kind) (quote rk-union))");
    {
        ...
        while (((i < len) && !(done))) {
            __auto_type _mv_0 = ({ __auto_type _lst = variants; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_0.has_value) {
    ...
    if (((*t).kind == types_ResolvedTypeKind_rk_union)) {
        {
            ...
            while (((i < len) && !(done))) {
                __auto_type _mv_1 = ({ __auto_type _lst = variants; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1.has_value) {
...
uint8_t types_resolved_type_has_field(types_ResolvedType* t, slop_string name) {
    ...
    SLOP_PRE((((*t).kind == types_ResolvedTypeKind_rk_record)), "(== (. (deref t) kind) (quote rk-record))");
    {
        ...
        __auto_type found = 0;
        while (((i < len) && !(found))) {
            __auto_type _mv_2 = ({ __auto_type _lst = fields; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_2.has_value) {
...
slop_option_types_ResolvedType_ptr types_resolved_type_get_field_type(types_ResolvedType* t, slop_string name) {
    SLOP_PRE(((t != NULL)), "(!= t nil)");
    {
        ...
        while (((i < len) && !(found))) {
            __auto_type _mv_3 = ({ __auto_type _lst = fields; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_3.has_value) {
