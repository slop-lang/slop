    ...
    if (parser_sexpr_is_list(expr)) {
        {
            ...
            for (int64_t i = 1; i < len; i++) {
                ...
                if (_mv_307.has_value) {
                    __auto_type arg_expr = _mv_307.value;
                    {
    ...
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        while (i < len) {
            ...
            if (_mv_314.has_value) {
                ...
                __auto_type _mv_315 = (*clause);
                switch (_mv_315.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type clause_lst = _mv_315.data.lst;
                        {
                            ...
                            if (clause_len > 1) {
                                for (int64_t ci = 0; ci < (clause_len - 1); ci++) {
                                    ...
                                    if (_mv_316.has_value) {
                                        __auto_type clause_elem = _mv_316.value;
                                        {
                                ...
                                __auto_type _mv_317 = ({ __auto_type _lst = clause_items; size_t _idx = (size_t)(clause_len - 1); slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_317.has_value) {
                                    __auto_type body = _mv_317.value;
                                    {
                                                ...
                                                if ((scrutinee_kind == types_ResolvedTypeKind_rk_option) && string_eq(variant_name, SLOP_STR("some"))) {
                                                    ...
                                                    if (scrutinee_kind == types_ResolvedTypeKind_rk_result) {
                                                        if (string_eq(variant_name, SLOP_STR("ok"))) {
                                                            ...
                                                            __auto_type _mv_322 = (*scrutinee_type).inner_type2;
                                                            if (_mv_322.has_value) {
                                                                __auto_type inner2 = _mv_322.value;
                                                            ...
                                                            if (((int64_t)((payload_types).len)) > 0) {
                                                                ...
                                                                }
                                                                {
                                                                    ...
                                                                    for (int64_t pi = 2; pi < pat_len; pi++) {
                                                                        __auto_type _mv_324 = parser_sexpr_list_get(pattern, pi);
