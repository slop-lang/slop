...
slop_result_uint8_t_file_FileError file_file_close(file_File* f) {
    ...
    slop_result_uint8_t_file_FileError _retval;
    (*f).is_open = 0;
    {
        __auto_type result = fclose((*f).handle);
        if ((result == 0)) {
            return ((slop_result_uint8_t_file_FileError){ .is_ok = true, .data.ok = 1 });
        } else {
            return ((slop_result_uint8_t_file_FileError){ .is_ok = false, .data.err = file_FileError_io_error });
        }
    }
    ...
    SLOP_PRE(((*f).is_open), "(. (deref f) is-open)");
    {
        ...
        fseek((*f).handle, 0, SEEK_END);
        {
            ...
            fseek((*f).handle, start_pos, SEEK_SET);
            {
                ...
                {
                    __auto_type bytes_read = fread(buf, 1, ((uint64_t)(size)), (*f).handle);
...
slop_result_int_file_FileError file_file_write(file_File* f, slop_bytes data) {
    SLOP_PRE(((*f).is_open), "(. (deref f) is-open)");
    {
        __auto_type written = fwrite(((void*)(data.data)), 1, data.len, (*f).handle);
...
slop_result_int_file_FileError file_file_write_line(file_File* f, slop_string line) {
    SLOP_PRE(((*f).is_open), "(. (deref f) is-open)");
    {
        __auto_type written1 = fwrite(((void*)(line.data)), 1, line.len, (*f).handle);
        {
...
slop_result_int_file_FileError file_file_size(slop_string path) {
    {
        ...
        slop_arena* arena = &_arena;
        {
            ...
            {
                __auto_type result = stat(((uint8_t*)(path.data)), buf);
                if ((result != 0)) {
