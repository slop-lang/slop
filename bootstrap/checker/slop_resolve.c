...
void resolve_resolve_imports(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_330 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_330.has_value) {
                __auto_type expr = _mv_330.value;
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_331 = parser_sexpr_list_get(module_form, i);
                if (_mv_331.has_value) {
                    __auto_type item = _mv_331.value;
...
void resolve_resolve_import_stmt(env_TypeEnv* env, types_SExpr* import_form) {
    ...
    SLOP_PRE((parser_is_form(import_form, SLOP_STR("import"))), "(is-form import-form \"import\")");
    {
        ...
        __auto_type _mv_332 = (*import_form);
        switch (_mv_332.tag) {
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_332.data.lst;
                {
                    ...
                    if (_mv_333.has_value) {
                        __auto_type mod_name_expr = _mv_333.value;
                        __auto_type _mv_334 = (*mod_name_expr);
                        switch (_mv_334.tag) {
                            case types_SExpr_sym:
                            {
                                __auto_type mod_sym = _mv_334.data.sym;
                                __auto_type _mv_335 = ({ __auto_type _lst = items; size_t _idx = (size_t)2; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_335.has_value) {
                                    __auto_type names_expr = _mv_335.value;
                                    __auto_type _mv_336 = (*names_expr);
                                    switch (_mv_336.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type names_lst = _mv_336.data.lst;
                                            {
...
slop_option_string resolve_resolve_module_file(slop_arena* arena, slop_string module_name, slop_option_string from_file) {
    if (!(resolve_contains_slash(module_name))) {
        ...
        __auto_type _mv_337 = from_file;
        if (_mv_337.has_value) {
            __auto_type current_path = _mv_337.value;
            {
