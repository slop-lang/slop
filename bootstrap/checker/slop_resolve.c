...
void resolve_resolve_imports(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_257 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_257.has_value) {
                __auto_type expr = _mv_257.value;
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_258 = parser_sexpr_list_get(module_form, i);
                if (_mv_258.has_value) {
                    __auto_type item = _mv_258.value;
...
void resolve_resolve_import_stmt(env_TypeEnv* env, types_SExpr* import_form) {
    ...
    SLOP_PRE((parser_is_form(import_form, SLOP_STR("import"))), "(is-form import-form \"import\")");
    {
        ...
        __auto_type _mv_259 = (*import_form);
        switch (_mv_259.tag) {
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_259.data.lst;
                {
                    ...
                    if (_mv_260.has_value) {
                        __auto_type mod_name_expr = _mv_260.value;
                        __auto_type _mv_261 = (*mod_name_expr);
                        switch (_mv_261.tag) {
                            case types_SExpr_sym:
                            {
                                __auto_type mod_sym = _mv_261.data.sym;
                                ...
                                if (_mv_262.has_value) {
                                    __auto_type names_expr = _mv_262.value;
                                    __auto_type _mv_263 = (*names_expr);
                                    switch (_mv_263.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type names_lst = _mv_263.data.lst;
                                            {
...
slop_option_string resolve_resolve_module_file(slop_arena* arena, slop_string module_name, slop_option_string from_file) {
    if (!(resolve_contains_slash(module_name))) {
        ...
        __auto_type _mv_264 = from_file;
        if (_mv_264.has_value) {
            __auto_type current_path = _mv_264.value;
            {
