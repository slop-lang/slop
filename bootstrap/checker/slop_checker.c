...
void checker_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_378 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_378.has_value) {
                __auto_type expr = _mv_378.value;
...
uint8_t checker_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_379 = parser_sexpr_list_get(item, 0);
...
slop_string checker_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_380 = parser_sexpr_list_get(item, 0);
...
void checker_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_381 = parser_sexpr_list_get(module_form, 1);
        if (_mv_381.has_value) {
            __auto_type name_expr = _mv_381.value;
            {
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_382 = parser_sexpr_list_get(module_form, i);
...
slop_string checker_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if ((((int64_t)((exprs).len)) < 1)) {
        ...
        __auto_type _mv_383 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_383.has_value) {
            __auto_type first_expr = _mv_383.value;
            __auto_type _mv_384 = (*first_expr);
            switch (_mv_384.tag) {
                                    ...
                                    case types_SExpr_sym:
                                    {
                                        ...
                                        if (string_eq(sym.name, SLOP_STR("module"))) {
                                            ...
                                            if (_mv_387.has_value) {
                                                ...
                                                __auto_type _mv_388 = (*name_expr);
                                                switch (_mv_388.tag) {
                                                    case types_SExpr_sym:
                                                    {
                                                        __auto_type name_sym = _mv_388.data.sym;
...
void checker_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_389 = diag.level;
    if (_mv_389 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_389 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void checker_output_diagnostics_text(slop_arena* arena, slop_string filename, slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while ((i < len)) {
            __auto_type _mv_390 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_390.has_value) {
                __auto_type diag = _mv_390.value;
...
void checker_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while ((i < len)) {
            ...
            __auto_type _mv_391 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_391.has_value) {
                __auto_type diag = _mv_391.value;
...
void checker_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_392 = diag.level;
    if (_mv_392 == types_DiagnosticLevel_diag_warning) {
        checker_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_392 == types_DiagnosticLevel_diag_error) {
...
int64_t checker_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, checker_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_393 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_393.is_ok) {
            __auto_type e = _mv_393.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_393.is_ok) {
            __auto_type f = _mv_393.data.ok;
            __auto_type _mv_394 = file_file_read_all(arena, (&f));
            if (!_mv_394.is_ok) {
                __auto_type e = _mv_394.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_394.is_ok) {
                __auto_type source = _mv_394.data.ok;
                file_file_close((&f));
                __auto_type _mv_395 = parser_parse(arena, source);
                if (_mv_395.is_ok) {
                    __auto_type ast = _mv_395.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_395.data.err;
...
int64_t checker_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        while ((i < len)) {
            ...
            if (_mv_396.has_value) {
                ...
                __auto_type _mv_397 = diag.level;
                if (_mv_397 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_397 == types_DiagnosticLevel_diag_warning) {
                }
...
types_ResolvedType* checker_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_398 = parser_parse(arena, type_str);
    if (_mv_398.is_ok) {
        __auto_type type_ast = _mv_398.data.ok;
        if ((((int64_t)((type_ast).len)) == 0)) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_399 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        ...
        __auto_type _ = _mv_398.data.err;
        return env_env_get_int_type(env);
    }
}
...
void checker_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_400 = parser_parse(arena, params_str);
    if (_mv_400.is_ok) {
        __auto_type params_ast = _mv_400.data.ok;
        if ((((int64_t)((params_ast).len)) > 0)) {
            __auto_type _mv_401 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_401.has_value) {
                __auto_type params_list = _mv_401.value;
                __auto_type _mv_402 = (*params_list);
                switch (_mv_402.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_402.data.lst;
                        {
                            ...
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_403 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_403.has_value) {
                                    __auto_type param = _mv_403.value;
                                    __auto_type _mv_404 = (*param);
                                    switch (_mv_404.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_404.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if ((((int64_t)((param_items).len)) >= 2)) {
                                                    __auto_type _mv_405 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                        ...
                                                        switch (_mv_406.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_406.data.sym;
                                                                {
                                                                    ...
                                                                    __auto_type _mv_407 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_407.has_value) {
                                                                        __auto_type type_expr = _mv_407.value;
                                                                        __auto_type _mv_408 = (*type_expr);
                                                                        switch (_mv_408.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_408.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_408.data.lst;
                                                                                {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            if (_mv_409.has_value) {
                __auto_type a_inner = _mv_409.value;
                __auto_type _mv_410 = (*b).inner_type;
                if (_mv_410.has_value) {
                    __auto_type b_inner = _mv_410.value;
            ...
            if (_mv_411.has_value) {
                __auto_type a_inner = _mv_411.value;
                __auto_type _mv_412 = (*b).inner_type;
                if (_mv_412.has_value) {
                    __auto_type b_inner = _mv_412.value;
                    return checker_types_names_equal(a_inner, b_inner);
            ...
            __auto_type _mv_413 = (*a).inner_type;
            ...
            __auto_type _mv_414 = (*b).inner_type;
            if (_mv_414.has_value) {
                __auto_type base = _mv_414.value;
            ...
            if (_mv_415.has_value) {
                __auto_type a_base = _mv_415.value;
            ...
            if (_mv_416.has_value) {
                __auto_type b_base = _mv_416.value;
...
int64_t checker_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if ((string_len(context_file) > 0)) {
        __auto_type _mv_417 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_417.is_ok) {
            ...
            if (!_mv_418.is_ok) {
                ...
                if (_mv_419.is_ok) {
                    __auto_type context_ast = _mv_419.data.ok;
