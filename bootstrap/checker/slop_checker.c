...
void checker_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_402 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_402.has_value) {
                __auto_type expr = _mv_402.value;
...
slop_string checker_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_404 = parser_sexpr_list_get(item, 0);
...
void checker_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_405 = parser_sexpr_list_get(module_form, 1);
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_406 = parser_sexpr_list_get(module_form, i);
...
slop_string checker_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_407 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_407.has_value) {
            __auto_type first_expr = _mv_407.value;
            __auto_type _mv_408 = (*first_expr);
            switch (_mv_408.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_408.data.lst;
                    {
                        ...
                        if (((int64_t)((items).len)) < 2) {
                            ...
                            __auto_type _mv_409 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void checker_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_413 = diag.level;
    if (_mv_413 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_413 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void checker_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_415 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_415.has_value) {
                __auto_type diag = _mv_415.value;
...
void checker_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_416 = diag.level;
    if (_mv_416 == types_DiagnosticLevel_diag_warning) {
        checker_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_416 == types_DiagnosticLevel_diag_error) {
...
int64_t checker_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, checker_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_417 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_417.is_ok) {
            ...
            __auto_type f = _mv_417.data.ok;
            __auto_type _mv_418 = file_file_read_all(arena, (&f));
            if (!_mv_418.is_ok) {
                __auto_type e = _mv_418.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_418.is_ok) {
                __auto_type source = _mv_418.data.ok;
                file_file_close((&f));
                __auto_type _mv_419 = parser_parse(arena, source);
                if (_mv_419.is_ok) {
                    __auto_type ast = _mv_419.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_419.data.err;
...
int64_t checker_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_420 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_420.has_value) {
                __auto_type diag = _mv_420.value;
                __auto_type _mv_421 = diag.level;
                if (_mv_421 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_421 == types_DiagnosticLevel_diag_warning) {
                }
...
types_ResolvedType* checker_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_422 = parser_parse(arena, type_str);
    if (_mv_422.is_ok) {
        __auto_type type_ast = _mv_422.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_423 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void checker_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_424 = parser_parse(arena, params_str);
    if (_mv_424.is_ok) {
        __auto_type params_ast = _mv_424.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_425 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_425.has_value) {
                __auto_type params_list = _mv_425.value;
                __auto_type _mv_426 = (*params_list);
                switch (_mv_426.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_426.data.lst;
                        {
                            ...
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_427 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_427.has_value) {
                                    __auto_type param = _mv_427.value;
                                    __auto_type _mv_428 = (*param);
                                    switch (_mv_428.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_428.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_429 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                        ...
                                                        switch (_mv_430.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_430.data.sym;
                                                                {
                                                                    ...
                                                                    __auto_type _mv_431 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_431.has_value) {
                                                                        __auto_type type_expr = _mv_431.value;
                                                                        __auto_type _mv_432 = (*type_expr);
                                                                        switch (_mv_432.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_432.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_432.data.lst;
                                                                                {
...
uint8_t checker_types_names_equal(types_ResolvedType* a, types_ResolvedType* b) {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            __auto_type _mv_433 = (*a).inner_type;
            if (_mv_433.has_value) {
                __auto_type a_inner = _mv_433.value;
                __auto_type _mv_434 = (*b).inner_type;
                if (_mv_434.has_value) {
                    __auto_type b_inner = _mv_434.value;
            ...
            __auto_type _mv_435 = (*a).inner_type;
            if (_mv_435.has_value) {
                __auto_type a_inner = _mv_435.value;
                __auto_type _mv_436 = (*b).inner_type;
                if (_mv_436.has_value) {
                    __auto_type b_inner = _mv_436.value;
                    return checker_types_names_equal(a_inner, b_inner);
            ...
            __auto_type _mv_437 = (*a).inner_type;
            ...
            __auto_type _mv_438 = (*b).inner_type;
            if (_mv_438.has_value) {
                __auto_type base = _mv_438.value;
            ...
            __auto_type _mv_439 = (*a).inner_type;
            if (_mv_439.has_value) {
                __auto_type a_base = _mv_439.value;
            ...
            __auto_type _mv_440 = (*b).inner_type;
            if (_mv_440.has_value) {
                __auto_type b_base = _mv_440.value;
...
int64_t checker_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_441 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_441.is_ok) {
            __auto_type _ = _mv_441.data.err;
        } else if (_mv_441.is_ok) {
            __auto_type f = _mv_441.data.ok;
            __auto_type _mv_442 = file_file_read_all(arena, (&f));
            if (!_mv_442.is_ok) {
                __auto_type _ = _mv_442.data.err;
                file_file_close((&f));
            } else if (_mv_442.is_ok) {
                __auto_type source = _mv_442.data.ok;
                file_file_close((&f));
                __auto_type _mv_443 = parser_parse(arena, source);
                if (_mv_443.is_ok) {
                    __auto_type context_ast = _mv_443.data.ok;
                    collect_collect_module(env, context_ast);
                    resolve_resolve_imports(env, context_ast);
                    env_env_clear_diagnostics(env);
                } else if (!_mv_443.is_ok) {
                    __auto_type _ = _mv_443.data.err;
                }
            }
        }
    }
