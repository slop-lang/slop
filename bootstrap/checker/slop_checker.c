...
void checker_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_229 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_229.has_value) {
                __auto_type expr = _mv_229.value;
...
void checker_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_230 = parser_sexpr_list_get(module_form, 1);
        if (_mv_230.has_value) {
            __auto_type name_expr = _mv_230.value;
            {
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_231 = parser_sexpr_list_get(module_form, i);
                if (_mv_231.has_value) {
                    __auto_type item = _mv_231.value;
...
slop_string checker_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if ((((int64_t)((exprs).len)) < 1)) {
        ...
        __auto_type _mv_232 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_232.has_value) {
            __auto_type first_expr = _mv_232.value;
            __auto_type _mv_233 = (*first_expr);
            switch (_mv_233.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_233.data.lst;
                    {
                        __auto_type items = lst.items;
                        if ((((int64_t)((items).len)) < 2)) {
                            return SLOP_STR("unknown");
                        } else {
                            __auto_type _mv_234 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_234.has_value) {
                                __auto_type head = _mv_234.value;
                                __auto_type _mv_235 = (*head);
                                switch (_mv_235.tag) {
                                    case types_SExpr_sym:
                                    {
                                        __auto_type sym = _mv_235.data.sym;
                                        if (string_eq(sym.name, SLOP_STR("module"))) {
                                            __auto_type _mv_236 = ({ __auto_type _lst = items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                            if (_mv_236.has_value) {
                                                __auto_type name_expr = _mv_236.value;
                                                __auto_type _mv_237 = (*name_expr);
                                                switch (_mv_237.tag) {
                                                    case types_SExpr_sym:
                                                    {
                                                        __auto_type name_sym = _mv_237.data.sym;
...
void checker_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_238 = diag.level;
    if (_mv_238 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_238 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void checker_output_diagnostics_text(slop_arena* arena, slop_string filename, slop_list_types_Diagnostic diagnostics) {
    {
        ...
        __auto_type i = 0;
        while ((i < len)) {
            __auto_type _mv_239 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_239.has_value) {
                __auto_type diag = _mv_239.value;
...
void checker_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while ((i < len)) {
            ...
            __auto_type _mv_240 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_240.has_value) {
                __auto_type diag = _mv_240.value;
...
void checker_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_241 = diag.level;
    if (_mv_241 == types_DiagnosticLevel_diag_warning) {
        checker_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_241 == types_DiagnosticLevel_diag_error) {
...
int64_t checker_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, checker_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_242 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_242.is_ok) {
            ...
            __auto_type f = _mv_242.data.ok;
            __auto_type _mv_243 = file_file_read_all(arena, (&f));
            if (!_mv_243.is_ok) {
                ...
                __auto_type source = _mv_243.data.ok;
                file_file_close((&f));
                __auto_type _mv_244 = parser_parse(arena, source);
                if (_mv_244.is_ok) {
                    __auto_type ast = _mv_244.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_244.data.err;
...
int64_t checker_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        while ((i < len)) {
            ...
            if (_mv_245.has_value) {
                ...
                __auto_type _mv_246 = diag.level;
                if (_mv_246 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_246 == types_DiagnosticLevel_diag_warning) {
                }
...
types_ResolvedType* checker_resolve_type_name(env_TypeEnv* env, slop_string type_name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_247 = env_env_lookup_type(env, type_name);
...
void checker_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_248 = parser_parse(arena, params_str);
    if (_mv_248.is_ok) {
        __auto_type params_ast = _mv_248.data.ok;
        if ((((int64_t)((params_ast).len)) > 0)) {
            __auto_type _mv_249 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_249.has_value) {
                __auto_type params_list = _mv_249.value;
                __auto_type _mv_250 = (*params_list);
                switch (_mv_250.tag) {
                    case types_SExpr_lst:
                    {
...
int64_t checker_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if ((string_len(context_file) > 0)) {
        __auto_type _mv_251 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_251.is_ok) {
            __auto_type _ = _mv_251.data.err;
        } else if (_mv_251.is_ok) {
            __auto_type f = _mv_251.data.ok;
            __auto_type _mv_252 = file_file_read_all(arena, (&f));
            if (!_mv_252.is_ok) {
                __auto_type _ = _mv_252.data.err;
                file_file_close((&f));
            } else if (_mv_252.is_ok) {
                __auto_type source = _mv_252.data.ok;
                file_file_close((&f));
                __auto_type _mv_253 = parser_parse(arena, source);
                if (_mv_253.is_ok) {
                    __auto_type context_ast = _mv_253.data.ok;
                    collect_collect_module(env, context_ast);
                    resolve_resolve_imports(env, context_ast);
                } else if (!_mv_253.is_ok) {
                    __auto_type _ = _mv_253.data.err;
                }
            }
        }
    }
