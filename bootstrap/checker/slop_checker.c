...
void checker_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_436 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
uint8_t checker_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_437 = parser_sexpr_list_get(item, 0);
...
slop_string checker_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_438 = parser_sexpr_list_get(item, 0);
...
void checker_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_439 = parser_sexpr_list_get(module_form, 1);
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_440 = parser_sexpr_list_get(module_form, i);
                if (_mv_440.has_value) {
                    __auto_type item = _mv_440.value;
                    if (parser_is_form(item, SLOP_STR("fn"))) {
                        {
...
slop_string checker_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_441 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_441.has_value) {
            __auto_type first_expr = _mv_441.value;
            __auto_type _mv_442 = (*first_expr);
            switch (_mv_442.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_442.data.lst;
                    {
                        ...
                        if (((int64_t)((items).len)) < 2) {
                            ...
                            __auto_type _mv_443 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void checker_output_diagnostics_text(slop_arena* arena, slop_string filename, slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_448 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_448.has_value) {
                __auto_type diag = _mv_448.value;
...
void checker_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_449 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_449.has_value) {
                __auto_type diag = _mv_449.value;
...
void checker_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_450 = diag.level;
    if (_mv_450 == types_DiagnosticLevel_diag_warning) {
        checker_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_450 == types_DiagnosticLevel_diag_error) {
...
int64_t checker_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, checker_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_451 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_451.is_ok) {
            __auto_type e = _mv_451.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_451.is_ok) {
            __auto_type f = _mv_451.data.ok;
            __auto_type _mv_452 = file_file_read_all(arena, (&f));
            if (!_mv_452.is_ok) {
                __auto_type e = _mv_452.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_452.is_ok) {
                __auto_type source = _mv_452.data.ok;
                file_file_close((&f));
                __auto_type _mv_453 = parser_parse(arena, source);
                if (_mv_453.is_ok) {
                    __auto_type ast = _mv_453.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_453.data.err;
...
int64_t checker_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_454 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_454.has_value) {
                __auto_type diag = _mv_454.value;
                __auto_type _mv_455 = diag.level;
                if (_mv_455 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_455 == types_DiagnosticLevel_diag_warning) {
                }
...
types_ResolvedType* checker_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_456 = parser_parse(arena, type_str);
    if (_mv_456.is_ok) {
        __auto_type type_ast = _mv_456.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_457 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void checker_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_458 = parser_parse(arena, params_str);
    if (_mv_458.is_ok) {
        __auto_type params_ast = _mv_458.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_459 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_459.has_value) {
                __auto_type params_list = _mv_459.value;
                __auto_type _mv_460 = (*params_list);
                switch (_mv_460.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_460.data.lst;
                        {
                            ...
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_461 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_461.has_value) {
                                    __auto_type param = _mv_461.value;
                                    __auto_type _mv_462 = (*param);
                                    switch (_mv_462.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_462.data.lst;
                                            {
                                                ...
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_463 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                        ...
                                                        switch (_mv_464.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_464.data.sym;
                                                                {
                                                                    ...
                                                                    __auto_type _mv_465 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_465.has_value) {
                                                                        __auto_type type_expr = _mv_465.value;
                                                                        __auto_type _mv_466 = (*type_expr);
                                                                        switch (_mv_466.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_466.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_466.data.lst;
                                                                                {
...
uint8_t checker_types_names_equal(types_ResolvedType* a, types_ResolvedType* b) {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            __auto_type _mv_467 = (*a).inner_type;
            if (_mv_467.has_value) {
                __auto_type a_inner = _mv_467.value;
                __auto_type _mv_468 = (*b).inner_type;
                if (_mv_468.has_value) {
                    __auto_type b_inner = _mv_468.value;
            ...
            __auto_type _mv_469 = (*a).inner_type;
            if (_mv_469.has_value) {
                __auto_type a_inner = _mv_469.value;
                __auto_type _mv_470 = (*b).inner_type;
                if (_mv_470.has_value) {
                    __auto_type b_inner = _mv_470.value;
                    return checker_types_names_equal(a_inner, b_inner);
            ...
            __auto_type _mv_471 = (*a).inner_type;
            if (_mv_471.has_value) {
                __auto_type base = _mv_471.value;
            ...
            __auto_type _mv_472 = (*b).inner_type;
            if (_mv_472.has_value) {
                __auto_type base = _mv_472.value;
            ...
            __auto_type _mv_473 = (*a).inner_type;
            if (_mv_473.has_value) {
                __auto_type a_base = _mv_473.value;
            ...
            __auto_type _mv_474 = (*b).inner_type;
            if (_mv_474.has_value) {
                __auto_type b_base = _mv_474.value;
...
int64_t checker_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_475 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_475.is_ok) {
            __auto_type _ = _mv_475.data.err;
        } else if (_mv_475.is_ok) {
            __auto_type f = _mv_475.data.ok;
            __auto_type _mv_476 = file_file_read_all(arena, (&f));
            if (!_mv_476.is_ok) {
                __auto_type _ = _mv_476.data.err;
                file_file_close((&f));
            } else if (_mv_476.is_ok) {
                __auto_type source = _mv_476.data.ok;
                file_file_close((&f));
                __auto_type _mv_477 = parser_parse(arena, source);
                if (_mv_477.is_ok) {
                    __auto_type context_ast = _mv_477.data.ok;
