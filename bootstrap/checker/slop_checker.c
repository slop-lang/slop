...
void checker_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_439 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_439.has_value) {
                __auto_type expr = _mv_439.value;
...
uint8_t checker_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_440 = parser_sexpr_list_get(item, 0);
...
slop_string checker_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_441 = parser_sexpr_list_get(item, 0);
...
void checker_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_442 = parser_sexpr_list_get(module_form, 1);
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_443 = parser_sexpr_list_get(module_form, i);
                if (_mv_443.has_value) {
                    __auto_type item = _mv_443.value;
                    if (parser_is_form(item, SLOP_STR("fn"))) {
                        {
...
slop_string checker_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_444 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_444.has_value) {
            __auto_type first_expr = _mv_444.value;
            __auto_type _mv_445 = (*first_expr);
            switch (_mv_445.tag) {
...
void checker_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_452 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_452.has_value) {
                __auto_type diag = _mv_452.value;
...
void checker_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_453 = diag.level;
    if (_mv_453 == types_DiagnosticLevel_diag_warning) {
        checker_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_453 == types_DiagnosticLevel_diag_error) {
...
int64_t checker_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, checker_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_454 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_454.is_ok) {
            __auto_type e = _mv_454.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_454.is_ok) {
            __auto_type f = _mv_454.data.ok;
            __auto_type _mv_455 = file_file_read_all(arena, (&f));
            if (!_mv_455.is_ok) {
                __auto_type e = _mv_455.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_455.is_ok) {
                __auto_type source = _mv_455.data.ok;
                file_file_close((&f));
                __auto_type _mv_456 = parser_parse(arena, source);
                if (_mv_456.is_ok) {
                    __auto_type ast = _mv_456.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_456.data.err;
...
int64_t checker_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_457.has_value) {
                ...
                if (_mv_458 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_458 == types_DiagnosticLevel_diag_warning) {
                }
...
types_ResolvedType* checker_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_459 = parser_parse(arena, type_str);
    if (_mv_459.is_ok) {
        __auto_type type_ast = _mv_459.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_460 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void checker_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_461 = parser_parse(arena, params_str);
    if (_mv_461.is_ok) {
        __auto_type params_ast = _mv_461.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_462 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_462.has_value) {
                __auto_type params_list = _mv_462.value;
                __auto_type _mv_463 = (*params_list);
                switch (_mv_463.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_463.data.lst;
                        {
                            ...
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_464 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_464.has_value) {
                                    __auto_type param = _mv_464.value;
                                    __auto_type _mv_465 = (*param);
                                    switch (_mv_465.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_465.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_466 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                        ...
                                                        switch (_mv_467.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_467.data.sym;
                                                                {
                                                                    ...
                                                                    __auto_type _mv_468 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_468.has_value) {
                                                                        __auto_type type_expr = _mv_468.value;
                                                                        __auto_type _mv_469 = (*type_expr);
                                                                        switch (_mv_469.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_469.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_469.data.lst;
                                                                                {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            if (_mv_470.has_value) {
                __auto_type a_inner = _mv_470.value;
                __auto_type _mv_471 = (*b).inner_type;
                if (_mv_471.has_value) {
                    __auto_type b_inner = _mv_471.value;
            ...
            __auto_type _mv_472 = (*a).inner_type;
            if (_mv_472.has_value) {
                __auto_type a_inner = _mv_472.value;
                __auto_type _mv_473 = (*b).inner_type;
                if (_mv_473.has_value) {
                    __auto_type b_inner = _mv_473.value;
                    return checker_types_names_equal(a_inner, b_inner);
            ...
            if (_mv_474.has_value) {
                __auto_type base = _mv_474.value;
            ...
            if (_mv_475.has_value) {
                __auto_type base = _mv_475.value;
            ...
            if (_mv_476.has_value) {
                __auto_type a_base = _mv_476.value;
            ...
            if (_mv_477.has_value) {
                __auto_type b_base = _mv_477.value;
...
int64_t checker_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_478 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_478.is_ok) {
            __auto_type _ = _mv_478.data.err;
        } else if (_mv_478.is_ok) {
            __auto_type f = _mv_478.data.ok;
            __auto_type _mv_479 = file_file_read_all(arena, (&f));
            if (!_mv_479.is_ok) {
                __auto_type _ = _mv_479.data.err;
                file_file_close((&f));
            } else if (_mv_479.is_ok) {
                __auto_type source = _mv_479.data.ok;
                file_file_close((&f));
                __auto_type _mv_480 = parser_parse(arena, source);
                if (_mv_480.is_ok) {
                    __auto_type context_ast = _mv_480.data.ok;
