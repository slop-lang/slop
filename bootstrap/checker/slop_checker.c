...
uint8_t checker_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_392 = parser_sexpr_list_get(item, 0);
        if (_mv_392.has_value) {
            __auto_type head = _mv_392.value;
...
slop_string checker_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_393 = parser_sexpr_list_get(item, 0);
...
void checker_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_394 = parser_sexpr_list_get(module_form, 1);
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_395 = parser_sexpr_list_get(module_form, i);
                                        ...
                                        if (string_eq(sym.name, SLOP_STR("module"))) {
                                            ...
                                            if (_mv_400.has_value) {
                                                ...
                                                switch (_mv_401.tag) {
                                                    case types_SExpr_sym:
                                                    {
                                                        __auto_type name_sym = _mv_401.data.sym;
...
void checker_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_402 = diag.level;
    if (_mv_402 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_402 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void checker_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_405 = diag.level;
    if (_mv_405 == types_DiagnosticLevel_diag_warning) {
        checker_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_405 == types_DiagnosticLevel_diag_error) {
...
int64_t checker_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, checker_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_406 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_406.is_ok) {
            __auto_type e = _mv_406.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_406.is_ok) {
            __auto_type f = _mv_406.data.ok;
            __auto_type _mv_407 = file_file_read_all(arena, (&f));
            if (!_mv_407.is_ok) {
                __auto_type e = _mv_407.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_407.is_ok) {
                __auto_type source = _mv_407.data.ok;
                file_file_close((&f));
                __auto_type _mv_408 = parser_parse(arena, source);
                if (_mv_408.is_ok) {
                    __auto_type ast = _mv_408.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_408.data.err;
...
int64_t checker_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        while ((i < len)) {
            ...
            if (_mv_409.has_value) {
                ...
                if (_mv_410 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_410 == types_DiagnosticLevel_diag_warning) {
                }
...
types_ResolvedType* checker_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_411 = parser_parse(arena, type_str);
    if (_mv_411.is_ok) {
        __auto_type type_ast = _mv_411.data.ok;
...
void checker_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_413 = parser_parse(arena, params_str);
    if (_mv_413.is_ok) {
                            ...
                            for (int64_t i = 0; i < len; i++) {
                                ...
                                if (_mv_416.has_value) {
                                    ...
                                    switch (_mv_417.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_417.data.lst;
                                            {
                                                                ...
                                                                __auto_type name_sym = _mv_419.data.sym;
                                                                {
                                                                    ...
                                                                    if (_mv_420.has_value) {
                                                                        ...
                                                                        switch (_mv_421.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_421.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_421.data.lst;
                                                                                {
...
uint8_t checker_types_names_equal(types_ResolvedType* a, types_ResolvedType* b) {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            __auto_type _mv_422 = (*a).inner_type;
            if (_mv_422.has_value) {
                __auto_type a_inner = _mv_422.value;
                __auto_type _mv_423 = (*b).inner_type;
                if (_mv_423.has_value) {
                    __auto_type b_inner = _mv_423.value;
            ...
            __auto_type _mv_424 = (*a).inner_type;
            if (_mv_424.has_value) {
                __auto_type a_inner = _mv_424.value;
                __auto_type _mv_425 = (*b).inner_type;
                if (_mv_425.has_value) {
                    __auto_type b_inner = _mv_425.value;
                    return checker_types_names_equal(a_inner, b_inner);
            ...
            if (_mv_428.has_value) {
                __auto_type a_base = _mv_428.value;
            ...
            if (_mv_429.has_value) {
                __auto_type b_base = _mv_429.value;
...
int64_t checker_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if ((string_len(context_file) > 0)) {
        __auto_type _mv_430 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_430.is_ok) {
            __auto_type _ = _mv_430.data.err;
        } else if (_mv_430.is_ok) {
            __auto_type f = _mv_430.data.ok;
            __auto_type _mv_431 = file_file_read_all(arena, (&f));
            if (!_mv_431.is_ok) {
                __auto_type _ = _mv_431.data.err;
                file_file_close((&f));
            } else if (_mv_431.is_ok) {
                __auto_type source = _mv_431.data.ok;
                file_file_close((&f));
                __auto_type _mv_432 = parser_parse(arena, source);
                if (_mv_432.is_ok) {
                    __auto_type context_ast = _mv_432.data.ok;
