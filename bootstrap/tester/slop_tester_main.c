...
slop_string tester_main_lines_to_string(slop_arena* arena, slop_list_string lines) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_1676 = ({ __auto_type _lst = lines; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1676.has_value) {
                        __auto_type line = _mv_1676.value;
                ...
                }
                {
                    ...
                    int64_t j = 0;
                    while (j < len) {
                        __auto_type _mv_1677 = ({ __auto_type _lst = lines; size_t _idx = (size_t)j; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1677.has_value) {
                            __auto_type line = _mv_1677.value;
                            {
        ...
        while (i < argc) {
            {
                ...
                __auto_type _mv_1678 = tester_main_read_file(arena, path_ptr);
                if (_mv_1678.has_value) {
                    __auto_type source = _mv_1678.value;
...
int main(int argc, char** _c_argv) {
    ...
    if (argc >= 2) {
        if (string_eq(strlib_cstring_to_string(argv[1]), SLOP_STR("--version"))) {
            tester_main_print_str(((char*)(SLOP_STR("slop-tester 0.2.2\n").data)));
            return 0;
        }
    }
    if (argc < 2) {
    ...
    } else {
        {
            ...
            __auto_type _mv_1679 = tester_main_read_file(arena, ((char*)(argv[1])));
            if (_mv_1679.has_value) {
                __auto_type source = _mv_1679.value;
                {
