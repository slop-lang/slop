...
types_SExpr* test_emit_replace_wildcards(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_1545.tag) {
        ...
        case types_SExpr_lst:
        {
            __auto_type l = _mv_1545.data.lst;
            {
                __auto_type items = l.items;
                __auto_type len = ((int64_t)((items).len));
...
slop_string test_emit_prefix_symbol(slop_arena* arena, slop_string name, slop_string prefix, type_extract_TypeRegistry types) {
    if (string_eq(name, SLOP_STR("arena"))) {
        ...
        __auto_type _mv_1548 = type_extract_registry_lookup_import(types, name);
        if (_mv_1548.has_value) {
            __auto_type mod_name = _mv_1548.value;
    ...
    SLOP_PRE(((types != NULL)), "(!= types nil)");
    {
        ...
        test_emit_emit(ctx, SLOP_STR("#include <math.h>"));
        {
            ...
            while (j < import_count) {
                __auto_type _mv_1550 = ({ __auto_type _lst = imports; size_t _idx = (size_t)j; slop_option_type_extract_ImportEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        ...
        test_emit_emit(ctx, SLOP_STR(""));
        {
            ...
            }
            {
                ...
                int64_t i = 0;
                while (i < test_count) {
                    __auto_type _mv_1551 = ({ __auto_type _lst = tests; size_t _idx = (size_t)i; slop_option_extract_TestCase_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1551.has_value) {
                        __auto_type tc = _mv_1551.value;
...
uint8_t test_emit_test_mentions_arena(extract_TestCase tc) {
    ...
    if (tc.needs_arena) {
    ...
    } else {
        {
            ...
            while ((i < len) && !(found)) {
                __auto_type _mv_1554 = ({ __auto_type _lst = args; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1554.has_value) {
                    __auto_type arg = _mv_1554.value;
...
uint8_t test_emit_any_test_needs_arena(slop_list_extract_TestCase_ptr tests) {
    {
        ...
        uint8_t found = 0;
        while ((i < len) && !(found)) {
            __auto_type _mv_1555 = ({ __auto_type _lst = tests; size_t _idx = (size_t)i; slop_option_extract_TestCase_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1555.has_value) {
                __auto_type tc = _mv_1555.value;
...
slop_string test_emit_param_name_at(types_SExpr* params, int64_t idx) {
    ...
    __auto_type _mv_1556 = parser_sexpr_list_get(params, idx);
    if (_mv_1556.has_value) {
        __auto_type param = _mv_1556.value;
        {
            __auto_type plen = parser_sexpr_list_len(param);
            if (plen < 2) {
                return SLOP_STR("");
            } else {
                __auto_type _mv_1557 = parser_sexpr_list_get(param, (((plen == 2)) ? 0 : 1));
                if (_mv_1557.has_value) {
                    __auto_type name_expr = _mv_1557.value;
                ...
                if (string_len(pname) > 0) {
                    if (needs_arena && (i == arena_pos)) {
                    ...
                    } else {
                        {
                            ...
                            __auto_type _mv_1560 = ({ __auto_type _lst = args; size_t _idx = (size_t)arg_idx; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_string test_emit_build_precondition_check(slop_arena* arena, context_TranspileContext* tctx, extract_TestCase tc) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1561 = ({ __auto_type _lst = pres; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1561.has_value) {
                __auto_type cond_expr = _mv_1561.value;
                {
...
slop_option_string test_emit_first_error_since(slop_arena* arena, context_TranspileContext* tctx, int64_t before, slop_string file_name, int64_t fallback_line, int64_t fallback_col) {
    ...
    SLOP_PRE(((before >= 0)), "(>= before 0)");
    {
        ...
        if (((int64_t)((errors).len)) > before) {
            __auto_type _mv_1563 = ({ __auto_type _lst = errors; size_t _idx = (size_t)((int64_t)(before)); slop_option_context_TranspileError _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1563.has_value) {
                __auto_type e = _mv_1563.value;
                {
...
types_SExpr* test_emit_example_anchor(extract_TestCase tc) {
    ...
    __auto_type _mv_1564 = ({ __auto_type _lst = tc.args; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_1564.has_value) {
        __auto_type a = _mv_1564.value;
        return a;
...
void test_emit_emit_test_function_typed(test_emit_EmitContext* ctx, extract_TestCase tc, int64_t index, slop_string prefix, type_extract_TypeRegistry* types, context_TranspileContext* tctx, slop_string file_name) {
    ...
    SLOP_PRE(((types != NULL)), "(!= types nil)");
    {
        ...
        __auto_type _mv_1565 = problem;
        if (_mv_1565.has_value) {
            __auto_type reason = _mv_1565.value;
...
slop_string test_emit_make_c_fn_name(slop_arena* arena, slop_string fn_name, slop_option_string module_name, slop_string prefix) {
    {
        ...
        __auto_type _mv_1566 = module_name;
        if (_mv_1566.has_value) {
            __auto_type mod = _mv_1566.value;
...
slop_string test_emit_get_record_decl_type(slop_arena* arena, type_extract_TypeRegistry types, slop_string record_name) {
    __auto_type _mv_1571 = type_extract_registry_lookup(types, record_name);
                ...
                } else {
                    {
                        ...
                        while (i < len) {
                            ...
                            if (_mv_1575.has_value) {
                                __auto_type val = _mv_1575.value;
...
int64_t test_emit_count_wildcard_values(slop_list_types_SExpr_ptr field_values) {
    int64_t _retval = {0};
    {
        ...
        int64_t n = 0;
        while (i < len) {
            __auto_type _mv_1577 = ({ __auto_type _lst = field_values; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_string test_emit_build_record_field_comparisons_with_values(slop_arena* arena, slop_list_type_extract_TstFieldEntry fields, slop_list_types_SExpr_ptr field_values, slop_string result_accessor, slop_string expected_var) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1578 = ({ __auto_type _lst = fields; size_t _idx = (size_t)i; slop_option_type_extract_TstFieldEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1578.has_value) {
                __auto_type field = _mv_1578.value;
                {
    ...
    switch (_mv_1579.tag) {
        ...
        case types_SExpr_lst:
        {
            ...
            if (((int64_t)((l.items).len)) > 0) {
                __auto_type _mv_1580 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1580.has_value) {
                    __auto_type first = _mv_1580.value;
                    __auto_type _mv_1581 = (*first);
                    switch (_mv_1581.tag) {
                        case types_SExpr_sym:
                        {
                        ...
                        case types_SExpr_lst:
                        {
                            __auto_type _ = _mv_1581.data.lst;
                            return SLOP_STR("");
                        }
                        case types_SExpr_num:
                        {
...
slop_string test_emit_build_union_payload_comparison(slop_arena* arena, slop_string payload_type, slop_string c_variant, slop_string expected_var, type_extract_TypeRegistry types, slop_list_types_SExpr_ptr field_values) {
    {
        ...
        if (string_eq(payload_type, SLOP_STR(""))) {
            ...
            __auto_type _mv_1582 = type_extract_registry_get_record_fields(types, payload_type);
            if (_mv_1582.has_value) {
                __auto_type fields = _mv_1582.value;
...
slop_string test_emit_build_union_record_payload_comparison(slop_arena* arena, slop_list_type_extract_TstFieldEntry fields, slop_list_types_SExpr_ptr field_values, slop_string c_variant, slop_string expected_var) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1583 = ({ __auto_type _lst = fields; size_t _idx = (size_t)i; slop_option_type_extract_TstFieldEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1583.has_value) {
                __auto_type field = _mv_1583.value;
                {
                    __auto_type is_wildcard = ({ __auto_type _mv = ({ __auto_type _lst = field_values; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; }); _mv.has_value ? ({ __auto_type val = _mv.value; test_emit_is_wildcard_expr(val); }) : (0); });
                    if (!(is_wildcard)) {
                        {
                        ...
                        if (parser_sexpr_is_symbol(first)) {
                            {
                                ...
                                if (string_eq(name, SLOP_STR("union-new"))) {
                                    ...
                                    if ((string_eq(name, SLOP_STR("none"))) || (string_eq(name, SLOP_STR("some"))) || (string_eq(name, SLOP_STR("ok"))) || (string_eq(name, SLOP_STR("error")))) {
                                        ...
                                        __auto_type _mv_1586 = type_extract_registry_lookup_variant(types, name);
                                        if (_mv_1586.has_value) {
                                            __auto_type _ = _mv_1586.value;
                                            return 1;
                ...
                if (strlib_starts_with(name, SLOP_STR("'"))) {
                    {
                        ...
                        __auto_type _mv_1591 = type_extract_registry_lookup_enum_value(types, variant_name);
                    ...
                    if (_mv_1592.has_value) {
                        ...
                        if (parser_sexpr_is_symbol(first) && string_eq(parser_sexpr_get_symbol_name(first), SLOP_STR("quote"))) {
                            ...
                            if (_mv_1593.has_value) {
                                __auto_type quoted = _mv_1593.value;
                                if (parser_sexpr_is_symbol(quoted)) {
                                    __auto_type _mv_1594 = type_extract_registry_lookup_enum_value(types, parser_sexpr_get_symbol_name(quoted));
                                    if (_mv_1594.has_value) {
                                        __auto_type _ = _mv_1594.value;
                                        return 1;
                        ...
                        if (parser_sexpr_is_symbol(first)) {
                            {
                                ...
                                if (string_eq(name, SLOP_STR("none")) || (string_eq(name, SLOP_STR("some")) || (string_eq(name, SLOP_STR("ok")) || (string_eq(name, SLOP_STR("error")) || string_eq(name, SLOP_STR("list")))))) {
                                    ...
                                    __auto_type _mv_1597 = type_extract_registry_lookup(types, name);
...
slop_string test_emit_get_some_inner_typed(slop_arena* arena, types_SExpr* expr, context_TranspileContext* tctx) {
    if (parser_sexpr_list_len(expr) > 1) {
        __auto_type _mv_1598 = parser_sexpr_list_get(expr, 1);
...
slop_string test_emit_build_eq_function_name(slop_arena* arena, slop_string type_name, slop_string prefix, type_extract_TypeRegistry types) {
    {
        ...
        __auto_type _mv_1604 = type_extract_registry_lookup_import(types, type_name);
        if (_mv_1604.has_value) {
            __auto_type mod_name = _mv_1604.value;
            {
                __auto_type parts = ((slop_list_string){ .data = (slop_string*)slop_arena_alloc(arena, 16 * sizeof(slop_string)), .len = 0, .cap = 16 });
                ...
                __auto_type total_count = ((int64_t)((elements).len));
                {
                    ...
                    if (elem_count == 0) {
                    ...
                    } else {
                        {
                            ...
                            while (j < total_count) {
                                __auto_type _mv_1606 = ({ __auto_type _lst = elements; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1606.has_value) {
                                    __auto_type elem = _mv_1606.value;
                                ...
                                if (has_wildcard) {
                                ...
                                } else {
                                    {
                                        ...
                                        __auto_type is_complex = ((string_eq(elem_type, SLOP_STR("String"))) || (type_extract_registry_is_record(types, elem_type)) || (type_extract_registry_is_union(types, elem_type)));
                                        {
                                            ...
                                            __auto_type first = 1;
                                            while (i < total_count) {
                                                __auto_type _mv_1607 = ({ __auto_type _lst = elements; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
context_TranspileContext* test_emit_init_transpile_context(slop_arena* arena, slop_list_types_SExpr_ptr ast, slop_string module_name) {
    {
        ...
        if (((int64_t)((ast).len)) > 0) {
            __auto_type _mv_1608 = ({ __auto_type _lst = ast; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void test_emit_init_transpile_context_for_imports(slop_arena* arena, context_TranspileContext* tctx, slop_list_types_SExpr_ptr import_ast, slop_string import_mod_name) {
    {
        ...
        if (((int64_t)((import_ast).len)) > 0) {
            __auto_type _mv_1611 = ({ __auto_type _lst = import_ast; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void test_emit_re_prescan_main_module(slop_arena* arena, context_TranspileContext* tctx, slop_list_types_SExpr_ptr ast) {
    if (((int64_t)((ast).len)) > 0) {
        __auto_type _mv_1614 = ({ __auto_type _lst = ast; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            ...
            if (parser_is_form(first_expr, SLOP_STR("module"))) {
                {
                    ...
                    __auto_type i = 2;
                    if (i < mod_len) {
                        __auto_type _mv_1615 = parser_sexpr_list_get(first_expr, i);
                        if (_mv_1615.has_value) {
                            __auto_type maybe_export = _mv_1615.value;
                            if (parser_is_form(maybe_export, SLOP_STR("export"))) {
                                i = (i + 1);
                            }
                    ...
                    while (i < mod_len) {
                        ...
                        if (_mv_1616.has_value) {
                            __auto_type form = _mv_1616.value;
                            if (!(parser_is_form(form, SLOP_STR("@doc")))) {
                                ({ __auto_type _lst_p = &(body_forms); __auto_type _item = (form); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; });
                            }
...
void test_emit_register_all_field_types(context_TranspileContext* tctx, slop_list_types_SExpr_ptr body_forms) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1617 = ({ __auto_type _lst = body_forms; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        ...
                        if (parser_is_form(def_expr, SLOP_STR("record"))) {
                            {
                                ...
                                __auto_type j = 1;
                                while (j < field_count) {
                                    __auto_type _mv_1620 = parser_sexpr_list_get(def_expr, j);
                                    if (_mv_1620.has_value) {
                                        __auto_type field_def = _mv_1620.value;
                                        if (parser_sexpr_list_len(field_def) >= 2) {
                                            __auto_type _mv_1621 = parser_sexpr_list_get(field_def, 0);
                                            if (_mv_1621.has_value) {
                                                __auto_type field_name_expr = _mv_1621.value;
                                                if (parser_sexpr_is_symbol(field_name_expr)) {
                                                    {
                                                        __auto_type field_name = parser_sexpr_get_symbol_name(field_name_expr);
                                                        __auto_type _mv_1622 = parser_sexpr_list_get(field_def, 1);
                                                        if (_mv_1622.has_value) {
                                                            __auto_type field_type_expr = _mv_1622.value;
                                                            {
