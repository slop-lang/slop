...
slop_string tester_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if ((((int64_t)((exprs).len)) < 1)) {
        ...
        __auto_type _mv_1530 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_1530.has_value) {
            __auto_type first_expr = _mv_1530.value;
            if (parser_is_form(first_expr, SLOP_STR("module"))) {
                if ((parser_sexpr_list_len(first_expr) >= 2)) {
                    __auto_type _mv_1531 = parser_sexpr_list_get(first_expr, 1);
                    if (_mv_1531.has_value) {
                        __auto_type name_expr = _mv_1531.value;
...
slop_list_type_extract_ImportEntry tester_extract_imports(slop_arena* arena, slop_list_types_SExpr_ptr exprs) {
    {
        ...
        if ((((int64_t)((exprs).len)) > 0)) {
            __auto_type _mv_1532 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1532.has_value) {
                __auto_type module_expr = _mv_1532.value;
                if (parser_is_form(module_expr, SLOP_STR("module"))) {
                    {
                        __auto_type mod_len = parser_sexpr_list_len(module_expr);
                        __auto_type i = 2;
                        while ((i < mod_len)) {
                            __auto_type _mv_1533 = parser_sexpr_list_get(module_expr, i);
                            if (_mv_1533.has_value) {
                                ...
                                if (parser_is_form(child, SLOP_STR("import"))) {
                                    if ((parser_sexpr_list_len(child) >= 3)) {
                                        __auto_type _mv_1534 = parser_sexpr_list_get(child, 1);
                                            ...
                                            if (parser_sexpr_is_symbol(name_expr)) {
                                                {
                                                    ...
                                                    __auto_type _mv_1535 = parser_sexpr_list_get(child, 2);
                                                    if (_mv_1535.has_value) {
                                                        __auto_type sym_list = _mv_1535.value;
                                                        {
                                                            __auto_type sym_len = parser_sexpr_list_len(sym_list);
                                                            __auto_type j = 0;
                                                            while ((j < sym_len)) {
                                                                __auto_type _mv_1536 = parser_sexpr_list_get(sym_list, j);
                                                                if (_mv_1536.has_value) {
                                                                    __auto_type sym_expr = _mv_1536.value;
...
tester_TestResult tester_generate_tests(slop_arena* arena, slop_string source) {
    __auto_type _mv_1537 = parser_parse(arena, source);
    if (_mv_1537.is_ok) {
        __auto_type exprs = _mv_1537.data.ok;
        {
        ...
        __auto_type err = _mv_1537.data.err;
        {
...
tester_TestResult tester_generate_tests_with_imports(slop_arena* arena, slop_string source, slop_list_string import_sources) {
    __auto_type _mv_1538 = parser_parse(arena, source);
    if (_mv_1538.is_ok) {
        __auto_type exprs = _mv_1538.data.ok;
        {
            __auto_type mod_name = tester_extract_module_name(exprs);
            __auto_type imports = tester_extract_imports(arena, exprs);
            {
                ...
                } else {
                    {
                        ...
                        __auto_type tctx = test_emit_init_transpile_context(arena, exprs, mod_name);
                        {
                            ...
                            while ((i < import_count)) {
                                __auto_type _mv_1539 = ({ __auto_type _lst = import_sources; size_t _idx = (size_t)i; slop_option_string _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1539.has_value) {
                                    __auto_type import_src = _mv_1539.value;
                                    __auto_type _mv_1540 = parser_parse(arena, import_src);
                                    if (_mv_1540.is_ok) {
                                        __auto_type import_exprs = _mv_1540.data.ok;
                                        {
        ...
        __auto_type err = _mv_1538.data.err;
        {
            __auto_type error_msg = string_concat(arena, SLOP_STR("Parse error at line "), string_concat(arena, int_to_string(arena, err.line), string_concat(arena, SLOP_STR(", col "), string_concat(arena, int_to_string(arena, err.col), string_concat(arena, SLOP_STR(": "), err.message)))));
            return (tester_TestResult){0, ((slop_list_string){ .data = (slop_string*)slop_arena_alloc(arena, 16 * sizeof(slop_string)), .len = 0, .cap = 16 }), 0, SLOP_STR(""), error_msg};
        }
...
void tester_extract_types_from_module_ast(slop_arena* arena, slop_list_types_SExpr_ptr ast, type_extract_TypeRegistry* types, slop_string prefix) {
    {
        ...
        int64_t i = 0;
        while ((i < len)) {
            __auto_type _mv_1541 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1541.has_value) {
                __auto_type expr = _mv_1541.value;
...
void tester_extract_types_from_module_form(slop_arena* arena, types_SExpr* module_form, type_extract_TypeRegistry* types, slop_string prefix) {
    {
        ...
        while ((i < len)) {
            ...
            if (_mv_1542.has_value) {
                __auto_type form = _mv_1542.value;
...
void tester_extract_single_type_def(slop_arena* arena, types_SExpr* type_form, type_extract_TypeRegistry* types, slop_string prefix) {
    if ((parser_sexpr_list_len(type_form) >= 3)) {
        __auto_type _mv_1543 = parser_sexpr_list_get(type_form, 1);
            ...
            if (parser_sexpr_is_symbol(name_expr)) {
                {
                    ...
                    __auto_type _mv_1544 = parser_sexpr_list_get(type_form, 2);
...
type_extract_TstTypeEntry* tester_extract_record_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t i = 1;
        while ((i < len)) {
            __auto_type _mv_1545 = parser_sexpr_list_get(def, i);
            if (_mv_1545.has_value) {
                __auto_type field_def = _mv_1545.value;
                if ((parser_sexpr_list_len(field_def) >= 2)) {
                    __auto_type _mv_1546 = parser_sexpr_list_get(field_def, 0);
                    if (_mv_1546.has_value) {
                        __auto_type field_name_expr = _mv_1546.value;
                        if (parser_sexpr_is_symbol(field_name_expr)) {
                            {
                                __auto_type field_name = parser_sexpr_get_symbol_name(field_name_expr);
                                __auto_type _mv_1547 = parser_sexpr_list_get(field_def, 1);
                                if (_mv_1547.has_value) {
                                    __auto_type field_type_expr = _mv_1547.value;
                                    {
...
type_extract_TstTypeEntry* tester_extract_union_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while ((i < len)) {
            __auto_type _mv_1548 = parser_sexpr_list_get(def, i);
            if (_mv_1548.has_value) {
                __auto_type variant_def = _mv_1548.value;
                {
                    __auto_type vlen = parser_sexpr_list_len(variant_def);
                    if ((vlen >= 1)) {
                        __auto_type _mv_1549 = parser_sexpr_list_get(variant_def, 0);
                        if (_mv_1549.has_value) {
                            __auto_type variant_name_expr = _mv_1549.value;
...
type_extract_TstTypeEntry* tester_extract_enum_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while ((i < len)) {
            __auto_type _mv_1550 = parser_sexpr_list_get(def, i);
...
slop_string tester_sexpr_to_string_simple(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_1551.tag) {
        ...
        {
            __auto_type num = _mv_1551.data.num;
            return num.raw;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1551.data.str;
            return str.value;
        }
            ...
            __auto_type l = _mv_1551.data.lst;
            {
                ...
                while ((i < len)) {
                    ...
                    if (_mv_1552.has_value) {
                        __auto_type child = _mv_1552.value;
                        {
