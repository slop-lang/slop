        ...
        if (_mv_1623.has_value) {
            ...
            if (parser_is_form(first_expr, SLOP_STR("module"))) {
                if (parser_sexpr_list_len(first_expr) >= 2) {
                    __auto_type _mv_1624 = parser_sexpr_list_get(first_expr, 1);
                                            ...
                                            if (parser_sexpr_is_symbol(name_expr)) {
                                                {
                                                    ...
                                                    __auto_type _mv_1628 = parser_sexpr_list_get(child, 2);
                                                    if (_mv_1628.has_value) {
                                                        __auto_type sym_list = _mv_1628.value;
                                                        {
                                                            __auto_type sym_len = parser_sexpr_list_len(sym_list);
                                                            __auto_type j = 0;
                                                            while (j < sym_len) {
                                                                __auto_type _mv_1629 = parser_sexpr_list_get(sym_list, j);
                                                                if (_mv_1629.has_value) {
                                                                    __auto_type sym_expr = _mv_1629.value;
...
tester_TestResult tester_generate_tests(slop_arena* arena, slop_string source, slop_string file_name) {
    __auto_type _mv_1630 = parser_parse(arena, source);
    if (_mv_1630.is_ok) {
        __auto_type exprs = _mv_1630.data.ok;
        {
        ...
        __auto_type err = _mv_1630.data.err;
        {
                ...
                } else {
                    {
                        ...
                        __auto_type tctx = test_emit_init_transpile_context(arena, exprs, mod_name);
                        {
                            ...
                            while (i < import_count) {
                                __auto_type _mv_1632 = ({ __auto_type _lst = import_sources; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1632.has_value) {
                                    __auto_type import_src = _mv_1632.value;
                                    __auto_type _mv_1633 = parser_parse(arena, import_src);
                                    if (_mv_1633.is_ok) {
                                        __auto_type import_exprs = _mv_1633.data.ok;
                                        {
...
void tester_extract_types_from_module_ast(slop_arena* arena, slop_list_types_SExpr_ptr ast, type_extract_TypeRegistry* types, slop_string prefix) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1634 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1634.has_value) {
                __auto_type expr = _mv_1634.value;
...
void tester_extract_types_from_module_form(slop_arena* arena, types_SExpr* module_form, type_extract_TypeRegistry* types, slop_string prefix) {
    {
        ...
        int64_t i = 2;
        while (i < len) {
            __auto_type _mv_1635 = parser_sexpr_list_get(module_form, i);
...
type_extract_TstTypeEntry* tester_extract_union_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_1641.has_value) {
                __auto_type variant_def = _mv_1641.value;
                {
                    __auto_type vlen = parser_sexpr_list_len(variant_def);
                    if (vlen >= 1) {
                        __auto_type _mv_1642 = parser_sexpr_list_get(variant_def, 0);
                        if (_mv_1642.has_value) {
                            __auto_type variant_name_expr = _mv_1642.value;
...
slop_string tester_sexpr_to_string_simple(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_1644.tag) {
        ...
        {
            __auto_type num = _mv_1644.data.num;
            return num.raw;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1644.data.str;
            return str.value;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_1644.data.lst;
            {
                ...
                while (i < len) {
                    ...
                    if (_mv_1645.has_value) {
                        __auto_type child = _mv_1645.value;
                        {
