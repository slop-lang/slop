...
slop_string tester_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_1625 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_1625.has_value) {
            ...
            if (parser_is_form(first_expr, SLOP_STR("module"))) {
                if (parser_sexpr_list_len(first_expr) >= 2) {
                    __auto_type _mv_1626 = parser_sexpr_list_get(first_expr, 1);
...
slop_list_type_extract_ImportEntry tester_extract_imports(slop_arena* arena, slop_list_types_SExpr_ptr exprs) {
    {
        ...
        if (((int64_t)((exprs).len)) > 0) {
            __auto_type _mv_1627 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1627.has_value) {
                __auto_type module_expr = _mv_1627.value;
                if (parser_is_form(module_expr, SLOP_STR("module"))) {
                    {
                        __auto_type mod_len = parser_sexpr_list_len(module_expr);
                        __auto_type i = 2;
                        while (i < mod_len) {
                            __auto_type _mv_1628 = parser_sexpr_list_get(module_expr, i);
                            if (_mv_1628.has_value) {
                                __auto_type child = _mv_1628.value;
                                if (parser_is_form(child, SLOP_STR("import"))) {
                                    if (parser_sexpr_list_len(child) >= 3) {
                                        __auto_type _mv_1629 = parser_sexpr_list_get(child, 1);
                                            ...
                                            if (parser_sexpr_is_symbol(name_expr)) {
                                                {
                                                    ...
                                                    __auto_type _mv_1630 = parser_sexpr_list_get(child, 2);
                                                    if (_mv_1630.has_value) {
                                                        __auto_type sym_list = _mv_1630.value;
                                                        {
                                                            __auto_type sym_len = parser_sexpr_list_len(sym_list);
                                                            __auto_type j = 0;
                                                            while (j < sym_len) {
                                                                __auto_type _mv_1631 = parser_sexpr_list_get(sym_list, j);
                                                                if (_mv_1631.has_value) {
                                                                    __auto_type sym_expr = _mv_1631.value;
...
tester_TestResult tester_generate_tests_with_imports(slop_arena* arena, slop_string source, slop_list_string import_sources, slop_string file_name) {
    ...
    if (_mv_1633.is_ok) {
        __auto_type exprs = _mv_1633.data.ok;
        {
            __auto_type mod_name = tester_extract_module_name(exprs);
            __auto_type imports = tester_extract_imports(arena, exprs);
            {
                ...
                } else {
                    {
                        ...
                        __auto_type tctx = test_emit_init_transpile_context(arena, exprs, mod_name);
                        {
                            ...
                            while (i < import_count) {
                                __auto_type _mv_1634 = ({ __auto_type _lst = import_sources; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1634.has_value) {
                                    __auto_type import_src = _mv_1634.value;
                                    __auto_type _mv_1635 = parser_parse(arena, import_src);
                                    if (_mv_1635.is_ok) {
                                        __auto_type import_exprs = _mv_1635.data.ok;
                                        {
                                        ...
                                        __auto_type _ = _mv_1635.data.err;
                                    }
        ...
        __auto_type err = _mv_1633.data.err;
        {
            __auto_type error_msg = string_concat(arena, SLOP_STR("Parse error at line "), string_concat(arena, int_to_string(arena, err.line), string_concat(arena, SLOP_STR(", col "), string_concat(arena, int_to_string(arena, err.col), string_concat(arena, SLOP_STR(": "), err.message)))));
            return (tester_TestResult){0, ((slop_list_string){ .data = (slop_string*)slop_arena_alloc(arena, 16 * sizeof(slop_string)), .len = 0, .cap = 16 }), 0, SLOP_STR(""), error_msg};
        }
...
void tester_extract_types_from_module_ast(slop_arena* arena, slop_list_types_SExpr_ptr ast, type_extract_TypeRegistry* types, slop_string prefix) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1636 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1636.has_value) {
                __auto_type expr = _mv_1636.value;
...
void tester_extract_types_from_module_form(slop_arena* arena, types_SExpr* module_form, type_extract_TypeRegistry* types, slop_string prefix) {
    {
        ...
        int64_t i = 2;
        while (i < len) {
            __auto_type _mv_1637 = parser_sexpr_list_get(module_form, i);
...
void tester_extract_single_type_def(slop_arena* arena, types_SExpr* type_form, type_extract_TypeRegistry* types, slop_string prefix) {
    if (parser_sexpr_list_len(type_form) >= 3) {
        __auto_type _mv_1638 = parser_sexpr_list_get(type_form, 1);
            ...
            if (parser_sexpr_is_symbol(name_expr)) {
                {
                    ...
                    __auto_type _mv_1639 = parser_sexpr_list_get(type_form, 2);
                    if (_mv_1639.has_value) {
                        __auto_type def_expr = _mv_1639.value;
...
type_extract_TstTypeEntry* tester_extract_record_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_1640.has_value) {
                __auto_type field_def = _mv_1640.value;
                if (parser_sexpr_list_len(field_def) >= 2) {
                    __auto_type _mv_1641 = parser_sexpr_list_get(field_def, 0);
                    if (_mv_1641.has_value) {
                        __auto_type field_name_expr = _mv_1641.value;
                        if (parser_sexpr_is_symbol(field_name_expr)) {
                            {
                                __auto_type field_name = parser_sexpr_get_symbol_name(field_name_expr);
                                __auto_type _mv_1642 = parser_sexpr_list_get(field_def, 1);
                                if (_mv_1642.has_value) {
                                    __auto_type field_type_expr = _mv_1642.value;
                                    {
...
type_extract_TstTypeEntry* tester_extract_union_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while (i < len) {
            __auto_type _mv_1643 = parser_sexpr_list_get(def, i);
            if (_mv_1643.has_value) {
                __auto_type variant_def = _mv_1643.value;
                {
                    __auto_type vlen = parser_sexpr_list_len(variant_def);
                    if (vlen >= 1) {
                        __auto_type _mv_1644 = parser_sexpr_list_get(variant_def, 0);
                        if (_mv_1644.has_value) {
                            __auto_type variant_name_expr = _mv_1644.value;
...
type_extract_TstTypeEntry* tester_extract_enum_type_entry(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while (i < len) {
            __auto_type _mv_1645 = parser_sexpr_list_get(def, i);
...
slop_string tester_sexpr_to_string_simple(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_1646.tag) {
        ...
        {
            __auto_type num = _mv_1646.data.num;
            return num.raw;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1646.data.str;
            return str.value;
        }
            ...
            __auto_type l = _mv_1646.data.lst;
            {
                ...
                while (i < len) {
                    ...
                    if (_mv_1647.has_value) {
                        __auto_type child = _mv_1647.value;
                        {
