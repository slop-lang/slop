...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while ((!(done) && !(has_error))) {
            __auto_type _mv_578 = parser_lexer_next_token(arena, (&state));
            if (_mv_578.is_ok) {
                __auto_type tok = _mv_578.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_579 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_579.has_value) {
        __auto_type tok = _mv_579.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if ((tok.kind == parser_TokenType_tok_lparen)) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_580 = parser_parse_expr(arena, state);
            if (_mv_580.is_ok) {
                __auto_type quoted_expr = _mv_580.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_rparen)) {
                        ...
                        __auto_type _mv_581 = parser_parse_expr(arena, state);
                        if (_mv_581.is_ok) {
                            __auto_type expr = _mv_581.data.ok;
        ...
        if ((tok.kind == parser_TokenType_tok_number)) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                ...
                if (!_mv_582.is_ok) {
                    ...
                    __auto_type operand = _mv_582.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            if (!_mv_583.is_ok) {
                ...
                __auto_type operand = _mv_583.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_584 = parser_parse_infix_primary(arena, state);
            if (!_mv_584.is_ok) {
                __auto_type e = _mv_584.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_584.is_ok) {
                __auto_type quoted_expr = _mv_584.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    __auto_type _mv_585 = parser_parse_infix_primary(arena, state);
    if (!_mv_585.is_ok) {
        __auto_type e = _mv_585.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_585.is_ok) {
        __auto_type left = _mv_585.data.ok;
        {
                    ...
                    if (((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or")))))) {
                        {
                            ...
                            if ((op_prec < min_prec)) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_586 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_586.is_ok) {
                                        __auto_type e = _mv_586.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_586.is_ok) {
                                        __auto_type right = _mv_586.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_588 = parser_tokenize(arena, source);
    if (_mv_588.is_ok) {
        __auto_type tokens = _mv_588.data.ok;
        {
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_eof)) {
                        ...
                        __auto_type _mv_589 = parser_parse_expr(arena, (&state));
                        if (_mv_589.is_ok) {
                            __auto_type expr = _mv_589.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_590.tag) {
        ...
        {
            __auto_type s = _mv_590.data.sym;
            return s.line;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_590.data.str;
            return s.line;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_590.data.num;
            return n.line;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_590.data.lst;
            return l.line;
        }
    }
}
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_591.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_591.data.sym;
            return s.col;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_591.data.str;
            return s.col;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_591.data.num;
            return n.col;
        }
        case types_SExpr_lst:
        {
    ...
    switch (_mv_593.tag) {
        case types_SExpr_lst:
        {
            ...
            if ((((int64_t)((l.items).len)) == 0)) {
                ...
                __auto_type _mv_594 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_594.has_value) {
                    __auto_type first = _mv_594.value;
...
slop_option_types_SExpr_ptr parser_sexpr_list_get(types_SExpr* expr, int64_t index) {
    ...
    switch (_mv_596.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_596.data.lst;
...
slop_string parser_sexpr_number_string(types_SExpr* expr) {
    ...
    switch (_mv_602.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_602.data.num;
            return n.raw;
        }
        default: {
            return SLOP_STR("");
...
slop_string parser_sexpr_string_value(types_SExpr* expr) {
    ...
    switch (_mv_603.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_603.data.str;
            return s.value;
        }
        default: {
            return SLOP_STR("");
                ...
                __auto_type l = _mv_604.data.lst;
                {
                    ...
                    while ((i < len)) {
                        ...
                        if (_mv_605.has_value) {
                            __auto_type child = _mv_605.value;
                            {
                                __auto_type child_holes = parser_find_holes(arena, child);
                                __auto_type child_len = ((int64_t)((child_holes).len));
                                __auto_type j = 0;
                                while ((j < child_len)) {
                                    __auto_type _mv_606 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_606.has_value) {
                                        __auto_type h = _mv_606.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_607.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_607.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_607.data.num;
            if (num.is_float) {
            ...
            __auto_type lst = _mv_607.data.lst;
            {
                ...
                if ((len == 0)) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while ((i < len)) {
                            __auto_type _mv_608 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_608.has_value) {
                                __auto_type child = _mv_608.value;
                                {
        ...
        } else {
            {
                ...
                while ((i < len)) {
                    ...
                    if (_mv_609.has_value) {
                        __auto_type child = _mv_609.value;
                        {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_610.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_610.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_610.data.num;
            {
