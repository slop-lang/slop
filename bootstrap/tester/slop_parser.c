...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_614 = parser_lexer_next_token(arena, (&state));
            if (_mv_614.is_ok) {
                __auto_type tok = _mv_614.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_615 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_615.has_value) {
        __auto_type tok = _mv_615.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_616 = parser_parse_expr(arena, state);
            if (_mv_616.is_ok) {
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_618 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_618.is_ok) {
        __auto_type e = _mv_618.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_618.is_ok) {
        __auto_type expr = _mv_618.data.ok;
        __auto_type _mv_619 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_619.is_ok) {
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                ...
                if (!_mv_620.is_ok) {
                    ...
                    __auto_type operand = _mv_620.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
    ...
    switch (_mv_632.tag) {
        case types_SExpr_lst:
        {
            ...
            if (((int64_t)((l.items).len)) == 0) {
                ...
                __auto_type _mv_633 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    ...
                    while (i < len) {
                        ...
                        if (_mv_644.has_value) {
                            __auto_type child = _mv_644.value;
                            {
                                ...
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_645 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
