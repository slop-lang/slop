...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_615 = parser_lexer_next_token(arena, (&state));
            if (_mv_615.is_ok) {
                __auto_type tok = _mv_615.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_616 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_616.has_value) {
        __auto_type tok = _mv_616.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            if (_mv_617.is_ok) {
                __auto_type quoted_expr = _mv_617.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_rparen) {
                        ...
                        __auto_type _mv_618 = parser_parse_expr(arena, state);
                        if (_mv_618.is_ok) {
                            __auto_type expr = _mv_618.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_619 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_619.is_ok) {
        __auto_type e = _mv_619.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_619.is_ok) {
        __auto_type expr = _mv_619.data.ok;
        __auto_type _mv_620 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_620.is_ok) {
            __auto_type e = _mv_620.data.err;
            return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
        } else if (_mv_620.is_ok) {
            __auto_type _ = _mv_620.data.ok;
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                ...
                if (!_mv_621.is_ok) {
                    ...
                    __auto_type operand = _mv_621.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_622 = parser_parse_infix_primary(arena, state);
            if (!_mv_622.is_ok) {
                __auto_type e = _mv_622.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_622.is_ok) {
                __auto_type operand = _mv_622.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
        ...
        } else if (tok.kind == parser_TokenType_tok_lparen) {
            {
                ...
                parser_parser_advance(state);
                __auto_type _mv_623 = parser_parse_infix_primary(arena, state);
                if (!_mv_623.is_ok) {
            ...
            if (!_mv_624.is_ok) {
                ...
                __auto_type quoted_expr = _mv_624.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    ...
    if (!_mv_625.is_ok) {
        ...
        __auto_type left = _mv_625.data.ok;
        {
                    ...
                    if ((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or"))))) {
                        {
                            ...
                            if (op_prec < min_prec) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_626 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_626.is_ok) {
                                        __auto_type e = _mv_626.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_626.is_ok) {
                                        __auto_type right = _mv_626.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_628 = parser_tokenize(arena, source);
    if (_mv_628.is_ok) {
        __auto_type tokens = _mv_628.data.ok;
        {
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_630.tag) {
        ...
        {
            __auto_type s = _mv_630.data.sym;
            return s.line;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_630.data.str;
            return s.line;
        }
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_631.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_631.data.sym;
            return s.col;
        }
        case types_SExpr_str:
        {
        ...
        case types_SExpr_num:
        {
            __auto_type n = _mv_631.data.num;
            return n.col;
        }
        case types_SExpr_lst:
        {
    ...
    switch (_mv_633.tag) {
        case types_SExpr_lst:
        {
            ...
            if (((int64_t)((l.items).len)) == 0) {
                ...
                __auto_type _mv_634 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_634.has_value) {
                    __auto_type first = _mv_634.value;
...
uint8_t parser_sexpr_is_number(types_SExpr* expr) {
    ...
    switch (_mv_640.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_640.data.num;
            return 1;
        }
        default: {
            return 0;
...
slop_string parser_sexpr_string_value(types_SExpr* expr) {
    ...
    switch (_mv_643.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_643.data.str;
            return s.value;
        }
        default: {
            return SLOP_STR("");
                    ...
                    while (i < len) {
                        ...
                        if (_mv_645.has_value) {
                            __auto_type child = _mv_645.value;
                            {
                                ...
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_646 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_646.has_value) {
                                        __auto_type h = _mv_646.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_647.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_647.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_647.data.num;
            if (num.is_float) {
