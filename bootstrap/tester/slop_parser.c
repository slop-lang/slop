...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while ((!(done) && !(has_error))) {
            __auto_type _mv_549 = parser_lexer_next_token(arena, (&state));
            if (_mv_549.is_ok) {
                __auto_type tok = _mv_549.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_550 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_550.has_value) {
        __auto_type tok = _mv_550.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if ((tok.kind == parser_TokenType_tok_lparen)) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_551 = parser_parse_expr(arena, state);
            if (_mv_551.is_ok) {
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_rparen)) {
                        ...
                        __auto_type _mv_552 = parser_parse_expr(arena, state);
                        if (_mv_552.is_ok) {
                            __auto_type expr = _mv_552.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if ((tok.kind == parser_TokenType_tok_number)) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                parser_parser_advance(state);
                __auto_type _mv_553 = parser_parse_infix_primary(arena, state);
                if (!_mv_553.is_ok) {
                    __auto_type e = _mv_553.data.err;
                    return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
                } else if (_mv_553.is_ok) {
                    __auto_type operand = _mv_553.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_554 = parser_parse_infix_primary(arena, state);
            if (!_mv_554.is_ok) {
                __auto_type e = _mv_554.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_554.is_ok) {
                __auto_type operand = _mv_554.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_555 = parser_parse_infix_primary(arena, state);
            if (!_mv_555.is_ok) {
                __auto_type e = _mv_555.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_555.is_ok) {
                __auto_type quoted_expr = _mv_555.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    __auto_type _mv_556 = parser_parse_infix_primary(arena, state);
    if (!_mv_556.is_ok) {
        __auto_type e = _mv_556.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_556.is_ok) {
        __auto_type left = _mv_556.data.ok;
        {
                    ...
                    if (((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or")))))) {
                        {
                            ...
                            if ((op_prec < min_prec)) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_557 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_557.is_ok) {
                                        __auto_type e = _mv_557.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_557.is_ok) {
                                        __auto_type right = _mv_557.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (uint8_t*)slop_arena_alloc(arena, 128); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix(slop_arena* arena, parser_ParserState* state) {
    SLOP_PRE(((parser_parser_peek(state).kind == parser_TokenType_tok_lbrace)), "(== (. (parser-peek state) kind) (quote tok-lbrace))");
    {
        ...
        (*state).in_infix = 1;
        {
            ...
            (*state).in_infix = 0;
            __auto_type _mv_558 = result;
            if (!_mv_558.is_ok) {
                __auto_type e = _mv_558.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_558.is_ok) {
                __auto_type expr = _mv_558.data.ok;
                {
                    __auto_type tok = parser_parser_peek(state);
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_559 = parser_tokenize(arena, source);
    if (_mv_559.is_ok) {
        __auto_type tokens = _mv_559.data.ok;
        {
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_eof)) {
                        ...
                        __auto_type _mv_560 = parser_parse_expr(arena, (&state));
                        if (_mv_560.is_ok) {
                            __auto_type expr = _mv_560.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_561.tag) {
        ...
        {
            __auto_type s = _mv_561.data.sym;
            return s.line;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_561.data.str;
            return s.line;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_561.data.num;
            return n.line;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_561.data.lst;
            return l.line;
        }
    }
}
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_562.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_562.data.sym;
            return s.col;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_562.data.str;
            return s.col;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_562.data.num;
            return n.col;
        }
        case types_SExpr_lst:
        {
    ...
    switch (_mv_564.tag) {
        case types_SExpr_lst:
        {
            ...
            if ((((int64_t)((l.items).len)) == 0)) {
                ...
                __auto_type _mv_565 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
uint8_t parser_sexpr_is_list(types_SExpr* expr) {
    ...
    switch (_mv_568.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_568.data.lst;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_symbol(types_SExpr* expr) {
    ...
    switch (_mv_570.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_570.data.sym;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_number(types_SExpr* expr) {
    ...
    switch (_mv_571.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_571.data.num;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_string(types_SExpr* expr) {
    ...
    switch (_mv_572.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_572.data.str;
            return 1;
        }
        default: {
            return 0;
...
slop_string parser_sexpr_number_string(types_SExpr* expr) {
    ...
    switch (_mv_573.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_573.data.num;
            return n.raw;
        }
        default: {
            return SLOP_STR("");
...
slop_string parser_sexpr_string_value(types_SExpr* expr) {
    ...
    switch (_mv_574.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_574.data.str;
            return s.value;
        }
        default: {
            return SLOP_STR("");
...
slop_list_types_SExpr_ptr parser_find_holes(slop_arena* arena, types_SExpr* expr) {
    {
        ...
        switch (_mv_575.tag) {
            case types_SExpr_lst:
            {
                __auto_type l = _mv_575.data.lst;
                {
                    ...
                    while ((i < len)) {
                        ...
                        if (_mv_576.has_value) {
                            __auto_type child = _mv_576.value;
                            {
                                ...
                                __auto_type j = 0;
                                while ((j < child_len)) {
                                    __auto_type _mv_577 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_577.has_value) {
                                        __auto_type h = _mv_577.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_578.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_578.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_578.data.num;
            if (num.is_float) {
            ...
            __auto_type lst = _mv_578.data.lst;
            {
                ...
                if ((len == 0)) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while ((i < len)) {
                            __auto_type _mv_579 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_579.has_value) {
                                __auto_type child = _mv_579.value;
                                {
...
slop_string parser_json_print_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        if ((len == 0)) {
        ...
        } else {
            {
                ...
                __auto_type i = 0;
                while ((i < len)) {
                    __auto_type _mv_580 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_580.has_value) {
                        __auto_type child = _mv_580.value;
                        {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_581.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_581.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_581.data.num;
            {
