...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_631 = parser_lexer_next_token(arena, (&state));
            if (_mv_631.is_ok) {
                __auto_type tok = _mv_631.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_632 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_632.has_value) {
        __auto_type tok = _mv_632.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_633 = parser_parse_expr(arena, state);
            if (_mv_633.is_ok) {
                __auto_type quoted_expr = _mv_633.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_rparen) {
                        ...
                        __auto_type _mv_634 = parser_parse_expr(arena, state);
                        if (_mv_634.is_ok) {
                            __auto_type expr = _mv_634.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_635 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_635.is_ok) {
        __auto_type e = _mv_635.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_635.is_ok) {
        __auto_type expr = _mv_635.data.ok;
        __auto_type _mv_636 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_636.is_ok) {
            __auto_type e = _mv_636.data.err;
            return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
        } else if (_mv_636.is_ok) {
            __auto_type _ = _mv_636.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                parser_parser_advance(state);
                __auto_type _mv_637 = parser_parse_infix_primary(arena, state);
                if (!_mv_637.is_ok) {
                    __auto_type e = _mv_637.data.err;
                    return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
                } else if (_mv_637.is_ok) {
                    __auto_type operand = _mv_637.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_638 = parser_parse_infix_primary(arena, state);
            if (!_mv_638.is_ok) {
                __auto_type e = _mv_638.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_638.is_ok) {
                __auto_type operand = _mv_638.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
        ...
        } else if (tok.kind == parser_TokenType_tok_lparen) {
            {
                ...
                parser_parser_advance(state);
                __auto_type _mv_639 = parser_parse_infix_primary(arena, state);
                if (!_mv_639.is_ok) {
                    __auto_type _ = _mv_639.data.err;
                    (*state).pos = saved_pos;
                    return parser_parse_list(arena, state);
                } else if (_mv_639.is_ok) {
                    __auto_type _ = _mv_639.data.ok;
                    {
                        __auto_type next_tok = parser_parser_peek(state);
            ...
            parser_parser_advance(state);
            __auto_type _mv_640 = parser_parse_infix_primary(arena, state);
            if (!_mv_640.is_ok) {
                __auto_type e = _mv_640.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_640.is_ok) {
                __auto_type quoted_expr = _mv_640.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    __auto_type _mv_641 = parser_parse_infix_primary(arena, state);
    if (!_mv_641.is_ok) {
        __auto_type e = _mv_641.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_641.is_ok) {
        __auto_type left = _mv_641.data.ok;
        {
                    ...
                    if ((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or"))))) {
                        {
                            ...
                            if (op_prec < min_prec) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_642 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_642.is_ok) {
                                        __auto_type e = _mv_642.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_642.is_ok) {
                                        __auto_type right = _mv_642.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix(slop_arena* arena, parser_ParserState* state) {
    SLOP_PRE(((parser_parser_peek(state).kind == parser_TokenType_tok_lbrace)), "(== (. (parser-peek state) kind) (quote tok-lbrace))");
    {
        ...
        (*state).in_infix = 1;
        {
            ...
            (*state).in_infix = 0;
            __auto_type _mv_643 = result;
            if (!_mv_643.is_ok) {
                __auto_type e = _mv_643.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_643.is_ok) {
                __auto_type expr = _mv_643.data.ok;
                {
                    __auto_type tok = parser_parser_peek(state);
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_644 = parser_tokenize(arena, source);
    if (_mv_644.is_ok) {
        __auto_type tokens = _mv_644.data.ok;
        {
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_eof) {
                        ...
                        __auto_type _mv_645 = parser_parse_expr(arena, (&state));
                        if (_mv_645.is_ok) {
                            __auto_type expr = _mv_645.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_646.tag) {
        ...
        {
            __auto_type s = _mv_646.data.sym;
            return s.line;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_646.data.str;
            return s.line;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_646.data.num;
            return n.line;
        }
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_647.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type s = _mv_647.data.str;
            return s.col;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_647.data.num;
            return n.col;
        }
        case types_SExpr_lst:
        {
...
uint8_t parser_is_form(types_SExpr* expr, slop_string keyword) {
    ...
    switch (_mv_649.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_649.data.lst;
            if (((int64_t)((l.items).len)) == 0) {
                return 0;
            } else {
                __auto_type _mv_650 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_650.has_value) {
                    __auto_type first = _mv_650.value;
...
int64_t parser_sexpr_list_len(types_SExpr* expr) {
    ...
    switch (_mv_651.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_651.data.lst;
...
uint8_t parser_sexpr_is_list(types_SExpr* expr) {
    ...
    switch (_mv_653.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_653.data.lst;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_symbol(types_SExpr* expr) {
    ...
    switch (_mv_655.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_655.data.sym;
            return 1;
        }
        default: {
            return 0;
...
slop_string parser_sexpr_number_string(types_SExpr* expr) {
    ...
    switch (_mv_658.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_658.data.num;
            return n.raw;
        }
        default: {
            return SLOP_STR("");
...
slop_string parser_sexpr_string_value(types_SExpr* expr) {
    ...
    switch (_mv_659.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_659.data.str;
            return s.value;
        }
        default: {
            return SLOP_STR("");
...
slop_list_types_SExpr_ptr parser_find_holes(slop_arena* arena, types_SExpr* expr) {
    {
        ...
        switch (_mv_660.tag) {
            case types_SExpr_lst:
            {
                __auto_type l = _mv_660.data.lst;
                {
                    ...
                    while (i < len) {
                        ...
                        if (_mv_661.has_value) {
                            __auto_type child = _mv_661.value;
                            {
                                __auto_type child_holes = parser_find_holes(arena, child);
                                __auto_type child_len = ((int64_t)((child_holes).len));
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_662 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_662.has_value) {
                                        __auto_type h = _mv_662.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_663.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_663.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_663.data.num;
            if (num.is_float) {
                ...
                } else {
                    {
                        ...
                        while (i < len) {
                            ...
                            if (_mv_664.has_value) {
                                __auto_type child = _mv_664.value;
                                {
        ...
        } else {
            {
                ...
                while (i < len) {
                    ...
                    if (_mv_665.has_value) {
                        __auto_type child = _mv_665.value;
                        {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_666.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_666.data.sym;
            {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_666.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_666.data.num;
            {
