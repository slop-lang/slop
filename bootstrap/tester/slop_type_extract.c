...
type_extract_TypeRegistry* type_extract_extract_types_from_ast_with_imports(slop_arena* arena, slop_list_types_SExpr_ptr ast, slop_string module_prefix, slop_list_type_extract_ImportEntry imports) {
    {
        ...
        (*reg_ptr) = type_extract_make_type_registry_with_imports(arena, module_prefix, imports);
        {
            ...
            int64_t i = 0;
            while ((i < len)) {
                __auto_type _mv_1419 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1419.has_value) {
                    __auto_type expr = _mv_1419.value;
...
void type_extract_extract_types_from_module(slop_arena* arena, types_SExpr* module_form, type_extract_TypeRegistry* reg_ptr, slop_string prefix) {
    {
        ...
        int64_t i = 2;
        while ((i < len)) {
            __auto_type _mv_1420 = parser_sexpr_list_get(module_form, i);
            if (_mv_1420.has_value) {
                __auto_type form = _mv_1420.value;
...
void type_extract_extract_type_def(slop_arena* arena, types_SExpr* type_form, type_extract_TypeRegistry* reg_ptr, slop_string prefix) {
    if ((parser_sexpr_list_len(type_form) >= 3)) {
        __auto_type _mv_1421 = parser_sexpr_list_get(type_form, 1);
            ...
            if (parser_sexpr_is_symbol(name_expr)) {
                {
                    ...
                    __auto_type _mv_1422 = parser_sexpr_list_get(type_form, 2);
...
type_extract_TstTypeEntry* type_extract_extract_record_type(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t i = 1;
        while ((i < len)) {
            __auto_type _mv_1423 = parser_sexpr_list_get(def, i);
            if (_mv_1423.has_value) {
                __auto_type field_def = _mv_1423.value;
                if ((parser_sexpr_list_len(field_def) >= 2)) {
                    __auto_type _mv_1424 = parser_sexpr_list_get(field_def, 0);
                    if (_mv_1424.has_value) {
                        __auto_type field_name_expr = _mv_1424.value;
                        if (parser_sexpr_is_symbol(field_name_expr)) {
                            {
                                __auto_type field_name = parser_sexpr_get_symbol_name(field_name_expr);
                                __auto_type _mv_1425 = parser_sexpr_list_get(field_def, 1);
                                if (_mv_1425.has_value) {
                                    __auto_type field_type_expr = _mv_1425.value;
                                    {
...
type_extract_TstTypeEntry* type_extract_extract_union_type(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while ((i < len)) {
            __auto_type _mv_1426 = parser_sexpr_list_get(def, i);
            if (_mv_1426.has_value) {
                __auto_type variant_def = _mv_1426.value;
                {
                    __auto_type vlen = parser_sexpr_list_len(variant_def);
                    if ((vlen >= 1)) {
                        __auto_type _mv_1427 = parser_sexpr_list_get(variant_def, 0);
                        if (_mv_1427.has_value) {
                            __auto_type variant_name_expr = _mv_1427.value;
...
type_extract_TstTypeEntry* type_extract_extract_enum_type(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while ((i < len)) {
            __auto_type _mv_1428 = parser_sexpr_list_get(def, i);
...
slop_option_type_extract_TstTypeEntry_ptr type_extract_registry_lookup(type_extract_TypeRegistry reg, slop_string name) {
    {
        ...
        while (((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); }))) {
            __auto_type _mv_1429 = ({ __auto_type _lst = types; size_t _idx = (size_t)i; slop_option_type_extract_TstTypeEntry_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_option_type_extract_TstTypeEntry_ptr type_extract_registry_lookup_variant(type_extract_TypeRegistry reg, slop_string variant_name) {
    {
        ...
        slop_option_type_extract_TstTypeEntry_ptr found = (slop_option_type_extract_TstTypeEntry_ptr){.has_value = false};
        while (((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); }))) {
            __auto_type _mv_1430 = ({ __auto_type _lst = types; size_t _idx = (size_t)i; slop_option_type_extract_TstTypeEntry_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                ...
                if (((*entry).kind == type_extract_TstTypeEntryKind_te_union)) {
                    {
                        ...
                        while (((j < vlen) && !(vfound))) {
                            __auto_type _mv_1431 = ({ __auto_type _lst = variants; size_t _idx = (size_t)j; slop_option_type_extract_VariantEntry _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_option_type_extract_TstTypeEntry_ptr type_extract_registry_lookup_enum_value(type_extract_TypeRegistry reg, slop_string value_name) {
    {
        ...
        slop_option_type_extract_TstTypeEntry_ptr found = (slop_option_type_extract_TstTypeEntry_ptr){.has_value = false};
        while (((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); }))) {
            __auto_type _mv_1432 = ({ __auto_type _lst = types; size_t _idx = (size_t)i; slop_option_type_extract_TstTypeEntry_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                ...
                if (((*entry).kind == type_extract_TstTypeEntryKind_te_enum)) {
                    {
                        ...
                        __auto_type vfound = 0;
                        while (((j < vlen) && !(vfound))) {
                            __auto_type _mv_1433 = ({ __auto_type _lst = values; size_t _idx = (size_t)j; slop_option_type_extract_EnumValueEntry _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_1433.has_value) {
                                __auto_type eve = _mv_1433.value;
...
slop_option_string type_extract_registry_lookup_import(type_extract_TypeRegistry reg, slop_string symbol_name) {
    {
        ...
        while (((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); }))) {
            __auto_type _mv_1434 = ({ __auto_type _lst = entries; size_t _idx = (size_t)i; slop_option_type_extract_ImportEntry _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1434.has_value) {
                __auto_type entry = _mv_1434.value;
                {
                    ...
                    while (((j < slen) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); }))) {
                        __auto_type _mv_1435 = ({ __auto_type _lst = symbols; size_t _idx = (size_t)j; slop_option_string _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1435.has_value) {
                            __auto_type sym = _mv_1435.value;
...
uint8_t type_extract_registry_is_union(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1436 = type_extract_registry_lookup(reg, name);
...
uint8_t type_extract_registry_is_record(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1437 = type_extract_registry_lookup(reg, name);
...
uint8_t type_extract_registry_is_enum(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1438 = type_extract_registry_lookup(reg, name);
...
slop_option_type_extract_VariantEntry type_extract_registry_get_variant_info(type_extract_TypeRegistry reg, slop_string variant_name) {
    __auto_type _mv_1439 = type_extract_registry_lookup_variant(reg, variant_name);
    if (_mv_1439.has_value) {
        __auto_type entry = _mv_1439.value;
        {
            ...
            while (((i < vlen) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); }))) {
                __auto_type _mv_1440 = ({ __auto_type _lst = variants; size_t _idx = (size_t)i; slop_option_type_extract_VariantEntry _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1440.has_value) {
                    __auto_type ve = _mv_1440.value;
                    if (string_eq(ve.name, variant_name)) {
...
slop_option_list_type_extract_TstFieldEntry type_extract_registry_get_record_fields(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1441 = type_extract_registry_lookup(reg, name);
    if (_mv_1441.has_value) {
        __auto_type entry = _mv_1441.value;
