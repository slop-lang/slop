...
type_extract_TypeRegistry* type_extract_extract_types_from_ast_with_imports(slop_arena* arena, slop_list_types_SExpr_ptr ast, slop_string module_prefix, slop_list_type_extract_ImportEntry imports) {
    {
        ...
        (*reg_ptr) = type_extract_make_type_registry_with_imports(arena, module_prefix, imports);
        {
            ...
            int64_t i = 0;
            while (i < len) {
                __auto_type _mv_1512 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1512.has_value) {
                    __auto_type expr = _mv_1512.value;
...
void type_extract_extract_types_from_module(slop_arena* arena, types_SExpr* module_form, type_extract_TypeRegistry* reg_ptr, slop_string prefix) {
    {
        ...
        int64_t i = 2;
        while (i < len) {
            __auto_type _mv_1513 = parser_sexpr_list_get(module_form, i);
            if (_mv_1513.has_value) {
                __auto_type form = _mv_1513.value;
...
void type_extract_extract_type_def(slop_arena* arena, types_SExpr* type_form, type_extract_TypeRegistry* reg_ptr, slop_string prefix) {
    if (parser_sexpr_list_len(type_form) >= 3) {
        __auto_type _mv_1514 = parser_sexpr_list_get(type_form, 1);
            ...
            if (parser_sexpr_is_symbol(name_expr)) {
                {
                    ...
                    __auto_type _mv_1515 = parser_sexpr_list_get(type_form, 2);
...
type_extract_TstTypeEntry* type_extract_extract_record_type(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t i = 1;
        while (i < len) {
            __auto_type _mv_1516 = parser_sexpr_list_get(def, i);
            if (_mv_1516.has_value) {
                __auto_type field_def = _mv_1516.value;
                if (parser_sexpr_list_len(field_def) >= 2) {
                    __auto_type _mv_1517 = parser_sexpr_list_get(field_def, 0);
                    if (_mv_1517.has_value) {
                        __auto_type field_name_expr = _mv_1517.value;
                        if (parser_sexpr_is_symbol(field_name_expr)) {
                            {
                                __auto_type field_name = parser_sexpr_get_symbol_name(field_name_expr);
                                __auto_type _mv_1518 = parser_sexpr_list_get(field_def, 1);
                                if (_mv_1518.has_value) {
                                    __auto_type field_type_expr = _mv_1518.value;
                                    {
...
type_extract_TstTypeEntry* type_extract_extract_union_type(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while (i < len) {
            __auto_type _mv_1519 = parser_sexpr_list_get(def, i);
            if (_mv_1519.has_value) {
                __auto_type variant_def = _mv_1519.value;
                {
                    __auto_type vlen = parser_sexpr_list_len(variant_def);
                    if (vlen >= 1) {
                        __auto_type _mv_1520 = parser_sexpr_list_get(variant_def, 0);
                        if (_mv_1520.has_value) {
                            __auto_type variant_name_expr = _mv_1520.value;
...
type_extract_TstTypeEntry* type_extract_extract_enum_type(slop_arena* arena, slop_string name, slop_string c_name, types_SExpr* def) {
    {
        ...
        int64_t idx = 0;
        while (i < len) {
            __auto_type _mv_1521 = parser_sexpr_list_get(def, i);
            if (_mv_1521.has_value) {
                __auto_type val_expr = _mv_1521.value;
...
slop_option_type_extract_TstTypeEntry_ptr type_extract_registry_lookup(type_extract_TypeRegistry reg, slop_string name) {
    {
        ...
        while ((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); })) {
            __auto_type _mv_1522 = ({ __auto_type _lst = types; size_t _idx = (size_t)i; slop_option_type_extract_TstTypeEntry_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_option_type_extract_TstTypeEntry_ptr type_extract_registry_lookup_variant(type_extract_TypeRegistry reg, slop_string variant_name) {
    {
        ...
        slop_option_type_extract_TstTypeEntry_ptr found = (slop_option_type_extract_TstTypeEntry_ptr){.has_value = false};
        while ((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); })) {
            __auto_type _mv_1523 = ({ __auto_type _lst = types; size_t _idx = (size_t)i; slop_option_type_extract_TstTypeEntry_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                ...
                if ((*entry).kind == type_extract_TstTypeEntryKind_te_union) {
                    {
                        ...
                        while ((j < vlen) && !(vfound)) {
                            __auto_type _mv_1524 = ({ __auto_type _lst = variants; size_t _idx = (size_t)j; slop_option_type_extract_VariantEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_1524.has_value) {
                                __auto_type ve = _mv_1524.value;
...
slop_option_type_extract_TstTypeEntry_ptr type_extract_registry_lookup_enum_value(type_extract_TypeRegistry reg, slop_string value_name) {
    {
        ...
        slop_option_type_extract_TstTypeEntry_ptr found = (slop_option_type_extract_TstTypeEntry_ptr){.has_value = false};
        while ((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); })) {
            __auto_type _mv_1525 = ({ __auto_type _lst = types; size_t _idx = (size_t)i; slop_option_type_extract_TstTypeEntry_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1525.has_value) {
                __auto_type entry = _mv_1525.value;
                if ((*entry).kind == type_extract_TstTypeEntryKind_te_enum) {
                    {
                        ...
                        __auto_type vfound = 0;
                        while ((j < vlen) && !(vfound)) {
                            __auto_type _mv_1526 = ({ __auto_type _lst = values; size_t _idx = (size_t)j; slop_option_type_extract_EnumValueEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_1526.has_value) {
                                __auto_type eve = _mv_1526.value;
...
slop_option_string type_extract_registry_lookup_import(type_extract_TypeRegistry reg, slop_string symbol_name) {
    {
        ...
        while ((i < len) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); })) {
            __auto_type _mv_1527 = ({ __auto_type _lst = entries; size_t _idx = (size_t)i; slop_option_type_extract_ImportEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1527.has_value) {
                __auto_type entry = _mv_1527.value;
                {
                    ...
                    while ((j < slen) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); })) {
                        __auto_type _mv_1528 = ({ __auto_type _lst = symbols; size_t _idx = (size_t)j; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1528.has_value) {
                            __auto_type sym = _mv_1528.value;
...
uint8_t type_extract_registry_is_union(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1529 = type_extract_registry_lookup(reg, name);
    if (_mv_1529.has_value) {
        __auto_type entry = _mv_1529.value;
...
uint8_t type_extract_registry_is_record(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1530 = type_extract_registry_lookup(reg, name);
    if (_mv_1530.has_value) {
        __auto_type entry = _mv_1530.value;
...
uint8_t type_extract_registry_is_enum(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1531 = type_extract_registry_lookup(reg, name);
...
slop_option_type_extract_VariantEntry type_extract_registry_get_variant_info(type_extract_TypeRegistry reg, slop_string variant_name) {
    __auto_type _mv_1532 = type_extract_registry_lookup_variant(reg, variant_name);
    if (_mv_1532.has_value) {
        __auto_type entry = _mv_1532.value;
        {
            __auto_type variants = (*entry).variants;
            __auto_type vlen = ((int64_t)((variants).len));
            __auto_type i = 0;
            ...
            while ((i < vlen) && ({ __auto_type _mv = found; _mv.has_value ? (0) : (1); })) {
                __auto_type _mv_1533 = ({ __auto_type _lst = variants; size_t _idx = (size_t)i; slop_option_type_extract_VariantEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_1533.has_value) {
                    __auto_type ve = _mv_1533.value;
                    if (string_eq(ve.name, variant_name)) {
...
slop_option_list_type_extract_TstFieldEntry type_extract_registry_get_record_fields(type_extract_TypeRegistry reg, slop_string name) {
    __auto_type _mv_1534 = type_extract_registry_lookup(reg, name);
    if (_mv_1534.has_value) {
        __auto_type entry = _mv_1534.value;
