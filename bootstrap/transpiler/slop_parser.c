...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while ((!(done) && !(has_error))) {
            __auto_type _mv_602 = parser_lexer_next_token(arena, (&state));
            if (_mv_602.is_ok) {
                __auto_type tok = _mv_602.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_603 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_603.has_value) {
        __auto_type tok = _mv_603.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if ((tok.kind == parser_TokenType_tok_lparen)) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_604 = parser_parse_expr(arena, state);
            if (_mv_604.is_ok) {
                __auto_type quoted_expr = _mv_604.data.ok;
                {
                    __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
                ...
                __auto_type e = _mv_604.data.err;
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_rparen)) {
                        ...
                        __auto_type _mv_605 = parser_parse_expr(arena, state);
                        if (_mv_605.is_ok) {
                            __auto_type expr = _mv_605.data.ok;
        ...
        if ((tok.kind == parser_TokenType_tok_number)) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                ...
                if (!_mv_606.is_ok) {
                    ...
                    __auto_type operand = _mv_606.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
            ...
            parser_parser_advance(state);
            __auto_type _mv_607 = parser_parse_infix_primary(arena, state);
            if (!_mv_607.is_ok) {
                __auto_type e = _mv_607.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_607.is_ok) {
                __auto_type operand = _mv_607.data.ok;
                {
                    __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
            ...
            parser_parser_advance(state);
            __auto_type _mv_608 = parser_parse_infix_primary(arena, state);
            if (!_mv_608.is_ok) {
                __auto_type e = _mv_608.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_608.is_ok) {
                __auto_type quoted_expr = _mv_608.data.ok;
                {
                    __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
                    __auto_type quote_sym = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    __auto_type _mv_609 = parser_parse_infix_primary(arena, state);
    if (!_mv_609.is_ok) {
        __auto_type e = _mv_609.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_609.is_ok) {
        __auto_type left = _mv_609.data.ok;
        {
                    ...
                    if (((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or")))))) {
                        {
                            ...
                            if ((op_prec < min_prec)) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_610 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_610.is_ok) {
                                        __auto_type e = _mv_610.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_610.is_ok) {
                                        __auto_type right = _mv_610.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix(slop_arena* arena, parser_ParserState* state) {
    SLOP_PRE(((parser_parser_peek(state).kind == parser_TokenType_tok_lbrace)), "(== (. (parser-peek state) kind) (quote tok-lbrace))");
    {
        ...
        (*state).in_infix = 1;
        {
            ...
            (*state).in_infix = 0;
            __auto_type _mv_611 = result;
            if (!_mv_611.is_ok) {
                __auto_type e = _mv_611.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_611.is_ok) {
                __auto_type expr = _mv_611.data.ok;
                {
                    __auto_type tok = parser_parser_peek(state);
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_612 = parser_tokenize(arena, source);
    if (_mv_612.is_ok) {
        __auto_type tokens = _mv_612.data.ok;
        {
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_614.tag) {
        ...
        {
            __auto_type s = _mv_614.data.str;
            return s.line;
        }
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_615.tag) {
        ...
        case types_SExpr_num:
        {
            __auto_type n = _mv_615.data.num;
            return n.col;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_615.data.lst;
            return l.col;
        }
    }
}
    ...
    switch (_mv_617.tag) {
        case types_SExpr_lst:
        {
            ...
            if ((((int64_t)((l.items).len)) == 0)) {
                ...
                __auto_type _mv_618 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_618.has_value) {
                    __auto_type first = _mv_618.value;
...
int64_t parser_sexpr_list_len(types_SExpr* expr) {
    ...
    switch (_mv_619.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_619.data.lst;
...
uint8_t parser_sexpr_is_symbol(types_SExpr* expr) {
    ...
    switch (_mv_623.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_623.data.sym;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_number(types_SExpr* expr) {
    ...
    switch (_mv_624.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_624.data.num;
            return 1;
        }
        default: {
            return 0;
        ...
        switch (_mv_628.tag) {
            case types_SExpr_lst:
            {
                __auto_type l = _mv_628.data.lst;
                {
                    ...
                    __auto_type i = 0;
                    while ((i < len)) {
                        __auto_type _mv_629 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_629.has_value) {
                            __auto_type child = _mv_629.value;
                            {
                                ...
                                __auto_type j = 0;
                                while ((j < child_len)) {
                                    __auto_type _mv_630 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_630.has_value) {
                                        __auto_type h = _mv_630.value;
            ...
            __auto_type lst = _mv_631.data.lst;
            {
                ...
                if ((len == 0)) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while ((i < len)) {
                            __auto_type _mv_632 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_632.has_value) {
                                __auto_type child = _mv_632.value;
                                {
...
slop_string parser_json_print_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        if ((len == 0)) {
        ...
        } else {
            {
                ...
                __auto_type i = 0;
                while ((i < len)) {
                    __auto_type _mv_633 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_633.has_value) {
                        __auto_type child = _mv_633.value;
                        {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_634.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_634.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_634.data.num;
            {
