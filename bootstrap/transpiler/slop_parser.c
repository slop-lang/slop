...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_618 = parser_lexer_next_token(arena, (&state));
            if (_mv_618.is_ok) {
                __auto_type tok = _mv_618.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_619 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_619.has_value) {
        __auto_type tok = _mv_619.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_620 = parser_parse_expr(arena, state);
            if (_mv_620.is_ok) {
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_rparen) {
                        ...
                        if (_mv_621.is_ok) {
                            __auto_type expr = _mv_621.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_622 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_622.is_ok) {
        __auto_type e = _mv_622.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_622.is_ok) {
        __auto_type expr = _mv_622.data.ok;
        __auto_type _mv_623 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_623.is_ok) {
            __auto_type e = _mv_623.data.err;
            return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
        } else if (_mv_623.is_ok) {
            __auto_type _ = _mv_623.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                parser_parser_advance(state);
                __auto_type _mv_624 = parser_parse_infix_primary(arena, state);
                if (!_mv_624.is_ok) {
                    __auto_type e = _mv_624.data.err;
                    return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
                } else if (_mv_624.is_ok) {
                    __auto_type operand = _mv_624.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            if (!_mv_627.is_ok) {
                ...
                __auto_type quoted_expr = _mv_627.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    ...
    if (!_mv_628.is_ok) {
        ...
        __auto_type left = _mv_628.data.ok;
        {
                    ...
                    if ((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or"))))) {
                        {
                            ...
                            if (op_prec < min_prec) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_629 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_629.is_ok) {
                                        __auto_type e = _mv_629.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_629.is_ok) {
                                        __auto_type right = _mv_629.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_631 = parser_tokenize(arena, source);
    if (_mv_631.is_ok) {
        __auto_type tokens = _mv_631.data.ok;
        {
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_eof) {
                        ...
                        __auto_type _mv_632 = parser_parse_expr(arena, (&state));
                        if (_mv_632.is_ok) {
                            __auto_type expr = _mv_632.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_633.tag) {
        ...
        {
            __auto_type s = _mv_633.data.sym;
            return s.line;
        }
        ...
        {
            __auto_type n = _mv_633.data.num;
            return n.line;
        }
...
uint8_t parser_is_form(types_SExpr* expr, slop_string keyword) {
    ...
    switch (_mv_636.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_636.data.lst;
            if (((int64_t)((l.items).len)) == 0) {
                return 0;
            } else {
                __auto_type _mv_637 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_637.has_value) {
                    __auto_type first = _mv_637.value;
...
uint8_t parser_sexpr_is_number(types_SExpr* expr) {
    ...
    switch (_mv_643.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_643.data.num;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_string(types_SExpr* expr) {
    ...
    switch (_mv_644.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_644.data.str;
            return 1;
        }
        default: {
            return 0;
...
slop_string parser_sexpr_number_string(types_SExpr* expr) {
    ...
    switch (_mv_645.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_645.data.num;
            return n.raw;
        }
        default: {
            return SLOP_STR("");
                ...
                __auto_type l = _mv_647.data.lst;
                {
                    ...
                    while (i < len) {
                        ...
                        if (_mv_648.has_value) {
                            __auto_type child = _mv_648.value;
                            {
                                __auto_type child_holes = parser_find_holes(arena, child);
                                __auto_type child_len = ((int64_t)((child_holes).len));
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_649 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_649.has_value) {
                                        __auto_type h = _mv_649.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_650.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_650.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_650.data.num;
            if (num.is_float) {
                ...
                } else {
                    {
                        ...
                        while (i < len) {
                            ...
                            if (_mv_651.has_value) {
                                __auto_type child = _mv_651.value;
                                {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_653.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_653.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_653.data.num;
            {
