...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while ((!(done) && !(has_error))) {
            __auto_type _mv_617 = parser_lexer_next_token(arena, (&state));
            if (_mv_617.is_ok) {
                __auto_type tok = _mv_617.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_618 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_618.has_value) {
        __auto_type tok = _mv_618.value;
        return tok;
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_rparen)) {
                        ...
                        if (_mv_620.is_ok) {
                            __auto_type expr = _mv_620.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if ((tok.kind == parser_TokenType_tok_number)) {
            ...
            if (!_mv_623.is_ok) {
                ...
                __auto_type quoted_expr = _mv_623.data.ok;
                {
                    __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
                    __auto_type quote_sym = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    ...
    if (!_mv_624.is_ok) {
        ...
        __auto_type left = _mv_624.data.ok;
        {
                    ...
                    if (((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or")))))) {
                        {
                            ...
                            if ((op_prec < min_prec)) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_625 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_625.is_ok) {
                                        __auto_type e = _mv_625.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_625.is_ok) {
                                        __auto_type right = _mv_625.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)((uint8_t*)slop_arena_alloc(arena, 128)));
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_627 = parser_tokenize(arena, source);
    if (_mv_627.is_ok) {
        __auto_type tokens = _mv_627.data.ok;
        {
            ...
            while ((!(done) && !(has_error))) {
                {
                    ...
                    if ((tok.kind == parser_TokenType_tok_eof)) {
                        ...
                        if (_mv_628.is_ok) {
                            __auto_type expr = _mv_628.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_629.tag) {
        ...
        {
            __auto_type n = _mv_629.data.num;
            return n.line;
        }
    ...
    switch (_mv_632.tag) {
        case types_SExpr_lst:
        {
            ...
            if ((((int64_t)((l.items).len)) == 0)) {
                ...
                __auto_type _mv_633 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_633.has_value) {
                    __auto_type first = _mv_633.value;
...
uint8_t parser_sexpr_is_string(types_SExpr* expr) {
    ...
    switch (_mv_640.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_640.data.str;
            return 1;
        }
        default: {
            return 0;
        ...
        switch (_mv_643.tag) {
            case types_SExpr_lst:
            {
                __auto_type l = _mv_643.data.lst;
                {
                    ...
                    __auto_type i = 0;
                    while ((i < len)) {
                        __auto_type _mv_644 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_644.has_value) {
                            __auto_type child = _mv_644.value;
                            {
                                ...
                                __auto_type j = 0;
                                while ((j < child_len)) {
                                    __auto_type _mv_645 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_645.has_value) {
                                        __auto_type h = _mv_645.value;
                ...
                } else {
                    {
                        ...
                        while ((i < len)) {
                            ...
                            if (_mv_647.has_value) {
                                __auto_type child = _mv_647.value;
                                {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_649.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_649.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_649.data.num;
            {
