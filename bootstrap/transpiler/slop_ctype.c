...
slop_string ctype_to_c_type(slop_arena* arena, types_SExpr* expr) {
    ...
    __auto_type _mv_19 = (*expr);
    switch (_mv_19.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_19.data.sym;
            {
                __auto_type name = sym.name;
                __auto_type _mv_20 = ctype_builtin_type_c(arena, name);
                if (_mv_20.has_value) {
                    __auto_type c_type = _mv_20.value;
                    return c_type;
        ...
        {
            __auto_type lst = _mv_19.data.lst;
            return ctype_to_c_type_compound(arena, lst.items);
        }
        case types_SExpr_str:
        {
            __auto_type _ = _mv_19.data.str;
            return SLOP_STR("void*");
        }
        case types_SExpr_num:
        {
            __auto_type _ = _mv_19.data.num;
            return SLOP_STR("void*");
        }
    }
}
...
slop_string ctype_to_c_type_compound(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        if ((len < 1)) {
            ...
            __auto_type _mv_21 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_21.has_value) {
                __auto_type first_expr = _mv_21.value;
                __auto_type _mv_22 = (*first_expr);
                switch (_mv_22.tag) {
                    case types_SExpr_sym:
                    {
                        __auto_type head_sym = _mv_22.data.sym;
                        {
                            ...
                            if (string_eq(head, SLOP_STR("Ptr"))) {
                                ...
                                if ((len < 2)) {
                                    ...
                                    if (_mv_29.has_value) {
                                        __auto_type inner = _mv_29.value;
                                        {
                                            __auto_type inner_c = ctype_to_c_type(arena, inner);
                                            __auto_type inner_id = ctype_type_to_identifier(arena, inner_c);
            ...
            __auto_type args_list = _mv_31.data.lst;
            {
                ...
                if ((arg_count == 0)) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while ((i < arg_count)) {
                            __auto_type _mv_32 = ({ __auto_type _lst = arg_items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_32.has_value) {
    ...
    switch (_mv_33.tag) {
        ...
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_33.data.lst;
            {
                ...
                __auto_type i = 0;
                while ((i < len)) {
                    __auto_type _mv_34 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_34.has_value) {
...
slop_string ctype_range_type_to_c_type(slop_arena* arena, slop_list_types_SExpr_ptr items, int64_t len) {
    if ((len < 4)) {
    ...
    } else {
        {
            ...
            uint8_t has_min = 0;
            uint8_t has_max = 0;
            __auto_type _mv_35 = ({ __auto_type _lst = items; size_t _idx = (size_t)1; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_35.has_value) {
                __auto_type min_expr = _mv_35.value;
                __auto_type _mv_36 = (*min_expr);
                switch (_mv_36.tag) {
                    case types_SExpr_num:
                    {
            ...
            __auto_type _mv_37 = ({ __auto_type _lst = items; size_t _idx = (size_t)3; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_37.has_value) {
                __auto_type max_expr = _mv_37.value;
                __auto_type _mv_38 = (*max_expr);
                switch (_mv_38.tag) {
                    case types_SExpr_num:
                    {
                        __auto_type n = _mv_38.data.num;
                        has_max = 1;
...
slop_option_types_ResolvedType_ptr ctype_get_node_resolved_type(types_SExpr* expr) {
    ...
    switch (_mv_39.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_39.data.sym;
            return sym.resolved_type;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_39.data.str;
            return str.resolved_type;
        }
        case types_SExpr_num:
        {
            __auto_type num = _mv_39.data.num;
            return num.resolved_type;
        }
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_39.data.lst;
    ...
    SLOP_PRE(((rt != NULL)), "(!= rt nil)");
    {
        ...
        if ((kind == types_ResolvedTypeKind_rk_primitive)) {
        ...
        } else if (((kind == types_ResolvedTypeKind_rk_record) || ((kind == types_ResolvedTypeKind_rk_union) || (kind == types_ResolvedTypeKind_rk_enum)))) {
            {
                ...
                if (_mv_41.has_value) {
                    __auto_type mod_name = _mv_41.value;
