...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_619 = parser_lexer_next_token(arena, (&state));
            if (_mv_619.is_ok) {
                __auto_type tok = _mv_619.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_620 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_620.has_value) {
        __auto_type tok = _mv_620.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_621 = parser_parse_expr(arena, state);
            if (_mv_621.is_ok) {
                __auto_type quoted_expr = _mv_621.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_rparen) {
                        ...
                        __auto_type _mv_622 = parser_parse_expr(arena, state);
                        if (_mv_622.is_ok) {
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_623 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_623.is_ok) {
        __auto_type e = _mv_623.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_623.is_ok) {
        __auto_type expr = _mv_623.data.ok;
        __auto_type _mv_624 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_624.is_ok) {
            __auto_type e = _mv_624.data.err;
            return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
        } else if (_mv_624.is_ok) {
            __auto_type _ = _mv_624.data.ok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                parser_parser_advance(state);
                __auto_type _mv_625 = parser_parse_infix_primary(arena, state);
                if (!_mv_625.is_ok) {
                    __auto_type e = _mv_625.data.err;
                    return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
                } else if (_mv_625.is_ok) {
                    __auto_type operand = _mv_625.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_626 = parser_parse_infix_primary(arena, state);
            if (!_mv_626.is_ok) {
                __auto_type e = _mv_626.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_626.is_ok) {
                __auto_type operand = _mv_626.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
        ...
        } else if (tok.kind == parser_TokenType_tok_lparen) {
            {
                ...
                parser_parser_advance(state);
                __auto_type _mv_627 = parser_parse_infix_primary(arena, state);
                if (!_mv_627.is_ok) {
                    __auto_type _ = _mv_627.data.err;
                    (*state).pos = saved_pos;
                    return parser_parse_list(arena, state);
                } else if (_mv_627.is_ok) {
                    __auto_type _ = _mv_627.data.ok;
                    {
                        __auto_type next_tok = parser_parser_peek(state);
            ...
            parser_parser_advance(state);
            __auto_type _mv_628 = parser_parse_infix_primary(arena, state);
            if (!_mv_628.is_ok) {
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    ...
    if (!_mv_629.is_ok) {
        ...
        __auto_type left = _mv_629.data.ok;
        {
                    ...
                    if ((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or"))))) {
                        {
                            ...
                            if (op_prec < min_prec) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_630 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_630.is_ok) {
                                        __auto_type e = _mv_630.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_630.is_ok) {
                                        __auto_type right = _mv_630.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix(slop_arena* arena, parser_ParserState* state) {
    SLOP_PRE(((parser_parser_peek(state).kind == parser_TokenType_tok_lbrace)), "(== (. (parser-peek state) kind) (quote tok-lbrace))");
    {
        ...
        (*state).in_infix = 1;
        {
            ...
            (*state).in_infix = 0;
            __auto_type _mv_631 = result;
            if (!_mv_631.is_ok) {
                __auto_type e = _mv_631.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_631.is_ok) {
                __auto_type expr = _mv_631.data.ok;
                {
                    __auto_type tok = parser_parser_peek(state);
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_632 = parser_tokenize(arena, source);
    if (_mv_632.is_ok) {
        __auto_type tokens = _mv_632.data.ok;
        {
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_eof) {
                        ...
                        if (_mv_633.is_ok) {
                            __auto_type expr = _mv_633.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_634.tag) {
        ...
        {
            __auto_type s = _mv_634.data.str;
            return s.line;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_634.data.num;
            return n.line;
        }
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_635.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type s = _mv_635.data.str;
            return s.col;
        }
        case types_SExpr_num:
        {
        ...
        case types_SExpr_lst:
        {
            __auto_type l = _mv_635.data.lst;
            return l.col;
        }
    }
    SLOP_UNREACHABLE();
...
uint8_t parser_is_form(types_SExpr* expr, slop_string keyword) {
    ...
    switch (_mv_637.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_637.data.lst;
            if (((int64_t)((l.items).len)) == 0) {
                return 0;
            } else {
                __auto_type _mv_638 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_638.has_value) {
                    __auto_type first = _mv_638.value;
...
uint8_t parser_sexpr_is_symbol(types_SExpr* expr) {
    ...
    switch (_mv_643.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_643.data.sym;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_number(types_SExpr* expr) {
    ...
    switch (_mv_644.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_644.data.num;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_string(types_SExpr* expr) {
    ...
    switch (_mv_645.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_645.data.str;
            return 1;
        }
        default: {
            return 0;
                ...
                __auto_type l = _mv_648.data.lst;
                {
                    ...
                    while (i < len) {
                        ...
                        if (_mv_649.has_value) {
                            __auto_type child = _mv_649.value;
                            {
                                __auto_type child_holes = parser_find_holes(arena, child);
                                __auto_type child_len = ((int64_t)((child_holes).len));
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_650 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_650.has_value) {
                                        __auto_type h = _mv_650.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_651.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_651.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_651.data.num;
            if (num.is_float) {
                ...
                } else {
                    {
                        ...
                        while (i < len) {
                            ...
                            if (_mv_652.has_value) {
                                __auto_type child = _mv_652.value;
                                {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_654.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_654.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_654.data.num;
            {
