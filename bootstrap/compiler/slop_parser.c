...
slop_result_list_parser_Token_parser_ParseError parser_tokenize(slop_arena* arena, slop_string source) {
    {
        ...
        while (!(done) && !(has_error)) {
            __auto_type _mv_605 = parser_lexer_next_token(arena, (&state));
            if (_mv_605.is_ok) {
                __auto_type tok = _mv_605.data.ok;
...
parser_Token parser_parser_peek(parser_ParserState* state) {
    __auto_type _mv_606 = ({ __auto_type _lst = (*state).tokens; size_t _idx = (size_t)(*state).pos; slop_option_parser_Token _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    if (_mv_606.has_value) {
        __auto_type tok = _mv_606.value;
        return tok;
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_expr(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_lparen) {
            ...
            parser_parser_advance(state);
            __auto_type _mv_607 = parser_parse_expr(arena, state);
            if (_mv_607.is_ok) {
                __auto_type quoted_expr = _mv_607.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_rparen) {
                        ...
                        __auto_type _mv_608 = parser_parse_expr(arena, state);
                        if (_mv_608.is_ok) {
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_paren_group(slop_arena* arena, parser_ParserState* state) {
    parser_parser_advance(state);
    __auto_type _mv_609 = parser_parse_infix_prec(arena, state, 0);
    if (!_mv_609.is_ok) {
        __auto_type e = _mv_609.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_609.is_ok) {
        __auto_type expr = _mv_609.data.ok;
        __auto_type _mv_610 = parser_parser_expect(state, parser_TokenType_tok_rparen);
        if (!_mv_610.is_ok) {
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_primary(slop_arena* arena, parser_ParserState* state) {
    {
        ...
        if (tok.kind == parser_TokenType_tok_number) {
            ...
            if (string_eq(tok.value, SLOP_STR("not"))) {
                parser_parser_advance(state);
                __auto_type _mv_611 = parser_parse_infix_primary(arena, state);
                if (!_mv_611.is_ok) {
                    __auto_type e = _mv_611.data.err;
                    return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
                } else if (_mv_611.is_ok) {
                    __auto_type operand = _mv_611.data.ok;
                    {
                        __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
            ...
            parser_parser_advance(state);
            __auto_type _mv_612 = parser_parse_infix_primary(arena, state);
            if (!_mv_612.is_ok) {
                __auto_type e = _mv_612.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_612.is_ok) {
                __auto_type operand = _mv_612.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
        ...
        } else if (tok.kind == parser_TokenType_tok_lparen) {
            {
                ...
                parser_parser_advance(state);
                __auto_type _mv_613 = parser_parse_infix_primary(arena, state);
                if (!_mv_613.is_ok) {
                    __auto_type _ = _mv_613.data.err;
                    (*state).pos = saved_pos;
                    return parser_parse_list(arena, state);
                } else if (_mv_613.is_ok) {
                    __auto_type _ = _mv_613.data.ok;
                    {
                        __auto_type next_tok = parser_parser_peek(state);
            ...
            parser_parser_advance(state);
            __auto_type _mv_614 = parser_parse_infix_primary(arena, state);
            if (!_mv_614.is_ok) {
                __auto_type e = _mv_614.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_614.is_ok) {
                __auto_type quoted_expr = _mv_614.data.ok;
                {
                    __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type quote_sym = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
                    __auto_type items = ((slop_list_types_SExpr_ptr){ .data = (types_SExpr**)slop_arena_alloc(arena, 16 * sizeof(types_SExpr*)), .len = 0, .cap = 16 });
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix_prec(slop_arena* arena, parser_ParserState* state, int64_t min_prec) {
    __auto_type _mv_615 = parser_parse_infix_primary(arena, state);
    if (!_mv_615.is_ok) {
        __auto_type e = _mv_615.data.err;
        return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
    } else if (_mv_615.is_ok) {
        __auto_type left = _mv_615.data.ok;
        {
                    ...
                    if ((tok.kind == parser_TokenType_tok_operator) || ((tok.kind == parser_TokenType_tok_symbol) && (string_eq(tok.value, SLOP_STR("and")) || string_eq(tok.value, SLOP_STR("or"))))) {
                        {
                            ...
                            if (op_prec < min_prec) {
                            ...
                            } else {
                                {
                                    ...
                                    parser_parser_advance(state);
                                    __auto_type _mv_616 = parser_parse_infix_prec(arena, state, (op_prec + 1));
                                    if (!_mv_616.is_ok) {
                                        __auto_type e = _mv_616.data.err;
                                        has_error = 1;
                                        error_val = e;
                                    } else if (_mv_616.is_ok) {
                                        __auto_type right = _mv_616.data.ok;
                                        {
                                            __auto_type node = ((types_SExpr*)(({ __auto_type _alloc = (types_SExpr*)slop_arena_alloc(arena, sizeof(types_SExpr)); if (_alloc == NULL) { fprintf(stderr, "SLOP: arena alloc failed at %s:%d\n", __FILE__, __LINE__); abort(); } _alloc; })));
...
slop_result_types_SExpr_ptr_parser_ParseError parser_parse_infix(slop_arena* arena, parser_ParserState* state) {
    SLOP_PRE(((parser_parser_peek(state).kind == parser_TokenType_tok_lbrace)), "(== (. (parser-peek state) kind) (quote tok-lbrace))");
    {
        ...
        (*state).in_infix = 1;
        {
            ...
            (*state).in_infix = 0;
            __auto_type _mv_617 = result;
            if (!_mv_617.is_ok) {
                __auto_type e = _mv_617.data.err;
                return ((slop_result_types_SExpr_ptr_parser_ParseError){ .is_ok = false, .data.err = e });
            } else if (_mv_617.is_ok) {
                __auto_type expr = _mv_617.data.ok;
                {
                    __auto_type tok = parser_parser_peek(state);
...
slop_result_list_types_SExpr_ptr_parser_ParseError parser_parse(slop_arena* arena, slop_string source) {
    __auto_type _mv_618 = parser_tokenize(arena, source);
    if (_mv_618.is_ok) {
        __auto_type tokens = _mv_618.data.ok;
        {
            ...
            while (!(done) && !(has_error)) {
                {
                    ...
                    if (tok.kind == parser_TokenType_tok_eof) {
                        ...
                        __auto_type _mv_619 = parser_parse_expr(arena, (&state));
                        if (_mv_619.is_ok) {
                            __auto_type expr = _mv_619.data.ok;
...
int64_t parser_sexpr_line(types_SExpr* expr) {
    ...
    switch (_mv_620.tag) {
        ...
        {
            __auto_type s = _mv_620.data.str;
            return s.line;
        }
        case types_SExpr_num:
        {
            __auto_type n = _mv_620.data.num;
            return n.line;
        }
        case types_SExpr_lst:
        {
            __auto_type l = _mv_620.data.lst;
            return l.line;
        }
    }
}
...
int64_t parser_sexpr_col(types_SExpr* expr) {
    ...
    switch (_mv_621.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_621.data.sym;
            return s.col;
        }
        case types_SExpr_str:
        {
            __auto_type s = _mv_621.data.str;
            return s.col;
        }
        case types_SExpr_num:
        {
        ...
        case types_SExpr_lst:
        {
            __auto_type l = _mv_621.data.lst;
            return l.col;
        }
    }
}
...
uint8_t parser_is_form(types_SExpr* expr, slop_string keyword) {
    ...
    switch (_mv_623.tag) {
        case types_SExpr_lst:
        {
            __auto_type l = _mv_623.data.lst;
            if (((int64_t)((l.items).len)) == 0) {
                return 0;
            } else {
                __auto_type _mv_624 = ({ __auto_type _lst = l.items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_624.has_value) {
                    __auto_type first = _mv_624.value;
...
uint8_t parser_sexpr_is_symbol(types_SExpr* expr) {
    ...
    switch (_mv_629.tag) {
        case types_SExpr_sym:
        {
            __auto_type s = _mv_629.data.sym;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_number(types_SExpr* expr) {
    ...
    switch (_mv_630.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_630.data.num;
            return 1;
        }
        default: {
            return 0;
...
uint8_t parser_sexpr_is_string(types_SExpr* expr) {
    ...
    switch (_mv_631.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_631.data.str;
            return 1;
        }
        default: {
            return 0;
...
slop_string parser_sexpr_number_string(types_SExpr* expr) {
    ...
    switch (_mv_632.tag) {
        case types_SExpr_num:
        {
            __auto_type n = _mv_632.data.num;
            return n.raw;
        }
        default: {
            return SLOP_STR("");
...
slop_string parser_sexpr_string_value(types_SExpr* expr) {
    ...
    switch (_mv_633.tag) {
        case types_SExpr_str:
        {
            __auto_type s = _mv_633.data.str;
            return s.value;
        }
        default: {
            return SLOP_STR("");
...
slop_list_types_SExpr_ptr parser_find_holes(slop_arena* arena, types_SExpr* expr) {
    {
        ...
        switch (_mv_634.tag) {
            case types_SExpr_lst:
            {
                __auto_type l = _mv_634.data.lst;
                {
                    __auto_type items = l.items;
                    __auto_type len = ((int64_t)((items).len));
                    __auto_type i = 0;
                    while (i < len) {
                        __auto_type _mv_635 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_635.has_value) {
                            __auto_type child = _mv_635.value;
                            {
                                __auto_type child_holes = parser_find_holes(arena, child);
                                __auto_type child_len = ((int64_t)((child_holes).len));
                                __auto_type j = 0;
                                while (j < child_len) {
                                    __auto_type _mv_636 = ({ __auto_type _lst = child_holes; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                    if (_mv_636.has_value) {
                                        __auto_type h = _mv_636.value;
...
slop_string parser_pretty_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_637.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_637.data.sym;
            return parser_string_copy(arena, sym.name);
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_637.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_637.data.num;
            if (num.is_float) {
            ...
            __auto_type lst = _mv_637.data.lst;
            {
                ...
                if (len == 0) {
                ...
                } else {
                    {
                        ...
                        __auto_type i = 0;
                        while (i < len) {
                            __auto_type _mv_638 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_638.has_value) {
                                __auto_type child = _mv_638.value;
                                {
...
slop_string parser_json_print_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_639 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_639.has_value) {
                        __auto_type child = _mv_639.value;
                        {
...
slop_string parser_json_print(slop_arena* arena, types_SExpr* expr) {
    ...
    switch (_mv_640.tag) {
        ...
        case types_SExpr_str:
        {
            __auto_type str = _mv_640.data.str;
            {
        ...
        case types_SExpr_num:
        {
            __auto_type num = _mv_640.data.num;
            {
