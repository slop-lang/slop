...
void env_env_bind_var(env_TypeEnv* env, slop_string name, types_ResolvedType* var_type) {
    ...
    SLOP_PRE(((((int64_t)(((*env).scopes).len)) > 0)), "(> (list-len (. (deref env) scopes)) 0)");
    {
        ...
        __auto_type _mv_924 = ({ __auto_type _lst = scopes; size_t _idx = (size_t)top_idx; slop_option_env_CheckerScope_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_924.has_value) {
            __auto_type scope_ptr = _mv_924.value;
...
slop_option_types_ResolvedType_ptr env_scope_lookup_var(env_CheckerScope* scope_ptr, slop_string name) {
    SLOP_PRE(((scope_ptr != NULL)), "(!= scope-ptr nil)");
    {
        ...
        for (int64_t j = 0; j < num_bindings; j++) {
            __auto_type _mv_925 = ({ __auto_type _lst = bindings; size_t _idx = (size_t)j; slop_option_env_VarBinding _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            ...
            if (!(found)) {
                {
                    ...
                    __auto_type _mv_926 = ({ __auto_type _lst = scopes; size_t _idx = (size_t)scope_idx; slop_option_env_CheckerScope_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_926.has_value) {
                        __auto_type scope_ptr = _mv_926.value;
                        __auto_type _mv_927 = env_scope_lookup_var(scope_ptr, name);
                        if (_mv_927.has_value) {
                            __auto_type var_type = _mv_927.value;
                            found = 1;
...
uint8_t env_env_constant_matches_module(env_ConstBinding binding, slop_string mod_name) {
    ...
    if (_mv_928.has_value) {
        __auto_type bmod = _mv_928.value;
...
slop_option_types_ResolvedType_ptr env_env_lookup_constant(env_TypeEnv* env, slop_string name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type _mv_931 = current_mod;
        ...
        if (!(found)) {
            __auto_type _mv_933 = env_env_resolve_import(env, name);
            if (_mv_933.has_value) {
                __auto_type qualified_name = _mv_933.value;
                {
...
slop_option_types_ResolvedType_ptr env_env_lookup_type(env_TypeEnv* env, slop_string name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_937 = env_env_lookup_type_direct(env, name);
...
uint8_t env_env_is_type_visible(env_TypeEnv* env, types_ResolvedType* t) {
    ...
    if (!_mv_942.has_value) {
        ...
        __auto_type _mv_943 = env_env_get_module(env);
        if (_mv_943.has_value) {
            __auto_type current = _mv_943.value;
...
uint8_t env_env_is_function_visible(env_TypeEnv* env, types_FnSignature* sig) {
    ...
    __auto_type _mv_944 = (*sig).module_name;
    if (!_mv_944.has_value) {
        ...
        if (_mv_945.has_value) {
            __auto_type current = _mv_945.value;
            return string_eq(mod, current);
...
slop_option_types_FnSignature_ptr env_env_lookup_function_direct(env_TypeEnv* env, slop_string name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_946 = ({ __auto_type _lst = functions; size_t _idx = (size_t)i; slop_option_types_FnSignature_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_option_types_FnSignature_ptr env_env_lookup_function(env_TypeEnv* env, slop_string name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_947 = env_env_lookup_function_direct(env, name);
    if (_mv_947.has_value) {
        ...
        if (env_env_is_function_visible(env, sig)) {
            ...
            if (_mv_948.has_value) {
                __auto_type import_qualified = _mv_948.value;
        ...
        if (_mv_950.has_value) {
            __auto_type import_qualified = _mv_950.value;
            return env_env_lookup_function_direct(env, import_qualified);
        } else if (!_mv_950.has_value) {
            __auto_type _mv_951 = env_env_get_module(env);
...
slop_option_string env_env_resolve_import(env_TypeEnv* env, slop_string local_name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_952 = ({ __auto_type _lst = imports; size_t _idx = (size_t)i; slop_option_env_ImportEntry _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_952.has_value) {
                __auto_type entry = _mv_952.value;
...
uint8_t env_env_variant_matches_module(env_VariantMapping v, slop_string mod_name) {
    ...
    if (_mv_953.has_value) {
        __auto_type vmod = _mv_953.value;
...
slop_option_string env_env_lookup_variant(env_TypeEnv* env, slop_string variant_name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        __auto_type _mv_955 = current_mod;
                    ...
                    if (_mv_958.has_value) {
                        ...
                        if (string_eq(v.variant_name, variant_name)) {
                            ...
                            if (_mv_959.has_value) {
                                __auto_type vmod = _mv_959.value;
                                __auto_type _mv_960 = env_env_resolve_import(env, v.enum_name);
            ...
            if (_mv_961.has_value) {
                __auto_type v1 = _mv_961.value;
                {
                    ...
                    for (int64_t j = (i + 1); j < len; j++) {
                        __auto_type _mv_962 = ({ __auto_type _lst = variants; size_t _idx = (size_t)j; slop_option_env_VariantMapping _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_962.has_value) {
                            __auto_type v2 = _mv_962.value;
...
uint8_t env_env_same_module_opt(slop_option_string a, slop_option_string b) {
    ...
    if (!_mv_963.has_value) {
        ...
        __auto_type amod = _mv_963.value;
...
uint8_t env_env_is_module_loaded(env_TypeEnv* env, slop_string module_path) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        uint8_t found = 0;
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_966 = ({ __auto_type _lst = modules; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_966.has_value) {
                __auto_type path = _mv_966.value;
...
uint8_t env_env_is_type_param(env_TypeEnv* env, slop_string name) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            if (!(found)) {
                __auto_type _mv_967 = ({ __auto_type _lst = params; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_967.has_value) {
                    __auto_type p = _mv_967.value;
                    if (string_eq(p, name)) {
                        found = 1;
                    }
