...
slop_string compiler_lines_to_string(slop_arena* arena, slop_list_string lines) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_1879 = ({ __auto_type _lst = lines; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1879.has_value) {
                        __auto_type line = _mv_1879.value;
                ...
                }
                {
                    ...
                    int64_t j = 0;
                    while (j < len) {
                        __auto_type _mv_1880 = ({ __auto_type _lst = lines; size_t _idx = (size_t)j; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1880.has_value) {
                            __auto_type line = _mv_1880.value;
                            {
...
slop_string compiler_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_1881 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_1881.has_value) {
            __auto_type first_expr = _mv_1881.value;
            __auto_type _mv_1882 = (*first_expr);
            switch (_mv_1882.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_1882.data.lst;
                    {
                        __auto_type items = lst.items;
                        if (((int64_t)((items).len)) < 2) {
                            return SLOP_STR("unknown");
                        } else {
                            __auto_type _mv_1883 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_1883.has_value) {
                                __auto_type head = _mv_1883.value;
                                __auto_type _mv_1884 = (*head);
                                switch (_mv_1884.tag) {
                                    case types_SExpr_sym:
                                    {
                                        __auto_type sym = _mv_1884.data.sym;
                                        if (string_eq(sym.name, SLOP_STR("module"))) {
                                            ...
                                            if (_mv_1885.has_value) {
                                                __auto_type name_expr = _mv_1885.value;
                                                __auto_type _mv_1886 = (*name_expr);
                                                switch (_mv_1886.tag) {
                                                    case types_SExpr_sym:
                                                    {
                                                        __auto_type name_sym = _mv_1886.data.sym;
...
void compiler_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1887 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1887.has_value) {
                __auto_type expr = _mv_1887.value;
...
uint8_t compiler_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1888 = parser_sexpr_list_get(item, 0);
        if (_mv_1888.has_value) {
            __auto_type head = _mv_1888.value;
...
slop_string compiler_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1889 = parser_sexpr_list_get(item, 0);
        if (_mv_1889.has_value) {
            __auto_type head = _mv_1889.value;
...
void compiler_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_1890 = parser_sexpr_list_get(module_form, 1);
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_1891 = parser_sexpr_list_get(module_form, i);
                if (_mv_1891.has_value) {
                    __auto_type item = _mv_1891.value;
                    if (parser_is_form(item, SLOP_STR("fn"))) {
                        {
...
int64_t compiler_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1892 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1892.has_value) {
                __auto_type diag = _mv_1892.value;
                __auto_type _mv_1893 = diag.level;
                if (_mv_1893 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_1893 == types_DiagnosticLevel_diag_warning) {
                }
...
void compiler_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_1894 = diag.level;
    if (_mv_1894 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_1894 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void compiler_output_diagnostics_text(slop_arena* arena, slop_string filename, slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1895 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1895.has_value) {
                __auto_type diag = _mv_1895.value;
...
void compiler_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_1896 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1896.has_value) {
                __auto_type diag = _mv_1896.value;
...
void compiler_output_single_diagnostic_json(slop_arena* arena, types_Diagnostic diag) {
    ...
    __auto_type _mv_1897 = diag.level;
    if (_mv_1897 == types_DiagnosticLevel_diag_warning) {
        compiler_print_str(((uint8_t*)(SLOP_STR("\"warning\"").data)));
    } else if (_mv_1897 == types_DiagnosticLevel_diag_error) {
...
int64_t compiler_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, compiler_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_1898 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1898.is_ok) {
            __auto_type e = _mv_1898.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_1898.is_ok) {
            __auto_type f = _mv_1898.data.ok;
            __auto_type _mv_1899 = file_file_read_all(arena, (&f));
            if (!_mv_1899.is_ok) {
                __auto_type e = _mv_1899.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_1899.is_ok) {
                __auto_type source = _mv_1899.data.ok;
                file_file_close((&f));
                __auto_type _mv_1900 = parser_parse(arena, source);
                if (_mv_1900.is_ok) {
                    __auto_type ast = _mv_1900.data.ok;
                    {
                ...
                } else if (!_mv_1900.is_ok) {
                    __auto_type parse_err = _mv_1900.data.err;
...
types_ResolvedType* compiler_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1901 = parser_parse(arena, type_str);
    if (_mv_1901.is_ok) {
        __auto_type type_ast = _mv_1901.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_1902 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1902.has_value) {
                __auto_type type_expr = _mv_1902.value;
    ...
    } else if (!_mv_1901.is_ok) {
        __auto_type _ = _mv_1901.data.err;
        return env_env_get_int_type(env);
    }
    SLOP_UNREACHABLE();
}
...
void compiler_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1903 = parser_parse(arena, params_str);
    if (_mv_1903.is_ok) {
        __auto_type params_ast = _mv_1903.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_1904 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1904.has_value) {
                __auto_type params_list = _mv_1904.value;
                __auto_type _mv_1905 = (*params_list);
                switch (_mv_1905.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_1905.data.lst;
                        {
                            __auto_type items = lst.items;
                            __auto_type len = ((int64_t)((items).len));
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_1906 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1906.has_value) {
                                    __auto_type param = _mv_1906.value;
                                    __auto_type _mv_1907 = (*param);
                                    switch (_mv_1907.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_1907.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_1908 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                    if (_mv_1908.has_value) {
                                                        __auto_type name_expr = _mv_1908.value;
                                                        __auto_type _mv_1909 = (*name_expr);
                                                        switch (_mv_1909.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1909.data.sym;
                                                                {
                                                                    __auto_type param_name = name_sym.name;
                                                                    __auto_type _mv_1910 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_1910.has_value) {
                                                                        __auto_type type_expr = _mv_1910.value;
                                                                        __auto_type _mv_1911 = (*type_expr);
                                                                        switch (_mv_1911.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_1911.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_1911.data.lst;
                                                                                {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            if (_mv_1912.has_value) {
                __auto_type a_inner = _mv_1912.value;
                __auto_type _mv_1913 = (*b).inner_type;
                if (_mv_1913.has_value) {
                    __auto_type b_inner = _mv_1913.value;
            ...
            __auto_type _mv_1914 = (*a).inner_type;
            if (_mv_1914.has_value) {
                __auto_type a_inner = _mv_1914.value;
                __auto_type _mv_1915 = (*b).inner_type;
                if (_mv_1915.has_value) {
                    __auto_type b_inner = _mv_1915.value;
                    return compiler_types_names_equal(a_inner, b_inner);
            ...
            __auto_type _mv_1916 = (*a).inner_type;
            if (_mv_1916.has_value) {
                __auto_type base = _mv_1916.value;
            ...
            __auto_type _mv_1917 = (*b).inner_type;
            if (_mv_1917.has_value) {
                __auto_type base = _mv_1917.value;
            ...
            __auto_type _mv_1918 = (*a).inner_type;
            if (_mv_1918.has_value) {
                __auto_type a_base = _mv_1918.value;
            ...
            __auto_type _mv_1919 = (*b).inner_type;
            if (_mv_1919.has_value) {
                __auto_type b_base = _mv_1919.value;
...
int64_t compiler_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_1920 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_1920.is_ok) {
            __auto_type _ = _mv_1920.data.err;
        } else if (_mv_1920.is_ok) {
            __auto_type f = _mv_1920.data.ok;
            __auto_type _mv_1921 = file_file_read_all(arena, (&f));
            if (!_mv_1921.is_ok) {
                __auto_type _ = _mv_1921.data.err;
                file_file_close((&f));
...
int64_t compiler_transpile_single_file(env_TypeEnv* env, context_TranspileContext* ctx, slop_arena* arena, uint8_t* filename, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_1923 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1923.is_ok) {
            __auto_type e = _mv_1923.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_1923.is_ok) {
            __auto_type f = _mv_1923.data.ok;
            __auto_type _mv_1924 = file_file_read_all(arena, (&f));
            if (!_mv_1924.is_ok) {
                __auto_type e = _mv_1924.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_1924.is_ok) {
                __auto_type source = _mv_1924.data.ok;
                file_file_close((&f));
                __auto_type _mv_1925 = parser_parse(arena, source);
                if (_mv_1925.is_ok) {
                    __auto_type ast = _mv_1925.data.ok;
                    {
                        __auto_type mod_name = compiler_extract_module_name(ast);
                        env_env_clear_imports(env);
                        env_env_clear_diagnostics(env);
                    ...
                    __auto_type parse_err = _mv_1925.data.err;
                    printf("%.*s", (int)(filename_str).len, (filename_str).data);
                    printf("%s", ":");
                    printf("%lld", (long long)(parse_err.line));
                    printf("%s", ":");
...
slop_string compiler_emit_sexpr_inner(slop_arena* arena, types_SExpr* expr) {
    ...
    __auto_type _mv_1926 = (*expr);
    switch (_mv_1926.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_1926.data.sym;
            return sym.name;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1926.data.str;
            return string_concat(arena, SLOP_STR("\""), string_concat(arena, str.value, SLOP_STR("\"")));
        }
        case types_SExpr_num:
        {
            __auto_type num = _mv_1926.data.num;
            return num.raw;
        }
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_1926.data.lst;
...
slop_string compiler_emit_typed_sexpr(slop_arena* arena, types_SExpr* expr) {
    SLOP_PRE(((expr != NULL)), "(!= expr nil)");
    {
        ...
        __auto_type _mv_1927 = type_opt;
        if (_mv_1927.has_value) {
            __auto_type rt = _mv_1927.value;
            {
...
slop_string compiler_emit_typed_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1928 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1928.has_value) {
                __auto_type item = _mv_1928.value;
                {
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            int64_t i = 0;
            while (i < len) {
                __auto_type _mv_1929 = parser_sexpr_list_get(module_form, i);
                if (_mv_1929.has_value) {
                    __auto_type item = _mv_1929.value;
                    {
...
slop_string compiler_emit_typed_ast(slop_arena* arena, slop_list_types_SExpr_ptr ast) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1930 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1930.has_value) {
                __auto_type expr = _mv_1930.value;
                {
...
int64_t compiler_typed_ast_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, compiler_OutputFormat format, uint8_t first) {
    ...
    {
        __auto_type filename_str = strlib_cstring_to_string(filename);
        __auto_type _mv_1931 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1931.is_ok) {
            __auto_type e = _mv_1931.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_1931.is_ok) {
            __auto_type f = _mv_1931.data.ok;
            __auto_type _mv_1932 = file_file_read_all(arena, (&f));
            if (!_mv_1932.is_ok) {
                __auto_type e = _mv_1932.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_1932.is_ok) {
                __auto_type source = _mv_1932.data.ok;
                file_file_close((&f));
                __auto_type _mv_1933 = parser_parse(arena, source);
                if (_mv_1933.is_ok) {
                    __auto_type ast = _mv_1933.data.ok;
                    {
                        __auto_type mod_name = compiler_extract_module_name(ast);
                        env_env_clear_imports(env);
                        env_env_clear_diagnostics(env);
                ...
                } else if (!_mv_1933.is_ok) {
                    __auto_type parse_err = _mv_1933.data.err;
                    printf("%.*s", (int)(filename_str).len, (filename_str).data);
                    printf("%s", ":");
                    printf("%lld", (long long)(parse_err.line));
                    printf("%s", ":");
