...
slop_string compiler_lines_to_string(slop_arena* arena, slop_list_string lines) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_1848 = ({ __auto_type _lst = lines; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1848.has_value) {
                        __auto_type line = _mv_1848.value;
                ...
                }
                {
                    ...
                    int64_t j = 0;
                    while (j < len) {
                        __auto_type _mv_1849 = ({ __auto_type _lst = lines; size_t _idx = (size_t)j; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1849.has_value) {
                            __auto_type line = _mv_1849.value;
                            {
...
slop_string compiler_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_1850 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_1850.has_value) {
            __auto_type first_expr = _mv_1850.value;
            __auto_type _mv_1851 = (*first_expr);
            switch (_mv_1851.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_1851.data.lst;
                    {
                        __auto_type items = lst.items;
                        if (((int64_t)((items).len)) < 2) {
                            return SLOP_STR("unknown");
                        } else {
                            __auto_type _mv_1852 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_1852.has_value) {
                                __auto_type head = _mv_1852.value;
                                __auto_type _mv_1853 = (*head);
                                switch (_mv_1853.tag) {
                                    case types_SExpr_sym:
                                    {
                                        ...
                                        if (string_eq(sym.name, SLOP_STR("module"))) {
                                            ...
                                            if (_mv_1854.has_value) {
                                                __auto_type name_expr = _mv_1854.value;
                                                __auto_type _mv_1855 = (*name_expr);
                                                switch (_mv_1855.tag) {
                                                    case types_SExpr_sym:
                                                    {
                                                        __auto_type name_sym = _mv_1855.data.sym;
...
void compiler_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1856 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1856.has_value) {
                __auto_type expr = _mv_1856.value;
...
uint8_t compiler_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1857 = parser_sexpr_list_get(item, 0);
...
slop_string compiler_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1858 = parser_sexpr_list_get(item, 0);
...
void compiler_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_1859 = parser_sexpr_list_get(module_form, 1);
        if (_mv_1859.has_value) {
            __auto_type name_expr = _mv_1859.value;
            {
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_1860 = parser_sexpr_list_get(module_form, i);
                if (_mv_1860.has_value) {
                    __auto_type item = _mv_1860.value;
                    if (parser_is_form(item, SLOP_STR("fn"))) {
                        {
...
int64_t compiler_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1861 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1861.has_value) {
                __auto_type diag = _mv_1861.value;
                __auto_type _mv_1862 = diag.level;
                if (_mv_1862 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_1862 == types_DiagnosticLevel_diag_warning) {
                }
...
void compiler_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_1863 = diag.level;
    if (_mv_1863 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_1863 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void compiler_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_1865 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1865.has_value) {
                __auto_type diag = _mv_1865.value;
...
int64_t compiler_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, compiler_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_1867 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1867.is_ok) {
            __auto_type e = _mv_1867.data.err;
            ...
            if (!_mv_1868.is_ok) {
                __auto_type e = _mv_1868.data.err;
                file_file_close((&f));
                ...
                if (_mv_1869.is_ok) {
                    __auto_type ast = _mv_1869.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_1869.data.err;
...
types_ResolvedType* compiler_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1870 = parser_parse(arena, type_str);
    if (_mv_1870.is_ok) {
        __auto_type type_ast = _mv_1870.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_1871 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        ...
        __auto_type _ = _mv_1870.data.err;
        return env_env_get_int_type(env);
    }
    SLOP_UNREACHABLE();
}
...
void compiler_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1872 = parser_parse(arena, params_str);
    if (_mv_1872.is_ok) {
        __auto_type params_ast = _mv_1872.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_1873 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1873.has_value) {
                __auto_type params_list = _mv_1873.value;
                __auto_type _mv_1874 = (*params_list);
                switch (_mv_1874.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_1874.data.lst;
                        {
                            __auto_type items = lst.items;
                            __auto_type len = ((int64_t)((items).len));
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_1875 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1875.has_value) {
                                    __auto_type param = _mv_1875.value;
                                    __auto_type _mv_1876 = (*param);
                                    switch (_mv_1876.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_1876.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_1877 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                    if (_mv_1877.has_value) {
                                                        __auto_type name_expr = _mv_1877.value;
                                                        __auto_type _mv_1878 = (*name_expr);
                                                        switch (_mv_1878.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1878.data.sym;
                                                                {
                                                                    __auto_type param_name = name_sym.name;
                                                                    __auto_type _mv_1879 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_1879.has_value) {
                                                                        __auto_type type_expr = _mv_1879.value;
                                                                        __auto_type _mv_1880 = (*type_expr);
                                                                        switch (_mv_1880.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_1880.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_1880.data.lst;
                                                                                {
        ...
        __auto_type _ = _mv_1872.data.err;
    }
}
...
uint8_t compiler_types_names_equal(types_ResolvedType* a, types_ResolvedType* b) {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            __auto_type _mv_1881 = (*a).inner_type;
            if (_mv_1881.has_value) {
                __auto_type a_inner = _mv_1881.value;
                __auto_type _mv_1882 = (*b).inner_type;
                if (_mv_1882.has_value) {
                    __auto_type b_inner = _mv_1882.value;
            ...
            __auto_type _mv_1883 = (*a).inner_type;
            if (_mv_1883.has_value) {
                __auto_type a_inner = _mv_1883.value;
                __auto_type _mv_1884 = (*b).inner_type;
                if (_mv_1884.has_value) {
                    __auto_type b_inner = _mv_1884.value;
                    return compiler_types_names_equal(a_inner, b_inner);
            ...
            __auto_type _mv_1885 = (*a).inner_type;
            if (_mv_1885.has_value) {
                __auto_type base = _mv_1885.value;
            ...
            __auto_type _mv_1886 = (*b).inner_type;
            if (_mv_1886.has_value) {
                __auto_type base = _mv_1886.value;
            ...
            if (_mv_1887.has_value) {
                __auto_type a_base = _mv_1887.value;
            ...
            if (_mv_1888.has_value) {
                __auto_type b_base = _mv_1888.value;
...
int64_t compiler_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_1889 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_1889.is_ok) {
            __auto_type _ = _mv_1889.data.err;
...
slop_string compiler_emit_sexpr_inner(slop_arena* arena, types_SExpr* expr) {
    ...
    __auto_type _mv_1895 = (*expr);
    switch (_mv_1895.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_1895.data.sym;
            return sym.name;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1895.data.str;
            return string_concat(arena, SLOP_STR("\""), string_concat(arena, str.value, SLOP_STR("\"")));
        }
        case types_SExpr_num:
        {
            __auto_type num = _mv_1895.data.num;
            return num.raw;
        }
        case types_SExpr_lst:
        {
            __auto_type lst = _mv_1895.data.lst;
...
slop_string compiler_emit_typed_sexpr(slop_arena* arena, types_SExpr* expr) {
    SLOP_PRE(((expr != NULL)), "(!= expr nil)");
    {
        ...
        __auto_type _mv_1896 = type_opt;
        if (_mv_1896.has_value) {
            __auto_type rt = _mv_1896.value;
            {
...
slop_string compiler_emit_typed_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_1897.has_value) {
                __auto_type item = _mv_1897.value;
                {
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            int64_t i = 0;
            while (i < len) {
                __auto_type _mv_1898 = parser_sexpr_list_get(module_form, i);
                if (_mv_1898.has_value) {
                    __auto_type item = _mv_1898.value;
                    {
