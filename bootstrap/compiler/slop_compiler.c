...
slop_string compiler_lines_to_string(slop_arena* arena, slop_list_string lines) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_1858 = ({ __auto_type _lst = lines; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1858.has_value) {
                        __auto_type line = _mv_1858.value;
                ...
                }
                {
                    ...
                    int64_t j = 0;
                    while (j < len) {
                        __auto_type _mv_1859 = ({ __auto_type _lst = lines; size_t _idx = (size_t)j; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1859.has_value) {
                            __auto_type line = _mv_1859.value;
                            {
...
slop_string compiler_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_1860 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_1860.has_value) {
            __auto_type first_expr = _mv_1860.value;
            __auto_type _mv_1861 = (*first_expr);
            switch (_mv_1861.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_1861.data.lst;
                    {
                        __auto_type items = lst.items;
                        if (((int64_t)((items).len)) < 2) {
                            return SLOP_STR("unknown");
                        } else {
                            __auto_type _mv_1862 = ({ __auto_type _lst = items; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_1862.has_value) {
                                __auto_type head = _mv_1862.value;
                                __auto_type _mv_1863 = (*head);
                                switch (_mv_1863.tag) {
                                    case types_SExpr_sym:
                                    {
                                        __auto_type sym = _mv_1863.data.sym;
                                        if (string_eq(sym.name, SLOP_STR("module"))) {
                                            ...
                                            if (_mv_1864.has_value) {
                                                __auto_type name_expr = _mv_1864.value;
                                                __auto_type _mv_1865 = (*name_expr);
                                                switch (_mv_1865.tag) {
                                                    case types_SExpr_sym:
                                                    {
                                                        __auto_type name_sym = _mv_1865.data.sym;
...
void compiler_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1866 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1866.has_value) {
                __auto_type expr = _mv_1866.value;
...
uint8_t compiler_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1867 = parser_sexpr_list_get(item, 0);
        if (_mv_1867.has_value) {
            __auto_type head = _mv_1867.value;
...
slop_string compiler_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1868 = parser_sexpr_list_get(item, 0);
        if (_mv_1868.has_value) {
            __auto_type head = _mv_1868.value;
...
void compiler_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_1869 = parser_sexpr_list_get(module_form, 1);
        if (_mv_1869.has_value) {
            __auto_type name_expr = _mv_1869.value;
            {
...
int64_t compiler_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_1871.has_value) {
                ...
                if (_mv_1872 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_1872 == types_DiagnosticLevel_diag_warning) {
                }
...
void compiler_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_1873 = diag.level;
    if (_mv_1873 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_1873 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void compiler_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_1875 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1875.has_value) {
                __auto_type diag = _mv_1875.value;
...
types_ResolvedType* compiler_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1880 = parser_parse(arena, type_str);
    if (_mv_1880.is_ok) {
        __auto_type type_ast = _mv_1880.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_1881 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
void compiler_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1882 = parser_parse(arena, params_str);
    if (_mv_1882.is_ok) {
        __auto_type params_ast = _mv_1882.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_1883 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1883.has_value) {
                __auto_type params_list = _mv_1883.value;
                __auto_type _mv_1884 = (*params_list);
                switch (_mv_1884.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_1884.data.lst;
                        {
                            ...
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_1885 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1885.has_value) {
                                    __auto_type param = _mv_1885.value;
                                    __auto_type _mv_1886 = (*param);
                                    switch (_mv_1886.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_1886.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_1887 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                        ...
                                                        switch (_mv_1888.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1888.data.sym;
                                                                {
                                                                    ...
                                                                    __auto_type _mv_1889 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_1889.has_value) {
                                                                        __auto_type type_expr = _mv_1889.value;
                                                                        __auto_type _mv_1890 = (*type_expr);
                                                                        switch (_mv_1890.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_1890.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_1890.data.lst;
                                                                                {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            if (_mv_1891.has_value) {
                __auto_type a_inner = _mv_1891.value;
                __auto_type _mv_1892 = (*b).inner_type;
                if (_mv_1892.has_value) {
                    __auto_type b_inner = _mv_1892.value;
            ...
            if (_mv_1895.has_value) {
                __auto_type base = _mv_1895.value;
            ...
            if (_mv_1897.has_value) {
                __auto_type a_base = _mv_1897.value;
            ...
            if (_mv_1898.has_value) {
                __auto_type b_base = _mv_1898.value;
...
int64_t compiler_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_1899 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_1899.is_ok) {
            __auto_type _ = _mv_1899.data.err;
        } else if (_mv_1899.is_ok) {
            __auto_type f = _mv_1899.data.ok;
            __auto_type _mv_1900 = file_file_read_all(arena, (&f));
            if (!_mv_1900.is_ok) {
                __auto_type _ = _mv_1900.data.err;
                file_file_close((&f));
            } else if (_mv_1900.is_ok) {
                __auto_type source = _mv_1900.data.ok;
                file_file_close((&f));
                __auto_type _mv_1901 = parser_parse(arena, source);
                if (_mv_1901.is_ok) {
                    __auto_type context_ast = _mv_1901.data.ok;
...
int64_t compiler_transpile_single_file(env_TypeEnv* env, context_TranspileContext* ctx, slop_arena* arena, uint8_t* filename, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_1902 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1902.is_ok) {
            __auto_type e = _mv_1902.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
            ...
            if (!_mv_1903.is_ok) {
                ...
                if (_mv_1904.is_ok) {
                    __auto_type ast = _mv_1904.data.ok;
                    {
                        __auto_type mod_name = compiler_extract_module_name(ast);
                        env_env_clear_imports(env);
                        env_env_clear_diagnostics(env);
                    ...
                    __auto_type parse_err = _mv_1904.data.err;
                    printf("%.*s", (int)(filename_str).len, (filename_str).data);
                    printf("%s", ":");
                    printf("%lld", (long long)(parse_err.line));
                    printf("%s", ":");
...
slop_string compiler_emit_sexpr_inner(slop_arena* arena, types_SExpr* expr) {
    ...
    __auto_type _mv_1905 = (*expr);
    switch (_mv_1905.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_1905.data.sym;
            return sym.name;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1905.data.str;
            return string_concat(arena, SLOP_STR("\""), string_concat(arena, str.value, SLOP_STR("\"")));
        }
        case types_SExpr_num:
        {
            __auto_type num = _mv_1905.data.num;
            return num.raw;
        }
        case types_SExpr_lst:
        {
...
slop_string compiler_emit_typed_sexpr(slop_arena* arena, types_SExpr* expr) {
    SLOP_PRE(((expr != NULL)), "(!= expr nil)");
    {
        ...
        __auto_type _mv_1906 = type_opt;
        if (_mv_1906.has_value) {
            __auto_type rt = _mv_1906.value;
            {
...
slop_string compiler_emit_typed_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_1907.has_value) {
                __auto_type item = _mv_1907.value;
                {
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            int64_t i = 0;
            while (i < len) {
                __auto_type _mv_1908 = parser_sexpr_list_get(module_form, i);
                if (_mv_1908.has_value) {
                    __auto_type item = _mv_1908.value;
                    {
...
slop_string compiler_emit_typed_ast(slop_arena* arena, slop_list_types_SExpr_ptr ast) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1909 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1909.has_value) {
                __auto_type expr = _mv_1909.value;
                {
...
int64_t compiler_typed_ast_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, compiler_OutputFormat format, uint8_t first) {
    ...
    {
        __auto_type filename_str = strlib_cstring_to_string(filename);
        __auto_type _mv_1910 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1910.is_ok) {
            __auto_type e = _mv_1910.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_1910.is_ok) {
            __auto_type f = _mv_1910.data.ok;
            __auto_type _mv_1911 = file_file_read_all(arena, (&f));
            if (!_mv_1911.is_ok) {
                __auto_type e = _mv_1911.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_1911.is_ok) {
                __auto_type source = _mv_1911.data.ok;
                file_file_close((&f));
                __auto_type _mv_1912 = parser_parse(arena, source);
                if (_mv_1912.is_ok) {
                    __auto_type ast = _mv_1912.data.ok;
                    {
                        __auto_type mod_name = compiler_extract_module_name(ast);
                        env_env_clear_imports(env);
                        env_env_clear_diagnostics(env);
                    ...
                    __auto_type parse_err = _mv_1912.data.err;
                    printf("%.*s", (int)(filename_str).len, (filename_str).data);
                    printf("%s", ":");
                    printf("%lld", (long long)(parse_err.line));
                    printf("%s", ":");
