...
slop_string compiler_lines_to_string(slop_arena* arena, slop_list_string lines) {
    {
        ...
        if (len == 0) {
        ...
        } else {
            {
                ...
                int64_t i = 0;
                while (i < len) {
                    __auto_type _mv_1817 = ({ __auto_type _lst = lines; size_t _idx = (size_t)i; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1817.has_value) {
                        __auto_type line = _mv_1817.value;
                ...
                }
                {
                    ...
                    int64_t j = 0;
                    while (j < len) {
                        __auto_type _mv_1818 = ({ __auto_type _lst = lines; size_t _idx = (size_t)j; slop_option_string _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_1818.has_value) {
                            __auto_type line = _mv_1818.value;
                            {
...
slop_string compiler_extract_module_name(slop_list_types_SExpr_ptr exprs) {
    if (((int64_t)((exprs).len)) < 1) {
        ...
        __auto_type _mv_1819 = ({ __auto_type _lst = exprs; size_t _idx = (size_t)0; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
        if (_mv_1819.has_value) {
            __auto_type first_expr = _mv_1819.value;
            __auto_type _mv_1820 = (*first_expr);
            switch (_mv_1820.tag) {
                case types_SExpr_lst:
                {
                    __auto_type lst = _mv_1820.data.lst;
                    {
...
void compiler_check_all_functions(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1825 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1825.has_value) {
                __auto_type expr = _mv_1825.value;
...
uint8_t compiler_is_annotation_form(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1826 = parser_sexpr_list_get(item, 0);
        if (_mv_1826.has_value) {
            __auto_type head = _mv_1826.value;
...
slop_string compiler_get_form_name(types_SExpr* item) {
    if (parser_sexpr_is_list(item)) {
        __auto_type _mv_1827 = parser_sexpr_list_get(item, 0);
...
void compiler_check_module_functions(env_TypeEnv* env, types_SExpr* module_form) {
    ...
    if (parser_sexpr_is_list(module_form)) {
        __auto_type _mv_1828 = parser_sexpr_list_get(module_form, 1);
        if (_mv_1828.has_value) {
            __auto_type name_expr = _mv_1828.value;
            {
        ...
        }
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_1829 = parser_sexpr_list_get(module_form, i);
                if (_mv_1829.has_value) {
                    __auto_type item = _mv_1829.value;
                    if (parser_is_form(item, SLOP_STR("fn"))) {
                        {
...
int64_t compiler_count_errors(slop_list_types_Diagnostic diagnostics) {
    {
        ...
        while (i < len) {
            ...
            if (_mv_1830.has_value) {
                ...
                if (_mv_1831 == types_DiagnosticLevel_diag_error) {
                    errors = (errors + 1);
                } else if (_mv_1831 == types_DiagnosticLevel_diag_warning) {
                }
...
void compiler_print_diagnostic(slop_arena* arena, slop_string filename, types_Diagnostic diag) {
    ...
    printf("%s", ": ");
    __auto_type _mv_1832 = diag.level;
    if (_mv_1832 == types_DiagnosticLevel_diag_warning) {
        printf("%s", "warning: ");
    } else if (_mv_1832 == types_DiagnosticLevel_diag_error) {
        printf("%s", "error: ");
    }
...
void compiler_output_diagnostics_json(slop_arena* arena, slop_list_types_Diagnostic diagnostics) {
    putchar(91);
    {
        ...
        while (i < len) {
            ...
            __auto_type _mv_1834 = ({ __auto_type _lst = diagnostics; size_t _idx = (size_t)i; slop_option_types_Diagnostic _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1834.has_value) {
                __auto_type diag = _mv_1834.value;
...
int64_t compiler_check_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, compiler_OutputFormat format, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_1836 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1836.is_ok) {
            __auto_type e = _mv_1836.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_1836.is_ok) {
            __auto_type f = _mv_1836.data.ok;
            __auto_type _mv_1837 = file_file_read_all(arena, (&f));
            if (!_mv_1837.is_ok) {
                __auto_type e = _mv_1837.data.err;
                file_file_close((&f));
                printf("%s", "Error: Could not read file: ");
                printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
                return 1;
            } else if (_mv_1837.is_ok) {
                __auto_type source = _mv_1837.data.ok;
                file_file_close((&f));
                __auto_type _mv_1838 = parser_parse(arena, source);
                if (_mv_1838.is_ok) {
                    __auto_type ast = _mv_1838.data.ok;
                    {
                    ...
                    __auto_type parse_err = _mv_1838.data.err;
...
types_ResolvedType* compiler_resolve_type_string(env_TypeEnv* env, slop_arena* arena, slop_string type_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1839 = parser_parse(arena, type_str);
    if (_mv_1839.is_ok) {
        __auto_type type_ast = _mv_1839.data.ok;
        if (((int64_t)((type_ast).len)) == 0) {
            return env_env_get_int_type(env);
        } else {
            __auto_type _mv_1840 = ({ __auto_type _lst = type_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1840.has_value) {
                __auto_type type_expr = _mv_1840.value;
...
void compiler_parse_and_bind_params(env_TypeEnv* env, slop_arena* arena, slop_string params_str) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    __auto_type _mv_1841 = parser_parse(arena, params_str);
    if (_mv_1841.is_ok) {
        __auto_type params_ast = _mv_1841.data.ok;
        if (((int64_t)((params_ast).len)) > 0) {
            __auto_type _mv_1842 = ({ __auto_type _lst = params_ast; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1842.has_value) {
                __auto_type params_list = _mv_1842.value;
                __auto_type _mv_1843 = (*params_list);
                switch (_mv_1843.tag) {
                    case types_SExpr_lst:
                    {
                        __auto_type lst = _mv_1843.data.lst;
                        {
                            __auto_type items = lst.items;
                            __auto_type len = ((int64_t)((items).len));
                            for (int64_t i = 0; i < len; i++) {
                                __auto_type _mv_1844 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1844.has_value) {
                                    __auto_type param = _mv_1844.value;
                                    __auto_type _mv_1845 = (*param);
                                    switch (_mv_1845.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type param_lst = _mv_1845.data.lst;
                                            {
                                                __auto_type param_items = param_lst.items;
                                                if (((int64_t)((param_items).len)) >= 2) {
                                                    __auto_type _mv_1846 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)0; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                    if (_mv_1846.has_value) {
                                                        __auto_type name_expr = _mv_1846.value;
                                                        __auto_type _mv_1847 = (*name_expr);
                                                        switch (_mv_1847.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1847.data.sym;
                                                                {
                                                                    __auto_type param_name = name_sym.name;
                                                                    __auto_type _mv_1848 = ({ __auto_type _lst = param_items; size_t _idx = (size_t)1; struct { bool has_value; __typeof__(_lst.data[0]) value; } _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                    if (_mv_1848.has_value) {
                                                                        __auto_type type_expr = _mv_1848.value;
                                                                        __auto_type _mv_1849 = (*type_expr);
                                                                        switch (_mv_1849.tag) {
                                                                            case types_SExpr_sym:
                                                                            {
                                                                                __auto_type type_sym = _mv_1849.data.sym;
                                                                                {
                                                                            ...
                                                                            case types_SExpr_lst:
                                                                            {
                                                                                __auto_type _ = _mv_1849.data.lst;
                                                                                {
    ...
    SLOP_PRE(((b != NULL)), "(!= b nil)");
    {
        ...
        if (string_eq(a_name, b_name)) {
            ...
            if (_mv_1854.has_value) {
                __auto_type base = _mv_1854.value;
            ...
            if (_mv_1855.has_value) {
                __auto_type base = _mv_1855.value;
            ...
            if (_mv_1856.has_value) {
                __auto_type a_base = _mv_1856.value;
            ...
            if (_mv_1857.has_value) {
                __auto_type b_base = _mv_1857.value;
...
int64_t compiler_check_expr_mode(slop_arena* arena, env_TypeEnv* env, slop_string expr_str, slop_string type_str, slop_string context_file, slop_string params_str) {
    ...
    if (string_len(context_file) > 0) {
        __auto_type _mv_1858 = file_file_open(context_file, file_FileMode_read);
        if (!_mv_1858.is_ok) {
            ...
            if (!_mv_1859.is_ok) {
                ...
                if (_mv_1860.is_ok) {
                    __auto_type context_ast = _mv_1860.data.ok;
...
int64_t compiler_transpile_single_file(env_TypeEnv* env, context_TranspileContext* ctx, slop_arena* arena, uint8_t* filename, uint8_t first) {
    ...
    SLOP_PRE(((filename != NULL)), "(!= filename nil)");
    {
        ...
        __auto_type _mv_1861 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1861.is_ok) {
            __auto_type e = _mv_1861.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
            ...
            if (!_mv_1862.is_ok) {
                ...
                if (_mv_1863.is_ok) {
                    __auto_type ast = _mv_1863.data.ok;
                    {
                        __auto_type mod_name = compiler_extract_module_name(ast);
                        env_env_clear_imports(env);
                        env_env_clear_diagnostics(env);
                    ...
                    __auto_type parse_err = _mv_1863.data.err;
                    printf("%.*s", (int)(filename_str).len, (filename_str).data);
                    printf("%s", ":");
                    printf("%lld", (long long)(parse_err.line));
                    printf("%s", ":");
...
slop_string compiler_emit_sexpr_inner(slop_arena* arena, types_SExpr* expr) {
    ...
    __auto_type _mv_1864 = (*expr);
    switch (_mv_1864.tag) {
        case types_SExpr_sym:
        {
            __auto_type sym = _mv_1864.data.sym;
            return sym.name;
        }
        case types_SExpr_str:
        {
            __auto_type str = _mv_1864.data.str;
            return string_concat(arena, SLOP_STR("\""), string_concat(arena, str.value, SLOP_STR("\"")));
        }
        case types_SExpr_num:
        {
            __auto_type num = _mv_1864.data.num;
            return num.raw;
        }
        case types_SExpr_lst:
        {
...
slop_string compiler_emit_typed_sexpr(slop_arena* arena, types_SExpr* expr) {
    SLOP_PRE(((expr != NULL)), "(!= expr nil)");
    {
        ...
        __auto_type _mv_1865 = type_opt;
        if (_mv_1865.has_value) {
            __auto_type rt = _mv_1865.value;
            {
...
slop_string compiler_emit_typed_list(slop_arena* arena, slop_list_types_SExpr_ptr items) {
    {
        ...
        int64_t i = 0;
        while (i < len) {
            __auto_type _mv_1866 = ({ __auto_type _lst = items; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            int64_t i = 0;
            while (i < len) {
                __auto_type _mv_1867 = parser_sexpr_list_get(module_form, i);
                if (_mv_1867.has_value) {
                    __auto_type item = _mv_1867.value;
                    {
...
int64_t compiler_typed_ast_single_file(env_TypeEnv* env, slop_arena* arena, uint8_t* filename, compiler_OutputFormat format, uint8_t first) {
    ...
    {
        __auto_type filename_str = strlib_cstring_to_string(filename);
        __auto_type _mv_1869 = file_file_open(filename_str, file_FileMode_read);
        if (!_mv_1869.is_ok) {
            __auto_type e = _mv_1869.data.err;
            printf("%s", "Error: Could not open file: ");
            printf("%.*s\n", (int)(filename_str).len, (filename_str).data);
            return 1;
        } else if (_mv_1869.is_ok) {
            __auto_type f = _mv_1869.data.ok;
            __auto_type _mv_1870 = file_file_read_all(arena, (&f));
            if (!_mv_1870.is_ok) {
                ...
                __auto_type source = _mv_1870.data.ok;
                file_file_close((&f));
                __auto_type _mv_1871 = parser_parse(arena, source);
                if (_mv_1871.is_ok) {
                    __auto_type ast = _mv_1871.data.ok;
                    {
                        __auto_type mod_name = compiler_extract_module_name(ast);
                        env_env_clear_imports(env);
                        env_env_clear_diagnostics(env);
                    ...
                    __auto_type parse_err = _mv_1871.data.err;
                    printf("%.*s", (int)(filename_str).len, (filename_str).data);
                    printf("%s", ":");
                    printf("%lld", (long long)(parse_err.line));
                    printf("%s", ":");
