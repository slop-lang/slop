...
void resolve_resolve_imports(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1829 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1829.has_value) {
                __auto_type expr = _mv_1829.value;
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_1830 = parser_sexpr_list_get(module_form, i);
...
void resolve_resolve_import_stmt(env_TypeEnv* env, types_SExpr* import_form) {
    ...
    SLOP_PRE((parser_is_form(import_form, SLOP_STR("import"))), "(is-form import-form \"import\")");
    {
        ...
        __auto_type _mv_1831 = (*import_form);
        switch (_mv_1831.tag) {
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_1831.data.lst;
                {
                    ...
                    if (_mv_1832.has_value) {
                        __auto_type mod_name_expr = _mv_1832.value;
                        __auto_type _mv_1833 = (*mod_name_expr);
                        switch (_mv_1833.tag) {
                            case types_SExpr_sym:
                            {
                                __auto_type mod_sym = _mv_1833.data.sym;
                                ...
                                if (_mv_1834.has_value) {
                                    __auto_type names_expr = _mv_1834.value;
                                    __auto_type _mv_1835 = (*names_expr);
                                    switch (_mv_1835.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type names_lst = _mv_1835.data.lst;
                                            {
                                                __auto_type name_items = names_lst.items;
                                                __auto_type name_len = ((int64_t)((name_items).len));
                                                for (int64_t j = 0; j < name_len; j++) {
                                                    __auto_type _mv_1836 = ({ __auto_type _lst = name_items; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                    if (_mv_1836.has_value) {
                                                        __auto_type name_expr = _mv_1836.value;
                                                        __auto_type _mv_1837 = (*name_expr);
                                                        switch (_mv_1837.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1837.data.sym;
                                                                {
...
slop_option_string resolve_resolve_module_file(slop_arena* arena, slop_string module_name, slop_option_string from_file) {
    if (!(resolve_contains_slash(module_name))) {
        ...
        __auto_type _mv_1838 = from_file;
        if (_mv_1838.has_value) {
            __auto_type current_path = _mv_1838.value;
            {
