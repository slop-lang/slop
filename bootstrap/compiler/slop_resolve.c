...
void resolve_resolve_imports(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1858 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1858.has_value) {
                __auto_type expr = _mv_1858.value;
    ...
    if (parser_sexpr_is_list(module_form)) {
        {
            ...
            for (int64_t i = 2; i < len; i++) {
                __auto_type _mv_1859 = parser_sexpr_list_get(module_form, i);
                if (_mv_1859.has_value) {
                    __auto_type item = _mv_1859.value;
...
void resolve_resolve_import_stmt(env_TypeEnv* env, types_SExpr* import_form) {
    ...
    SLOP_PRE((parser_is_form(import_form, SLOP_STR("import"))), "(is-form import-form \"import\")");
    {
        ...
        __auto_type _mv_1860 = (*import_form);
        switch (_mv_1860.tag) {
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_1860.data.lst;
                {
                    ...
                    if (_mv_1861.has_value) {
                        __auto_type mod_name_expr = _mv_1861.value;
                        __auto_type _mv_1862 = (*mod_name_expr);
                        switch (_mv_1862.tag) {
                            case types_SExpr_sym:
                            {
                                __auto_type mod_sym = _mv_1862.data.sym;
                                ...
                                if (_mv_1863.has_value) {
                                    __auto_type names_expr = _mv_1863.value;
                                    __auto_type _mv_1864 = (*names_expr);
                                    switch (_mv_1864.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type names_lst = _mv_1864.data.lst;
                                            {
                                                __auto_type name_items = names_lst.items;
                                                __auto_type name_len = ((int64_t)((name_items).len));
                                                for (int64_t j = 0; j < name_len; j++) {
                                                    __auto_type _mv_1865 = ({ __auto_type _lst = name_items; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                    if (_mv_1865.has_value) {
                                                        __auto_type name_expr = _mv_1865.value;
                                                        __auto_type _mv_1866 = (*name_expr);
                                                        switch (_mv_1866.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1866.data.sym;
                                                                {
...
slop_option_string resolve_resolve_module_file(slop_arena* arena, slop_string module_name, slop_option_string from_file) {
    if (!(resolve_contains_slash(module_name))) {
        ...
        __auto_type _mv_1867 = from_file;
        if (_mv_1867.has_value) {
            __auto_type current_path = _mv_1867.value;
            {
