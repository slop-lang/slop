...
void resolve_resolve_imports(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1850 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1850.has_value) {
                __auto_type expr = _mv_1850.value;
...
void resolve_resolve_import_stmt(env_TypeEnv* env, types_SExpr* import_form) {
    ...
    SLOP_PRE((parser_is_form(import_form, SLOP_STR("import"))), "(is-form import-form \"import\")");
    {
        ...
        __auto_type _mv_1852 = (*import_form);
        switch (_mv_1852.tag) {
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_1852.data.lst;
                {
                    ...
                    if (_mv_1853.has_value) {
                        __auto_type mod_name_expr = _mv_1853.value;
                        __auto_type _mv_1854 = (*mod_name_expr);
                        switch (_mv_1854.tag) {
                            case types_SExpr_sym:
                            {
                                __auto_type mod_sym = _mv_1854.data.sym;
                                ...
                                if (_mv_1855.has_value) {
                                    __auto_type names_expr = _mv_1855.value;
                                    __auto_type _mv_1856 = (*names_expr);
                                    switch (_mv_1856.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type names_lst = _mv_1856.data.lst;
                                            {
                                                __auto_type name_items = names_lst.items;
                                                __auto_type name_len = ((int64_t)((name_items).len));
                                                for (int64_t j = 0; j < name_len; j++) {
                                                    __auto_type _mv_1857 = ({ __auto_type _lst = name_items; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
...
slop_option_string resolve_resolve_module_file(slop_arena* arena, slop_string module_name, slop_option_string from_file) {
    if (!(resolve_contains_slash(module_name))) {
        ...
        __auto_type _mv_1859 = from_file;
        if (_mv_1859.has_value) {
            __auto_type current_path = _mv_1859.value;
            {
