...
void resolve_resolve_imports(env_TypeEnv* env, slop_list_types_SExpr_ptr ast) {
    SLOP_PRE(((env != NULL)), "(!= env nil)");
    {
        ...
        for (int64_t i = 0; i < len; i++) {
            __auto_type _mv_1723 = ({ __auto_type _lst = ast; size_t _idx = (size_t)i; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_1723.has_value) {
                __auto_type expr = _mv_1723.value;
...
void resolve_resolve_import_stmt(env_TypeEnv* env, types_SExpr* import_form) {
    ...
    SLOP_PRE((parser_is_form(import_form, SLOP_STR("import"))), "(is-form import-form \"import\")");
    {
        ...
        __auto_type _mv_1725 = (*import_form);
        switch (_mv_1725.tag) {
            case types_SExpr_lst:
            {
                __auto_type lst = _mv_1725.data.lst;
                {
                    __auto_type items = lst.items;
                    __auto_type _mv_1726 = ({ __auto_type _lst = items; size_t _idx = (size_t)1; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_1726.has_value) {
                        __auto_type mod_name_expr = _mv_1726.value;
                        __auto_type _mv_1727 = (*mod_name_expr);
                        switch (_mv_1727.tag) {
                            case types_SExpr_sym:
                            {
                                __auto_type mod_sym = _mv_1727.data.sym;
                                __auto_type _mv_1728 = ({ __auto_type _lst = items; size_t _idx = (size_t)2; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                if (_mv_1728.has_value) {
                                    __auto_type names_expr = _mv_1728.value;
                                    __auto_type _mv_1729 = (*names_expr);
                                    switch (_mv_1729.tag) {
                                        case types_SExpr_lst:
                                        {
                                            __auto_type names_lst = _mv_1729.data.lst;
                                            {
                                                __auto_type name_items = names_lst.items;
                                                __auto_type name_len = ((int64_t)((name_items).len));
                                                for (int64_t j = 0; j < name_len; j++) {
                                                    __auto_type _mv_1730 = ({ __auto_type _lst = name_items; size_t _idx = (size_t)j; slop_option_types_SExpr_ptr _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                    if (_mv_1730.has_value) {
                                                        __auto_type name_expr = _mv_1730.value;
                                                        __auto_type _mv_1731 = (*name_expr);
                                                        switch (_mv_1731.tag) {
                                                            case types_SExpr_sym:
                                                            {
                                                                __auto_type name_sym = _mv_1731.data.sym;
                                                                {
...
slop_option_string resolve_resolve_module_file(slop_arena* arena, slop_string module_name, slop_option_string from_file) {
    if (!(resolve_contains_slash(module_name))) {
        ...
        __auto_type _mv_1732 = from_file;
        if (_mv_1732.has_value) {
            __auto_type current_path = _mv_1732.value;
            {
