...
slop_string path_path_dirname(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_1235 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_1235.has_value) {
                __auto_type idx = _mv_1235.value;
                if ((idx == 0)) {
                    return SLOP_STR("/");
                } else {
...
slop_string path_path_basename(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_1236 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_1236.has_value) {
                __auto_type idx = _mv_1236.value;
                {
