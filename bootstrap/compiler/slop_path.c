...
slop_string path_path_dirname(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_1106 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_1106.has_value) {
                __auto_type idx = _mv_1106.value;
                if ((idx == 0)) {
                    return SLOP_STR("/");
                } else {
...
slop_string path_path_basename(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_1107 = strlib_last_index_of(path, SLOP_STR("/"));
            if (_mv_1107.has_value) {
                __auto_type idx = _mv_1107.value;
                {
...
slop_string path_path_extension(slop_arena* arena, slop_string path) {
    {
        ...
        if ((len == 0)) {
            ...
            __auto_type _mv_1108 = strlib_last_index_of(path, SLOP_STR("."));
            if (_mv_1108.has_value) {
                __auto_type idx = _mv_1108.value;
                {
